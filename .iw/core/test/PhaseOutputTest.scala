...
class PhaseOutputTest extends FunSuite:
  ...
  test("StartOutput.toJson is pretty-printed with indent = 2"):
    ...
    assert(json.contains("  "), s"Expected indentation in pretty-printed JSON, got: $json")

  // CommitOutput tests

  test("CommitOutput.toJson produces valid JSON with correct field names and types"):
