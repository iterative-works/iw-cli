...
class TmuxAdapterSendKeysTest extends FunSuite:
  ...
  override def afterEach(context: AfterEach): Unit =
    ...
    val result = iw.core.adapters.ProcessAdapter.run(Seq("tmux", "list-sessions", "-F", "#{session_name}"))
  ...
  test("sendKeys returns Left when session does not exist"):
    ...
    assert(result.isLeft, s"Expected Left for non-existent session, got: $result")
    val error = result.left.getOrElse("")
    assert(error.contains("Failed to send keys"), s"Error message should mention failure: $error")
    assert(error.contains(nonExistentSession), s"Error message should include session name: $error")
