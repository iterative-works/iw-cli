...
class ReviewStateAdapterTest extends munit.FunSuite:
  ...
  test("read returns Left when file does not exist"):
    val nonExistent = os.Path("/tmp/iw-test-nonexistent-" + System.currentTimeMillis() + "/review-state.json")
    val result = ReviewStateAdapter.read(nonExistent)
    assert(result.isLeft, s"Expected Left but got: $result")

  // ========== update Tests ==========

  test("update reads existing file, merges, validates, and writes updated content to disk"):
    withTempFile(sampleValidJson) { path =>
      ...
      assert(written.contains("In Review"), s"Updated content should contain display text: $written")
    }
  ...
  test("update does NOT write to disk when validation fails after merge"):
    ...
    withTempFile(invalidBaseJson) { path =>
      ...
      val input = ReviewStateUpdater.UpdateInput(displayText = Some("Updated"))
      val result = ReviewStateAdapter.update(path, input)
      assert(result.isLeft, s"Expected Left for validation failure but got: $result")
      // File content should be unchanged
      val currentContent = os.read(path)
      assertEquals(currentContent, originalContent, "File should not be modified when validation fails")
    }

  test("update returns Left with formatted error message listing validation errors when validation fails"):
    val invalidBaseJson =
      """{
        "version": 2,
        "issue_id": "IW-238",
    ...
    withTempFile(invalidBaseJson) { path =>
      ...
      assert(errorMessage.contains("Validation failed"), s"Error should mention validation: $errorMessage")
    }
