...
object PhaseArgs:
  /** Extract the value for a named argument (--name value) from an argument list. */
  ...
  /** Resolve issue ID from CLI arg or feature branch name.
    *
    * Returns Left with an error message if neither source yields a valid issue ID.
    */
  ...
  /** Resolve phase number from CLI arg or a raw phase number string from the branch.
    *
