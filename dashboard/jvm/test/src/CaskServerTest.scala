...
class CaskServerTest extends FunSuite:
  ...
  def startTestServer(statePath: String, port: Int): Thread =
    ...
    val ready = (0 until 50).exists { _ =>
      val isReady = try
        val response =
          quickRequest.get(uri"http://localhost:$port/health").send()
  ...
  test("GET /api/status startedAt is recent"):
    ...
    try
      ...
      assert(
        !startedAt.isBefore(beforeStart),
        s"startedAt $startedAt should not be before $beforeStart"
      )
      assert(
        !startedAt.isAfter(afterStart),
        s"startedAt $startedAt should not be after $afterStart"
      )
  ...
  test("DELETE /api/v1/worktrees/:issueId returns 200 and removes worktree"):
    ...
    try
      ...
      val stateJson = ujson.read(stateContent)
      assert(
        !stateJson("worktrees").obj.contains("IWLE-123"),
  ...
  test(
    "DELETE /api/v1/worktrees/:issueId returns 404 for non-existent worktree"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
    ...
    try
      ...
      assert(
        responseJson("message").str.contains("not found") || responseJson(
          "message"
        ).str.contains("IWLE-999")
      )
  ...
  test("status endpoint includes hosts field with single host"):
    ...
    try
      ...
      assert(
        statusJson.obj.contains("hosts"),
        "Status response should contain hosts field"
      )
  ...
  test("GET / with sshHost query parameter includes value in HTML"):
    ...
    try
      ...
      assert(
        html.contains("my-remote-server"),
        "HTML should contain SSH host value"
      )
      assert(
        html.contains("ssh-host-input"),
        "HTML should contain SSH host input field"
      )
  ...
  test("DELETE endpoint removes associated cache entries"):
    ...
    try
      ...
      Files.writeString(
        Paths.get(statePath),
        ujson.write(stateJson, indent = 2)
      )
      ...
      assert(
        !updatedStateJson.obj
          .get("issueCache")
          .exists(_.obj.contains("IWLE-123")),
  ...
  test("CaskServer constructor accepts devMode parameter"):
    val statePath = createTempStatePath()
    val port =
      9999 // Use fixed port for unit test (not actually starting server)
    val hosts = Seq("localhost")
    val startedAt = java.time.Instant.now()
  ...
  test(
    "GET /api/projects/:projectName/worktrees/changes returns 200 with HTML"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
  ...
  test(
    "GET /api/projects/:projectName/worktrees/changes filters by project name"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
    ...
    try
      ...
      assert(
        body.contains("IW-100"),
        "Response should contain IW-100 (iw-cli project)"
      )
      // Should NOT include the kanon worktree card
      assert(
        !body.contains("IWLE-200"),
        "Response should not contain IWLE-200 (kanon project)"
      )
  ...
  test(
    "GET /api/projects/:projectName/worktrees/changes with have param detects no changes"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
    ...
    try
      ...
      val response = quickRequest
        .get(
          uri"http://localhost:$port/api/projects/iw-cli/worktrees/changes?have=IW-300"
        )
        .send()
      ...
      assert(
        ...
        "No OOB swaps expected when client is up to date"
      )
  ...
  test("PUT /api/v1/projects/:projectName registers new project → 201"):
    ...
    val port = findAvailablePort()
    val tmpDir =
      Files.createTempDirectory("iw-project-test").toAbsolutePath.toString
  ...
  test(
    "PUT /api/v1/projects/:projectName with missing required field → 400 MISSING_FIELD"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
  ...
  test(
    "PUT /api/v1/projects/:projectName with malformed JSON → 400 MALFORMED_JSON"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
  ...
  test(
    "PUT /api/v1/projects/:projectName with empty path → 400 VALIDATION_ERROR"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
  ...
  test(
    "GET /projects/:projectName for registered project with no worktrees → 200"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
    val tmpDir =
    ...
    try
      ...
      quickRequest
        .put(
          uri"http://localhost:$port/api/v1/projects/my-zero-worktree-project"
        )
        .body(ujson.write(requestBody))
        .header("Content-Type", "application/json")
        .send()
  ...
  test("dashboard auto-prunes projects whose path does not exist on disk"):
    ...
    val port = findAvailablePort()
    val tmpDir =
      Files.createTempDirectory("iw-project-prune-test").toAbsolutePath.toString
    ...
    try
      ...
      val stateAfterRegister =
        ujson.read(Files.readString(Paths.get(statePath)))
      assert(
        stateAfterRegister("projects").obj.contains(tmpDir),
      ...
      val stateAfterDashboard =
        ujson.read(Files.readString(Paths.get(statePath)))
      val projectsContainsTmpDir = stateAfterDashboard.obj
        .get("projects")
  ...
  test("GET /worktrees/:issueId returns 200 with HTML for known worktree"):
    ...
    try
      ...
      val html = response.body
      assert(
        html.contains("IW-188"),
        "Response body should contain the issue ID"
      )
      assert(
        html.contains("worktree-detail"),
        "Response should render the detail view container"
      )
      // Without cached issue data, skeleton state is rendered
      assert(
        html.contains("Loading") || html.contains("skeleton"),
  ...
  test("GET /worktrees/NONEXISTENT returns 404 with error page"):
    ...
    try
      ...
      val html = response.body
      assert(
        html.contains("NONEXISTENT-999"),
        "Response body should contain the requested issue ID"
      )
      assert(
        html.contains("not registered") || html.contains("Not Found"),
        "Response body should contain a not-found message"
      )
      assert(
        html.contains("worktree-detail"),
        "Response should render the styled 404 view, not a raw error"
      )
      assert(
        html.contains("breadcrumb"),
        "404 response should contain breadcrumb navigation"
      )
      assert(
        html.contains("Projects"),
        "404 breadcrumb should contain Projects link"
      )
  ...
  test(
    "GET /worktrees/%3Cscript%3E (URL-encoded) returns 404 with HTML-escaped content"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
    ...
    try
      ...
      val response = quickRequest
        .get(
          uri"http://localhost:$port/worktrees/%3Cscript%3Ealert(1)%3C%2Fscript%3E"
        )
        .send()
      ...
      assert(
        html.contains("&lt;script&gt;") || html.contains("%3Cscript%3E"),
      ...
      assert(
        html.contains("worktree-detail"),
        "Response should render the styled 404 view"
      )
  ...
  test(
    "GET /worktrees/:issueId/artifacts returns page with back link to worktree detail"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
    val tmpWorktreeDir =
      Files.createTempDirectory("iw-artifact-test").toAbsolutePath.toString
    ...
    try
      ...
      val response = quickRequest
        .get(
          uri"http://localhost:$port/worktrees/IW-188/artifacts?path=test-artifact.md"
        )
        .send()
      ...
      val html = response.body
      assert(
        html.contains("Test Artifact"),
        "Artifact content should be rendered in the page"
      )
      assert(
        html.contains("Some content here"),
        "Artifact body content should appear in response"
      )
      assert(
        html.contains("href=\"/worktrees/IW-188\""),
        "Artifact page back link should point to worktree detail page"
      )
      assert(
        html.contains("Back to Worktree"),
        "Back link text should say 'Back to Worktree'"
      )
  ...
  test(
    ...
    try
      ...
      assert(
        !html.contains("<html"),
        "Fragment response should not contain <html tag"
      )
      assert(
        !html.contains("<head"),
        "Fragment response should not contain <head tag"
      )
  ...
  test(
    ...
    try
      ...
      assert(
        !html.contains("breadcrumb"),
        "Fragment response should not contain breadcrumb navigation"
      )
  ...
  test(
    ...
    try
      ...
      val response = quickRequest
        .get(
          uri"http://localhost:$port/worktrees/NONEXISTENT-999/detail-content"
        )
        .send()
  ...
  test(
    "GET /worktrees/:issueId/card response contains detail page link and tracker section"
  ):
    val statePath = createTempStatePath()
    val port = findAvailablePort()
