...
class WorktreeUnregistrationServiceTest extends FunSuite:
  ...
  test("unregister returns Right with updated state when worktree exists"):
    ...
    result.foreach { newState =>
      assert(
        !newState.worktrees.contains("IWLE-123"),
  ...
  test("unregister returns Left with error when worktree not found"):
    ...
    result.swap.foreach { error =>
      assert(
        error.contains("not found") || error.contains("IWLE-999"),
        s"Error message should mention issue: $error"
      )
    }
  ...
  test("unregister removes worktree from state.worktrees"):
    ...
    val state =
      ServerState(Map("IWLE-100" -> worktree1, "IWLE-200" -> worktree2))
  ...
  test("unregister removes associated issue cache entry"):
    ...
    result.foreach { newState =>
      assert(
        !newState.issueCache.contains("IWLE-123"),
        "Issue cache should be removed"
      )
    }
  ...
  test("unregister removes associated progress cache entry"):
    ...
    result.foreach { newState =>
      assert(
        !newState.progressCache.contains("IWLE-123"),
        "Progress cache should be removed"
      )
    }
  ...
  test("unregister removes associated PR cache entry"):
    ...
    result.foreach { newState =>
      assert(
        !newState.prCache.contains("IWLE-123"),
        "PR cache should be removed"
      )
    }
  ...
  test("pruneNonExistent removes worktrees with missing paths"):
    ...
    val newState =
      WorktreeUnregistrationService.pruneNonExistent(state, pathExists)
    ...
    assert(
      !newState.worktrees.contains("IWLE-200"),
      "Missing path should be removed"
    )
  ...
  test("pruneNonExistent removes associated caches for pruned worktrees"):
    ...
    val progressCache = Map(
      "IWLE-100" -> CachedProgress(
        WorkflowProgress(None, 0, List.empty, 0, 0),
        Map.empty
      ),
      "IWLE-200" -> CachedProgress(
        WorkflowProgress(None, 0, List.empty, 0, 0),
        Map.empty
      )
    )
    val prCache = Map(
      "IWLE-100" -> CachedPR(
        PullRequestData("http://example.com/pr/1", PRState.Open, 1, "PR 1"),
        now
      ),
      "IWLE-200" -> CachedPR(
        PullRequestData("http://example.com/pr/2", PRState.Open, 2, "PR 2"),
        now
      )
    )
    val state = ServerState(
