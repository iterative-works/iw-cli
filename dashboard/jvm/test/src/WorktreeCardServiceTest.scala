...
class WorktreeCardServiceTest extends munit.FunSuite:
  test("renderCard returns HTML fragment with card content") {
    ...
    val result = WorktreeCardService.renderCard(
      ...
      "test-server",
      (id: String) =>
        Right(Issue(id, "Test Issue From API", "Open", None, None)),
      (id: String, tracker: String, config: Option[String]) =>
        s"https://example.com/issue/$id"
    )
  ...
  test("renderCard returns fetchedReviewState when review-state.json exists"):
    ...
    try
      ...
      assert(
        result.fetchedReviewState.isDefined,
        "fetchedReviewState should be Some when review-state.json exists"
      )
  ...
  test(
    "renderCard returns None for fetchedReviewState when no review-state.json"
  ):
    ...
    try
      ...
      assert(
        result.fetchedReviewState.isEmpty,
        "fetchedReviewState should be None when review-state.json doesn't exist"
      )
    finally
      // Cleanup
      Files.deleteIfExists(issueDir)
  ...
  test("renderCard uses cached review state when mtime unchanged"):
    ...
    try
      ...
      val reviewStateCache = Map(
        issueId -> CachedReviewState(
          cachedState,
          Map(reviewStateFile.toString -> fileMtime)
        )
      )
      ...
      assert(
        result.fetchedReviewState.isDefined,
        "fetchedReviewState should be Some"
      )
  ...
  test("renderCard returns fetchedProgress when phase task files exist"):
    ...
    try
      ...
      assert(
        result.fetchedProgress.isDefined,
        "fetchedProgress should be Some when phase files exist"
      )
  ...
  test("renderCard returns None for fetchedProgress when no phase files"):
    ...
    try
      ...
      assert(
        result.fetchedProgress.isEmpty,
        "fetchedProgress should be None when no phase files exist"
      )
    finally
      // Cleanup
      Files.deleteIfExists(issueDir)
  ...
  test("renderCard uses cached progress when mtime unchanged"):
    ...
    try
      ...
      val progressCache = Map(
        issueId -> CachedProgress(
          cachedProgress,
          Map(phaseFile.toString -> fileMtime)
        )
      )
      ...
      assertEquals(
        returned.progress.totalPhases,
        99,
        "Should return cached progress when mtime unchanged"
      )
  ...
  test("renderCard returns fetchedPR when PR cache has data"):
    ...
    assert(
      result.fetchedPR.isDefined,
      "fetchedPR should be Some when PR cache has data"
    )
  ...
  test("renderCard returns None for fetchedPR when no PR cached"):
    ...
    assert(
      result.fetchedPR.isEmpty,
      "fetchedPR should be None when PR cache is empty and no fetchPR provided"
    )
  ...
  test("renderCard fetches fresh PR when not throttled and fetchPR provided"):
    ...
    val fetchPR =
      () => Right(Some(freshPR)): Either[String, Option[PullRequestData]]
    ...
    assert(
      result.fetchedPR.isDefined,
      "fetchedPR should be Some when fetchPR returns data"
    )
    val returned = result.fetchedPR.get
    ...
    assert(
      result.html.contains("pr-link"),
      "HTML should contain PR link section"
    )
    assert(
      result.html.contains("https://github.com/org/repo/pull/42"),
      "HTML should contain PR URL"
    )
  ...
  test("renderCard falls back to cached PR when fetchPR fails"):
    ...
    val fetchPR =
      () => Left("gh command failed"): Either[String, Option[PullRequestData]]
    ...
    assert(
      result.fetchedPR.isDefined,
      "fetchedPR should fall back to cached data on fetch failure"
    )
  ...
  test("renderCard does not call fetchPR when throttled"):
    ...
    val fetchPR = () => {
      ...
      Right(
        Some(PullRequestData("url", PRState.Open, 1, "Should not be called"))
