...
import iw.dashboard.presentation.views.CreationSuccessView
...
class CreationSuccessViewTest extends FunSuite:
  ...
  test("render includes tmux attach command"):
    ...
    assert(
      html.contains("tmux attach -t iw-cli-IW-79"),
  ...
  test("copy button has onclick handler with clipboard.writeText"):
    ...
    assert(
      ...
      "Should have clipboard API call"
    )
    assert(
      html.contains("tmux attach -t iw-cli-IW-79"),
      "Should include command in clipboard call"
    )
  ...
  test("render includes worktree path"):
    ...
    assert(
      html.contains("/home/user/projects/iw-cli-IW-79"),
  ...
  test("render has success icon"):
    ...
    assert(
      html.contains("success") || html.contains("✓") || html.contains(
        "&#x2714;"
      ),
  ...
  test("render has CSS class for success state"):
    ...
    assert(
      html.contains("creation-success"),
      "Should have creation-success class"
    )
