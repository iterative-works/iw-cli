...
import iw.dashboard.presentation.views.CreationErrorView
...
class CreationErrorViewTest extends FunSuite:
  ...
  test("render includes error message"):
    ...
    assert(
      html.contains("Failed to create git worktree"),
  ...
  test("render shows suggestion when present"):
    ...
    assert(
      html.contains("Remove the directory or register"),
  ...
  test("render does not show suggestion section when not present"):
    ...
    assert(
      !html.contains("Remove the directory"),
      "Should not show suggestion text from other errors"
    )
  ...
  test("render includes retry button when canRetry is true"):
    ...
    assert(
      html.contains("Retry") || html.contains("Try Again"),
  ...
  test("retry button has hx-post to /api/worktrees/create"):
    ...
    assert(
      html.contains("/api/worktrees/create"),
      "Should post to worktrees/create endpoint"
    )
  ...
  test("retry button targets modal body"):
    ...
    assert(
      html.contains("#modal-body-content") || html.contains("modal"),
      "Should target modal"
    )
  ...
  test("render includes dismiss button"):
    ...
    assert(
      html.contains("Dismiss") || html.contains("Close"),
  ...
  test("dismiss button closes modal"):
    ...
    assert(
      ...
      "Should use hx-get to close modal"
    )
  ...
  test("render has CSS class for error state"):
    ...
    assert(
      html.contains("creation-error") || html.contains("error"),
  ...
  test("render has error icon"):
    ...
    assert(
      html.contains("error-icon") || html.contains("✗") || html.contains(
        "&#x2717;"
      ) ||
  ...
  test("retry button includes issueId in hx-vals when provided"):
    ...
    assert(
      ...
      "Should have hx-vals attribute for retry data"
    )
    assert(html.contains("issueId"), "hx-vals should include issueId")
    assert(
      html.contains("IW-79"),
      "hx-vals should include the actual issue ID value"
    )
  ...
  test("retry button without issueId does not have hx-vals"):
    ...
    assert(
      !html.contains("hx-vals"),
      "Should not have hx-vals when issueId is None"
    )

  test(
    "retry button has hx-target-4xx and hx-target-5xx for error response handling"
  ):
    val html = CreationErrorView.render(errorWithRetry).render

    assert(
      html.contains("hx-target-4xx"),
      "Should have hx-target-4xx attribute"
    )
    assert(
      html.contains("hx-target-5xx"),
      "Should have hx-target-5xx attribute"
    )
