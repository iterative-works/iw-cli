...
class StateRepositoryTest extends munit.FunSuite:
  ...
  tempDir.test(
    "StateRepository.read with non-existent file returns empty state"
  ): tempDir =>
  ...
  tempDir.test("StateRepository.write then read returns same state"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("StateRepository.write uses atomic writes (tmp file + rename)"):
    tempDir =>
      ...
      assert(
        ...
        "Temporary file should be deleted after atomic write"
      )
  ...
  tempDir.test(
    "StateRepository deserializes ServerState with issueCache correctly"
  ): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("StateRepository handles empty issueCache"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("StateRepository handles multiple cached issues"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)

    val issue1 = IssueData(
      "IWLE-1",
      "Issue 1",
      "Open",
      None,
  ...
  tempDir.test("StateRepository serializes ServerState with progressCache"):
    tempDir =>
      ...
      val cached =
        CachedProgress(progress, Map("/path/phase-01-tasks.md" -> 1000L))
  ...
  tempDir.test(
    "StateRepository deserializes ServerState with progressCache correctly"
  ): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("StateRepository handles empty progressCache"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test(
    "StateRepository deserializes ServerState with prCache correctly"
  ): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
    ...
    readResult.foreach { loadedState =>
      ...
      assertEquals(
        loadedCached.fetchedAt,
        Instant.parse("2025-12-20T11:30:00Z")
      )
    }

  tempDir.test(
    "StateRepository prCache persists PullRequestData url, state, number"
  ): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("Old state.json without prCache loads successfully"): tempDir =>
    val statePath = tempDir.resolve("state.json")
  ...
  tempDir.test("StateRepository serializes ReviewState"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
    ...
    val reviewState = ReviewState(
      ...
      artifacts = List(
        ...
        ReviewArtifact(
          "Context",
          "project-management/issues/46/phase-08-context.md"
        )
      )
    )
  ...
  tempDir.test("StateRepository serializes CachedReviewState"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("StateRepository deserializes reviewStateCache"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("StateRepository concurrent writes don't corrupt state file"):
    tempDir =>
      ...
      assertEquals(
        tempFiles.length,
        0,
        "No temp files should remain after writes"
      )
