...
class IssueSearchServiceTest extends FunSuite:
  ...
  test("search with valid Linear issue ID returns result"):
    ...
    val results =
      IssueSearchService.search("IWLE-100", config, fetchIssue, searchIssues)
  ...
  test("search with invalid issue ID returns empty list"):
    ...
    val fetchIssue =
      (id: IssueId) => fail("fetchIssue should not be called for invalid ID")

    // Mock searchIssues - returns empty list (no matches)
    val searchIssues = (query: String) => Right(List.empty)

    val results =
      IssueSearchService.search("INVALID", config, fetchIssue, searchIssues)
  ...
  test("search with valid ID but fetch failure returns empty list"):
    ...
    val fetchIssue = (id: IssueId) => Left("Issue not found")

    // Mock searchIssues - returns empty list (no matches in text search either)
    val searchIssues = (query: String) => Right(List.empty)

    val results =
      IssueSearchService.search("IWLE-999", config, fetchIssue, searchIssues)
  ...
  test("search with GitHub issue ID returns result"):
    ...
    val fetchIssue =
      (id: IssueId) => Right(Issue("IW-79", "GitHub Issue", "Open", None, None))

    val searchIssues = (query: String) =>
      fail("searchIssues should not be called for exact ID match")

    val results =
      IssueSearchService.search("IW-79", config, fetchIssue, searchIssues)
  ...
  test("search with YouTrack issue ID returns result"):
    ...
    val results =
      IssueSearchService.search("PROJ-123", config, fetchIssue, searchIssues)
    ...
    results.foreach { list =>
      ...
      assert(
        result.url.contains("youtrack.example.com"),
        "URL should be YouTrack URL"
      )
    }
  ...
  test("search with empty query returns empty list"):
    ...
    val fetchIssue =
      (id: IssueId) => fail("fetchIssue should not be called for empty query")

    val searchIssues = (query: String) =>
      fail("searchIssues should not be called for empty query")

    val results =
      IssueSearchService.search("", config, fetchIssue, searchIssues)
  ...
  test("search with GitLab issue ID returns result with correct URL format"):
    ...
    val results =
      IssueSearchService.search("123", config, fetchIssue, searchIssues)
    ...
    results.foreach { list =>
      ...
      assert(
        result.url.contains("/-/issues/"),
        "URL should have GitLab path format"
      )
      assert(
        result.url.contains("my-org/my-project"),
  ...
  test("search with GitLab issue ID uses self-hosted baseUrl"):
    ...
    // GitLab requires numeric issue IDs
    val results =
      IssueSearchService.search("456", config, fetchIssue, searchIssues)

    assert(results.isRight, "Search should succeed")
    results.foreach { list =>
      val result = list.head
      assert(
        result.url.contains("gitlab.company.com"),
        "URL should use self-hosted base URL"
      )
      assert(
        result.url.contains("/-/issues/456"),
        "URL should have correct issue number"
      )
    }
  ...
  test("fetchRecent success case with GitHub tracker"):
    ...
    results.foreach { list =>
      ...
      assertEquals(
        list(0).hasWorktree,
        false,
        "Should not have worktree by default"
      )
    }
  ...
  test("fetchRecent with worktree check integration"):
    ...
    val results = IssueSearchService.fetchRecent(
      config,
      fetchRecentIssues,
      checkWorktreeExists
    )
    ...
    results.foreach { list =>
      ...
      assertEquals(
        list(1).hasWorktree,
        false,
        "Issue #131 should not have worktree"
      )
    }
  ...
  test("fetchRecent handles fetch errors gracefully"):
    ...
    val fetchRecentIssues =
      (limit: Int) => Left("Failed to fetch recent issues: API error")
  ...
  test("fetchRecent returns empty list when no issues"):
    ...
    val fetchRecentIssues = (limit: Int) => Right(List.empty)
  ...
  test(
    "search() exact ID match returns that issue (priority over text search)"
  ):
    val config = ProjectConfiguration.create(
      trackerType = IssueTrackerType.GitHub,
      team = "IW",
  ...
  test("search() invalid ID format triggers text search"):
    ...
    val results = IssueSearchService.search(
      "authentication",
      config,
      fetchIssue,
      searchIssues
    ...
    assert(results.isRight, "Search should succeed")
    assertEquals(
      results.map(_.length),
      Right(2),
      "Should return two results from text search"
    )
    results.foreach { list =>
      assertEquals(list(0).id, "#132")
  ...
  test("search() valid ID but not found triggers text search"):
    ...
    val results =
      IssueSearchService.search("IW-999", config, fetchIssue, searchIssues)

    assert(results.isRight, "Search should succeed with text search fallback")
    assertEquals(
      results.map(_.length),
      Right(1),
      "Should return one result from text search"
    )
    results.foreach { list =>
  ...
  test("search() text search returns matching issues"):
    ...
    val fetchIssue =
      (id: IssueId) => fail("fetchIssue should not be called for non-ID query")
    ...
    val results =
      IssueSearchService.search("fix bug", config, fetchIssue, searchIssues)
  ...
  test("search() text search error handling"):
    ...
    val searchIssues =
      (query: String) => Left("Failed to search issues: API error")

    val results =
      IssueSearchService.search("test query", config, fetchIssue, searchIssues)
  ...
  test("search() text search sorts closed issues to end"):
    ...
    val results =
      IssueSearchService.search("test", config, fetchIssue, searchIssues)
    ...
    results.foreach { list =>
      ...
      assertEquals(
        list(4).status,
        "Canceled",
        "Fifth should be Canceled (closed)"
      )
