...
class WorkflowProgressServiceTest extends FunSuite:
  ...
  test(
    "determineCurrentPhase returns first not-started phase if no in-progress"
  ):
    val phases = List(
      PhaseInfo(1, "Phase 1", "/p1", 10, 10), // Complete
  ...
  test("fetchProgress uses cache when mtimes match"):
    val phase1 = PhaseInfo(
      1,
      "Phase 1",
      "/worktree/project-management/issues/ISSUE-123/phase-01-tasks.md",
      10,
      5
    )
    val cachedProgress = WorkflowProgress(Some(1), 1, List(phase1), 5, 10)
    val filesMtime = Map(
      "/worktree/project-management/issues/ISSUE-123/phase-01-tasks.md" -> 1000L
    )
    ...
    assert(
      !readFileCalled.get,
      "readFile should not be called when cache is valid"
    )
  ...
  test("fetchProgress parses multiple phase files correctly"):
    ...
    val getMtime = (path: String) =>
      if path.contains("phase-01") || path.contains("phase-02") then
        Right(1000L)
      else Left("File not found")
  ...
  test(
    "computeProgress uses Phase Index size when Phase Index is non-empty (6-entry index, 3 files)"
  ):
    val phases = List(
      PhaseInfo(1, "Phase 1", "/p1", 10, 10),
      PhaseInfo(2, "Phase 2", "/p2", 15, 8),
    ...
    assertEquals(
      ...
      6
    ) // Should use phaseIndex.size, not phases.size

  test(
    "computeProgress uses Phase Index size when it matches file count (4-entry index, 4 files)"
  ):
    val phases = List(
      PhaseInfo(1, "Phase 1", "/p1", 10, 10),
      PhaseInfo(2, "Phase 2", "/p2", 15, 15),
  ...
  test(
    "computeProgress falls back to file count when Phase Index is empty (empty index, 3 files)"
  ):
    val phases = List(
      PhaseInfo(1, "Phase 1", "/p1", 10, 10),
      PhaseInfo(2, "Phase 2", "/p2", 15, 8),
