...
import iw.dashboard.ArtifactService
...
class ArtifactServiceTest extends munit.FunSuite:
  ...
  test(
    "loadArtifact returns Left when PathValidator rejects path (absolute path)"
  ):
    val readFile = (path: Path) => Right("content")
  ...
  test(
    "loadArtifact returns Left when PathValidator rejects path (directory traversal)"
  ):
    val readFile = (path: Path) => Right("content")
  ...
  test("extractLabel returns filename from path"):
    ...
    val label2 = ArtifactService.extractLabel(
      Paths.get("project-management/issues/46/analysis.md")
    )
    assertEquals(label2, "analysis.md")

    val label3 =
      ArtifactService.extractLabel(Paths.get("/absolute/path/to/file.txt"))
