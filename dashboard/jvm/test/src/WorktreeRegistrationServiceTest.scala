...
import iw.dashboard.WorktreeRegistrationService
...
class WorktreeRegistrationServiceTest extends FunSuite:
  ...
  test("updateLastSeen updates timestamp for existing worktree"):
    ...
    val result =
      WorktreeRegistrationService.updateLastSeen("IWLE-123", later, state)
  ...
  test("updateLastSeen returns Left for non-existent worktree"):
    ...
    val result =
      WorktreeRegistrationService.updateLastSeen("IWLE-999", timestamp, state)
