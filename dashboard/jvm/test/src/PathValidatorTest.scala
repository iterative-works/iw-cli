...
class PathValidatorTest extends FunSuite:
  ...
  test("validateArtifactPath accepts valid relative path within worktree"):
    ...
    assert(
      resolvedPath.toString.contains("project-management"),
      "Path should contain project-management"
    )

  test("validateArtifactPath rejects empty path"):
    val result =
      PathValidator.validateArtifactPath(worktreePath, "", noOpResolver)
  ...
  test("validateArtifactPath accepts path with .. that stays within boundary"):
    val result = PathValidator.validateArtifactPath(
      worktreePath,
      "project-management/issues/../tasks.md",
      noOpResolver
    )

    assert(
      result.isRight,
      s"Expected Right but got Left: ${result.swap.toOption}"
    )
    val resolvedPath = result.toOption.get
  ...
  test(
    "validateArtifactPath accepts filename containing .. like file..name.md"
  ):
    val result = PathValidator.validateArtifactPath(
      worktreePath,
      "docs/file..name.md",
    ...
    val resolvedPath = result.toOption.get
    assert(
      resolvedPath.toString.contains("file..name.md"),
  ...
  test("validateArtifactPath rejects symlink pointing outside worktree"):
    ...
    val testDir =
      Paths.get("/tmp/pathvalidator-test-" + System.currentTimeMillis())
    ...
    try
      ...
      java.nio.file.Files
        .writeString(outside.resolve("secret.txt"), "secret data")

      // Create symlink inside worktree pointing outside
      val symlinkPath = worktree.resolve("escape-link")
      java.nio.file.Files
        .createSymbolicLink(symlinkPath, outside.resolve("secret.txt"))
  ...
  test("validateArtifactPath accepts symlink pointing within worktree"):
    ...
    try
      java.nio.file.Files.createDirectories(worktree)
      java.nio.file.Files
        .writeString(worktree.resolve("target.txt"), "valid data")

      // Create symlink inside worktree pointing to another file in worktree
      val symlinkPath = worktree.resolve("valid-link")
      java.nio.file.Files
        .createSymbolicLink(symlinkPath, worktree.resolve("target.txt"))

      val result = PathValidator.validateArtifactPath(worktree, "valid-link")

      assert(
        result.isRight,
        s"Should accept symlink pointing within worktree, got: ${result.swap.toOption}"
      )
    finally
      // Clean up
      scala.util.Try(deleteRecursively(testDir))
  ...
  test("validateArtifactPath handles broken symlinks gracefully"):
    ...
    try
      ...
      java.nio.file.Files
        .createSymbolicLink(symlinkPath, worktree.resolve("nonexistent.txt"))
  ...
  test("validateArtifactPath end-to-end with real worktree path"):
    // Create temp directory structure for testing
    val testDir =
      Paths.get("/tmp/pathvalidator-e2e-test-" + System.currentTimeMillis())
    val worktree = testDir.resolve("worktree")

    try
      java.nio.file.Files
        .createDirectories(worktree.resolve("project-management/issues/46"))
      ...
      assert(
        result.isRight,
        s"Should accept valid path with real I/O, got: ${result.swap.toOption}"
      )
      val resolvedPath = result.toOption.get
      assert(
    ...
    finally scala.util.Try(deleteRecursively(testDir))
  ...
  test("validateArtifactPath handles Unicode path components"):
    ...
    val resolvedPath = result.toOption.get
    assert(
      resolvedPath.toString.contains("příklad"),
  ...
  test("validateArtifactPath handles special characters in filenames"):
    ...
    val resolvedPath = result.toOption.get
    assert(
      resolvedPath.toString.contains("file-name_with+special$chars.md"),
      "Filename should be preserved"
    )
