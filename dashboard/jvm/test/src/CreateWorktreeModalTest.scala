...
import iw.dashboard.presentation.views.CreateWorktreeModal
...
class CreateWorktreeModalTest extends FunSuite:
  ...
  test("render creates modal with correct structure"):
    ...
    assert(
      html.contains("id=\"create-worktree-modal\""),
      "Modal should have correct ID"
    )
    assert(html.contains("class=\"modal\""), "Modal should have modal class")
    assert(
      html.contains("class=\"modal-backdrop\""),
      "Modal should have backdrop"
    )
    assert(
      html.contains("class=\"modal-content\""),
      "Modal should have content div"
    )
  ...
  test("render includes close button with HTMX attributes"):
    ...
    assert(
      html.contains("hx-get") && html.contains("/api/modal/close"),
      "Close button should use hx-get to close modal"
    )
  ...
  test("render includes search input with correct attributes"):
    ...
    assert(
      html.contains("id=\"issue-search-input\""),
      "Should have search input ID"
    )
    assert(html.contains("type=\"text\""), "Should be text input")
    assert(
      html.contains("placeholder=\"Search by issue ID"),
  ...
  test("search input includes HTMX debounce attributes"):
    ...
    assert(
      html.contains("hx-get=\"/api/issues/search\""),
      "Should target search endpoint"
    )
    assert(
    ...
    assert(
      html.contains("hx-target=\"#search-results\""),
      "Should target results div"
    )
  ...
  test("render includes search results container"):
    ...
    assert(
      html.contains("id=\"search-results\""),
      "Should have search results div"
    )
    assert(
      html.contains("class=\"search-results\""),
      "Should have search results class"
    )
  ...
  test("render includes loading indicator element"):
    ...
    assert(
      html.contains("id=\"creation-spinner\""),
      "Should have creation spinner element"
    )
  ...
  test("loading indicator has htmx-indicator class"):
    ...
    assert(
      ...
      "Should have htmx-indicator class for auto show/hide"
    )
  ...
  test("modal body has wrapper div for content swap"):
    ...
    assert(
      html.contains("id=\"modal-body-content\""),
      "Should have modal-body-content div for HTMX target"
    )
  ...
  test("search results container has hx-trigger load for auto-load"):
    ...
    assert(
      ...
      "Should have load trigger to auto-load recent issues"
    )
  ...
  test("search results container has hx-get pointing to recent endpoint"):
    ...
    assert(
      html.contains("hx-get=\"/api/issues/recent\""),
      "Should target recent issues endpoint"
    )
  ...
  test("search results container has hx-swap innerHTML"):
    ...
    assert(
      html.contains("id=\"search-results\"") && html.contains(
        "hx-swap=\"innerHTML\""
      ),
  ...
  test(
    "search results container includes project parameter when projectPath provided"
  ):
    val html = CreateWorktreeModal.render(Some("/path/to/project")).render

    assert(
      html.contains("hx-get=\"/api/issues/recent?project="),
      "Should include project parameter in URL"
    )
    assert(
      html.contains("%2Fpath%2Fto%2Fproject"),
      "Should have URL-encoded project path"
    )
