...
import iw.dashboard.GitStatusService
...
class GitStatusServiceTest extends FunSuite:
  ...
  test("getGitStatus returns error when git command fails"):
    val execCommand =
      (cmd: String, args: Array[String]) => Left("Not a git repository")
  ...
  test("parseBranchName extracts branch from output"):
    ...
    assertEquals(
      GitStatusService.parseBranchName("feature-branch"),
      Some("feature-branch")
    )
  ...
  test("isWorkingTreeClean returns false for multiple changes"):
    assert(
      !GitStatusService.isWorkingTreeClean(
        " M file1.txt\n M file2.txt\n?? file3.txt"
      )
    )
  ...
  test("getGitStatus uses git -C flag with worktree path"):
    val capturedArgs =
      new java.util.concurrent.atomic.AtomicReference[Option[Array[String]]](
        None
      )
    val execCommand = (cmd: String, args: Array[String]) =>
