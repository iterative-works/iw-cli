...
import iw.dashboard.IssueCacheService
...
class IssueCacheServiceTest extends FunSuite:
  ...
  test("fetchWithCache uses valid cache (2 min ago, no API call)"):
    val now = Instant.now()
    val cachedData =
      createTestIssueData("IWLE-123", now.minusSeconds(120)) // 2 min ago
    ...
    assertEquals(
      result.map(_._2).getOrElse(false),
      true,
      "fromCache should be true"
    )
    assert(
      fetchFnCalls.isEmpty,
      "fetchFn should NOT be called when cache is valid"
    )
  ...
  test("fetchWithCache refreshes expired cache (6 min ago, API called)"):
    val now = Instant.now()
    val cachedData =
      createTestIssueData("IWLE-123", now.minusSeconds(360)) // 6 min ago
    val cache = Map("IWLE-123" -> CachedIssue(cachedData, ttlMinutes = 5))

    val freshIssue =
      Issue("IWLE-123", "Fresh title", "In Progress", Some("John"), None)
  ...
  test("fresh data returned when cache expired and API succeeds"):
    val now = Instant.now()
    val oldData =
      createTestIssueData("PROJ-1", now.minusSeconds(600)) // 10 min ago
    val cache = Map("PROJ-1" -> CachedIssue(oldData))

    val freshIssue =
      Issue("PROJ-1", "Updated Issue", "Done", None, Some("New description"))
    val fetchFn = (_: String) => Right(freshIssue)
    val urlBuilder = (id: String) => s"https://example.com/$id"
  ...
  test("stale cache returned when API fails but cache exists"):
    val now = Instant.now()
    val cachedData = createTestIssueData(
      "IWLE-456",
    ...
    assertEquals(
      fromCache,
      true,
      "fromCache should be true when using stale fallback"
    )
  ...
  test("buildIssueUrl generates YouTrack URL correctly with baseUrl"):
    ...
    val url =
      IssueCacheService.buildIssueUrl("PROJ-456", "YouTrack", Some(baseUrl))

    assert(
      url.contains("youtrack.example.com"),
      "Should contain YouTrack base URL"
    )
  ...
  test("buildIssueUrl extracts number from IW-72 format for GitHub"):
    val repository = "iterative-works/iw-cli"
    val url =
      IssueCacheService.buildIssueUrl("IW-72", "GitHub", Some(repository))
  ...
  test("buildIssueUrl handles lowercase tracker types"):
    ...
    val youtrackUrl = IssueCacheService.buildIssueUrl(
      "PROJ-1",
      "youtrack",
      Some("https://yt.example.com")
    )
    assert(youtrackUrl.contains("yt.example.com"))

    val githubUrl =
      IssueCacheService.buildIssueUrl("72", "github", Some("owner/repo"))
  ...
  test("buildIssueUrl generates GitLab URL with default gitlab.com"):
    val url = IssueCacheService.buildIssueUrl(
      "123",
      "GitLab",
  ...
  test("buildIssueUrl generates self-hosted GitLab URL with custom baseUrl"):
    ...
    val url = IssueCacheService.buildIssueUrl(
      "456",
      "GitLab",
      Some("team/app|https://gitlab.company.com")
    )
  ...
  test("buildIssueUrl handles lowercase gitlab tracker type"):
    val url =
      IssueCacheService.buildIssueUrl("42", "gitlab", Some("owner/repo"))
  ...
  test("getCachedOnly returns stale cache without fetching"):
    val now = Instant.now()
    val staleData = createTestIssueData(
      "IWLE-789",
      now.minusSeconds(600)
    ) // 10 minutes ago (very stale)
