...
import iw.dashboard.presentation.views.{MainProjectsView, ProjectSummary}
...
class MainProjectsViewTest extends FunSuite:
  test("render with empty projects list shows empty state"):
    ...
    assert(
      html.contains("./iw register"),
      "Empty state should mention ./iw register"
    )
  ...
  test("render with multiple projects shows all project cards"):
    ...
    val summary1 =
      ProjectSummary(project1, worktreeCount = 0, attentionCount = 0)
    val summary2 =
      ProjectSummary(project2, worktreeCount = 0, attentionCount = 0)
  ...
  test("team renders as link when trackerUrl is provided"):
    ...
    assert(
      html.contains("href=\"https://github.com/iterative-works/iw-cli/issues\"")
    )
  ...
  test("project name links to project details page"):
    ...
    assert(
      ...
      "Should link to /projects/iw-cli"
    )
  ...
  test("project name link wraps the project heading"):
    ...
    assert(
      html.contains("<a ") && html.contains("iw-cli</h3>"),
      "Link should wrap the h3 heading"
    )
  ...
  test("create button still present alongside project link"):
    ...
    assert(
      html.contains("create-worktree-button"),
      "Create button should still be present"
    )
    assert(
      html.contains("/api/modal/create-worktree"),
      "Create button should have modal URL"
    )
