...
class SampleDataGeneratorTest extends FunSuite:
  ...
  test("issueCache contains entries for all 5 worktrees"):
    ...
    assertEquals(
      issueIds,
      Set("IWLE-123", "IWLE-456", "GH-100", "YT-111", "YT-222")
    )
  ...
  test("reviewStateCache contains entries for 4 worktrees"):
    ...
    assert(
      issueIds.contains("IWLE-123"),
      "Should have review state for IWLE-123"
    )
    assert(
      issueIds.contains("IWLE-456"),
      "Should have review state for IWLE-456"
    )
    ...
    assert(
      !issueIds.contains("YT-111"),
      "Should NOT have review state for YT-111"
    )
  ...
  test("generated data includes missing assignee edge case"):
    ...
    assert(
      hasUnassignedIssue,
      "Should have at least one issue without assignee (GH-100)"
    )
    ...
    try
      ...
      repository.read() match
        case Right(readState) =>
          ...
          assertEquals(
            readState.issueCache.size,
            5,
            "Should have 5 cached issues"
          )
          assertEquals(
            readState.progressCache.size,
            4,
            "Should have 4 cached progress entries"
          )
          assertEquals(readState.prCache.size, 4, "Should have 4 cached PRs")
          assertEquals(
            readState.reviewStateCache.size,
            4,
            "Should have 4 cached review states"
          )
