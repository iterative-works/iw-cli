...
class WorktreeListSyncTest extends munit.FunSuite:
  ...
  test("detectChanges handles mixed changes"):
    ...
    val newIds = List(
      ...
      "IW-1",
      "IW-3"
    ) // IW-2 deleted, IW-4 added, order same for remaining
  ...
  test("generateAdditionOob includes hx-swap-oob attribute"):
    ...
    assert(
      html.contains("hx-swap-oob=\"afterbegin:#worktree-list\""),
      "should use afterbegin when no predecessor"
    )
  ...
  test("generateDeletionOob includes hx-swap-oob delete attribute"):
    ...
    assert(
      html.contains("hx-swap-oob=\"delete\""),
      "should contain delete swap"
    )
    assert(html.contains("id=\"card-IW-123\""), "should contain card ID")
  ...
  test("generateReorderOob generates delete followed by add"):
    ...
    assert(
      html.contains("hx-swap-oob=\"afterbegin:#worktree-list\""),
      "should contain afterbegin swap"
    )
    assert(html.contains("id=\"card-IW-123\""), "should contain card ID")
  ...
  test("generateChangesResponse combines all OOB swaps"):
    val now = Instant.now()
    val registration1 =
      WorktreeRegistration("IW-1", "/path/1", "github", "team", now, now)
    val registration2 =
      WorktreeRegistration("IW-2", "/path/2", "github", "team", now, now)
    ...
    assert(
      html.contains("hx-swap-oob=\"afterbegin:#worktree-list\""),
      "IW-1 should insert at beginning (before IW-2)"
    )
  ...
  test("findPredecessor returns None when new ID should be first"):
    val result =
      WorktreeListSync.findPredecessor("IW-050", List("IW-100", "IW-200"))
    assertEquals(result, None)

  test("findPredecessor returns correct predecessor for middle insertion"):
    val result = WorktreeListSync.findPredecessor(
      "IW-150",
      List("IW-100", "IW-200", "IW-300")
    )
  ...
  test("generateAdditionOob with predecessorId = Some uses afterend"):
    ...
    assert(
      ...
      "should use afterend with predecessor ID"
    )
    assert(html.contains("id=\"card-IW-123\""), "should contain card ID")
  ...
  test(
    "generateChangesResponse: addition inserts at beginning when ID is smallest"
  ):
    val now = Instant.now()
    val registration1 =
      WorktreeRegistration("IW-050", "/path/1", "github", "team", now, now)
    val registration2 =
      WorktreeRegistration("IW-100", "/path/2", "github", "team", now, now)
    val registration3 =
      WorktreeRegistration("IW-200", "/path/3", "github", "team", now, now)
    val issueData1 = IssueData(
      "IW-050",
      "Issue 50",
      "Open",
    ...
    assert(
      html.contains("hx-swap-oob=\"afterbegin:#worktree-list\""),
      "should insert at beginning when ID is smallest"
    )

  test(
    "generateChangesResponse: addition inserts in middle at correct position"
  ):
    val now = Instant.now()
    val registration1 =
      WorktreeRegistration("IW-100", "/path/1", "github", "team", now, now)
    val registration2 =
      WorktreeRegistration("IW-150", "/path/2", "github", "team", now, now)
    val registration3 =
      WorktreeRegistration("IW-200", "/path/3", "github", "team", now, now)
    val registration4 =
      WorktreeRegistration("IW-300", "/path/4", "github", "team", now, now)
    val issueData2 = IssueData(
      "IW-150",
      "Issue 150",
      "Open",
  ...
  test("generateChangesResponse: addition inserts at end when ID is largest"):
    ...
    val registration1 =
      WorktreeRegistration("IW-100", "/path/1", "github", "team", now, now)
    val registration2 =
      WorktreeRegistration("IW-200", "/path/2", "github", "team", now, now)
    val registration3 =
      WorktreeRegistration("IW-300", "/path/3", "github", "team", now, now)
    ...
    assert(
      html.contains("hx-swap-oob=\"afterend:#card-IW-200\""),
      "should insert after IW-200 (last existing card)"
    )

  test(
    "generateChangesResponse: multiple additions each get correct predecessor"
  ):
    val now = Instant.now()
    val registration1 =
      WorktreeRegistration("IW-050", "/path/1", "github", "team", now, now)
    ...
    assert(
      html.contains("hx-swap-oob=\"afterbegin:#worktree-list\""),
      "IW-050 should insert at beginning"
    )
    assert(
      html.contains("hx-swap-oob=\"afterend:#card-IW-100\""),
      "IW-200 should insert after IW-100"
    )
  ...
  test("generateChangesResponse: mixed additions and deletions"):
    ...
    val registration2 =
      WorktreeRegistration("IW-150", "/path/2", "github", "team", now, now)
    val registration3 =
      WorktreeRegistration("IW-175", "/path/3", "github", "team", now, now)
    val issueData3 = IssueData(
      "IW-175",
      "Issue 175",
      "Open",
