...
class ReviewStateServiceTest extends munit.FunSuite:
  ...
  test("parseReviewStateJson handles multiple artifacts"):
    ...
    assertEquals(
      state.artifacts.map(_.label),
      List("Analysis", "Context", "Tasks")
    )
  ...
  test("parseReviewStateJson parses badges with schema 'type' field"):
    ...
    assertEquals(
      state.badges,
      Some(List(Badge("Simple", "info"), Badge("Urgent", "warning")))
    )
  ...
  test("parseReviewStateJson parses display with schema 'type' field"):
    ...
    assertEquals(
      state.display,
      Some(Display("Phase 01", Some("Implementing core logic"), "info"))
    )
  ...
  test("fetchReviewState returns cached state when mtime unchanged"):
    ...
    val readFile =
      (path: String) => fail("File should not be read when cache is valid")
  ...
  test(
    "fetchReviewState cache hit returns CachedReviewState without file read"
  ):
    val issueId = "IWLE-123"
    val worktreePath = "/path/to/worktree"
    val reviewStatePath =
    ...
    val cachedReviewState =
      CachedReviewState(cachedState, Map(reviewStatePath -> 1000L))
  ...
  test(
    "fetchReviewState cache miss returns new CachedReviewState with updated mtime"
  ):
    val issueId = "IWLE-123"
    val worktreePath = "/path/to/worktree"
    val reviewStatePath =
  ...
  test(
    "parseReviewStateJson parses activity and workflow_type when both present"
  ):
    val json = """{
      "artifacts": [{"label": "Test", "path": "test.md"}],
      "activity": "working",
  ...
  test(
    "parseReviewStateJson returns activity=None and workflowType=None when neither field present"
  ):
    val json = """{
      "artifacts": [{"label": "Test", "path": "test.md"}]
    }"""
  ...
  test("writeReviewStateJson includes activity field in output"):
    ...
    // Verify JSON shape contains the expected key
    val json = ReviewStateService.writeReviewStateJson(state)
    assert(
      json.contains("\"activity\""),
      s"JSON should contain 'activity' key: $json"
    )
  ...
  test("writeReviewStateJson includes workflow_type field in output"):
    val state = ReviewState(
      display = None,
      badges = None,
      taskLists = None,
    ...
    val json = ReviewStateService.writeReviewStateJson(state)
    assert(
      json.contains("\"workflow_type\""),
      s"JSON should contain 'workflow_type' key: $json"
    )
  ...
  test("writeReviewStateJson omits activity and workflow_type when None"):
    val state = ReviewState(
      display = None,
      badges = None,
      taskLists = None,
    ...
    assert(
      !json.contains("\"activity\""),
      s"JSON should not contain 'activity' key: $json"
    )
    assert(
      !json.contains("\"workflow_type\""),
      s"JSON should not contain 'workflow_type' key: $json"
    )
