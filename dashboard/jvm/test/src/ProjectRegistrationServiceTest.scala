...
class ProjectRegistrationServiceTest extends FunSuite:
  ...
  test(
    "register creates new ProjectRegistration with provided fields and wasCreated=true"
  ):
  ...
  test("register existing project updates fields and returns wasCreated=false"):
    ...
    val state = ServerState(
      Map.empty,
      projects = Map("/home/user/projects/my-project" -> existing)
    )
  ...
  test("deregister removes project from state"):
    ...
    val newState = ProjectRegistrationService.deregister(
  ...
  test("deregister is idempotent for non-existent path"):
    ...
    val newState = ProjectRegistrationService.deregister(
      "/home/user/projects/non-existent",
      state
    )
