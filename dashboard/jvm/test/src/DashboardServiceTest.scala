...
class DashboardServiceTest extends FunSuite:
  ...
  test("renderDashboard includes review state when present in cache"):
    ...
    val reviewStateCache = Map(
      "IWLE-123" -> CachedReviewState(
        reviewState,
        Map(
          "/path/to/worktree/project-management/issues/IWLE-123/review-state.json" -> 1000L
        )
      )
    )
  ...
  test(
    "renderDashboard with multiple worktrees and mixed review state availability"
  ):
    ...
    val reviewStateCache = Map(
      "IWLE-200" -> CachedReviewState(
        reviewState,
        Map(
          "/path2/project-management/issues/IWLE-200/review-state.json" -> 1000L
        )
      )
    )
    ...
    assert(html.contains("<!DOCTYPE html>"))
    assert(
      html.contains("main-projects-section") || html.contains(
  ...
  test(
    "fetchReviewStateForWorktree with missing state file doesn't crash dashboard"
  ):
  ...
  test("Dashboard HTML links to external CSS file for styles"):
    ...
    assert(
      html.contains("href=\"/static/dashboard.css\""),
  ...
  test("Dashboard HTML links to external JS file for visibilitychange handler"):
    ...
    assert(
      html.contains("src=\"/static/dashboard.js\""),
      "Should link to external JS file"
    )
  ...
  test("renderDashboard with devMode=true renders DEV MODE banner"):
    ...
    assert(
      ...
      "Should contain dev-mode-banner div element"
    )
  ...
  test("renderDashboard with devMode=false does NOT render DEV MODE banner"):
    ...
    assert(
      !html.contains("<div class=\"dev-mode-banner\">"),
      "Should NOT contain dev-mode-banner div element"
    )
    assert(
  ...
  test(
    "renderDashboard links to external CSS file containing dev-mode-banner styles"
  ):
    ...
    assert(
      html.contains("href=\"/static/dashboard.css\""),
      "Should link to external CSS file with .dev-mode-banner styles"
    )
  ...
  test(
    "renderDashboard output does NOT contain inline visibilitychange script"
  ):
    val html = DashboardService.renderDashboard(
      worktrees = List.empty,
      reviewStateCache = Map.empty,
    ...
    val hasInlineScript =
      html.contains("<script>") && html.contains("visibilitychange")
    assert(
      !hasInlineScript,
      "Should not contain inline script with visibilitychange"
    )
  ...
  test("renderDashboard root page does NOT contain worktree-list div"):
    ...
    assert(
      !html.contains("worktree-list"),
      "Root page should not contain worktree-list div"
    )
    assert(html.contains("<!DOCTYPE html>"))
    assert(
      html.contains("main-projects-section") || html.contains(
  ...
  test("renderDashboard root page does NOT contain worktree-card HTML"):
    ...
    assert(
      !html.contains("worktree-card"),
      "Root page should not contain worktree-card elements"
    )
    assert(html.contains("<!DOCTYPE html>"))
    assert(html.contains("iw Dashboard"))
  ...
  test("renderDashboard root page does NOT poll /api/worktrees/changes"):
    ...
    assert(
      !html.contains("/api/worktrees/changes"),
      "Root page should not poll /api/worktrees/changes"
    )
    assert(html.contains("<!DOCTYPE html>"))
    assert(
      html.contains("modal-container"),
      "Modal container should still be present"
    )

  // Phase 2 Tests: Registered projects appear in dashboard even without worktrees (IW-148)

  test(
    "renderDashboard with registered projects and no worktrees renders project section"
  ):
    ...
    assert(html.contains("<!DOCTYPE html>"))
    assert(
      html.contains("main-projects-section") || html.contains("my-project"),
