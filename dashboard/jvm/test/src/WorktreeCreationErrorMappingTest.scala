...
import iw.dashboard.domain.{UserFriendlyError, WorktreeCreationError}
...
class WorktreeCreationErrorMappingTest extends FunSuite:
  ...
  test("DirectoryExists maps to user-friendly error with title"):
    ...
    assert(
      friendly.title.contains("Directory") || friendly.title.contains("Exists"),
  ...
  test("DirectoryExists maps with descriptive message"):
    ...
    assert(
      friendly.message.contains("exists") || friendly.message.contains(
        "directory"
      ),
  ...
  test("DirectoryExists maps with helpful suggestion"):
    ...
    assert(
      friendly.suggestion.get.contains("Remove") || friendly.suggestion.get
        .contains("open"),
  ...
  test("DirectoryExists is not retryable"):
    ...
    assert(
      !friendly.canRetry,
      "Should not be retryable until directory is handled"
    )
  ...
  test("AlreadyHasWorktree maps to user-friendly error"):
    val error =
      WorktreeCreationError.AlreadyHasWorktree("IW-79", "../iw-cli-IW-79")
    val friendly = WorktreeCreationError.toUserFriendly(error, testIssueId)

    assert(friendly.title.nonEmpty, "Should have a title")
    assert(
      friendly.title.toLowerCase
        .contains("already") || friendly.title.toLowerCase.contains("exists"),
  ...
  test("AlreadyHasWorktree maps with attach command suggestion"):
    ...
    assert(friendly.suggestion.isDefined, "Should have a suggestion")
    assert(
      friendly.suggestion.get.contains("tmux attach"),
      "Suggestion should show attach command"
    )
  ...
  test("GitError maps to user-friendly error"):
    ...
    assert(friendly.title.nonEmpty, "Should have a title")
    assert(
      friendly.title.toLowerCase.contains("git") || friendly.title.toLowerCase
        .contains("failed"),
  ...
  test("GitError maps with generic message (no internal details)"):
    ...
    assert(
      friendly.message.toLowerCase.contains("git"),
      "Should mention git in message"
    )
    assert(
      !friendly.message.contains("fatal"),
  ...
  test("GitError includes issueId for retry"):
    ...
    assert(
      friendly.issueId.contains(testIssueId),
      "Should include issueId for retry button"
    )
  ...
  test("TmuxError maps to user-friendly error"):
    ...
    assert(
      friendly.title.toLowerCase
        .contains("session") || friendly.title.toLowerCase.contains("failed"),
  ...
  test("TmuxError maps with generic message (no internal details)"):
    ...
    assert(
      friendly.message.toLowerCase.contains("session"),
      "Should mention session in message"
    )
    assert(
      !friendly.message.contains("xyz"),
      "Should not expose internal error details"
    )
  ...
  test("IssueNotFound maps to user-friendly error"):
    ...
    assert(friendly.title.nonEmpty, "Should have a title")
    assert(
      friendly.title.toLowerCase.contains("not found"),
  ...
  test("ApiError maps to user-friendly error"):
    ...
    assert(
      friendly.title.toLowerCase
        .contains("connection") || friendly.title.toLowerCase.contains("error"),
  ...
  test("ApiError maps with generic message (no internal details)"):
    ...
    assert(
      friendly.message.toLowerCase.contains(
        "connect"
      ) || friendly.message.toLowerCase.contains("tracker"),
      "Should mention connection issue"
    )
    assert(
      !friendly.message.contains("500"),
      "Should not expose internal error details"
    )
  ...
  test("ApiError maps with suggestion"):
    ...
    assert(friendly.suggestion.isDefined, "Should have a suggestion")
    assert(
      friendly.suggestion.get.toLowerCase.contains(
        "check"
      ) || friendly.suggestion.get.toLowerCase.contains("connection"),
  ...
  test("CreationInProgress maps to user-friendly error"):
    ...
    assert(friendly.title.nonEmpty, "Should have a title")
    assert(
      friendly.title.toLowerCase.contains(
        "progress"
      ) || friendly.title.toLowerCase.contains("creation"),
  ...
  test("CreationInProgress maps with descriptive message"):
    ...
    assert(
      friendly.message.toLowerCase.contains(
        "already"
      ) || friendly.message.toLowerCase.contains("progress"),
  ...
  test("CreationInProgress maps with helpful suggestion"):
    ...
    assert(
      friendly.suggestion.get.toLowerCase.contains(
        "wait"
      ) || friendly.suggestion.get.toLowerCase.contains("complete"),
  ...
  test("CreationInProgress is retryable"):
    ...
    assert(
      friendly.canRetry,
      "Should be retryable after current creation completes"
    )
