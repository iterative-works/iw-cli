...
class CaskServer(
  ...
  def dashboard(sshHost: Option[String] = None): cask.Response[String] =
    ...
    val prunedIds =
      stateService.pruneWorktrees(wt => os.exists(os.Path(wt.path, os.pwd)))

    // Auto-prune non-existent projects
    val prunedProjectPaths =
      stateService.pruneProjects(p => os.exists(os.Path(p.path, os.pwd)))
  ...
  def projectDetails(
    ...
    val filteredWorktrees =
      MainProjectService.filterByProjectName(allWorktrees, projectName)
    ...
    mainProjectOpt match
      ...
      case Some(mainProject) =>
        ...
        val worktreesWithData = filteredWorktrees.map { wt =>
          val issueData = DashboardService.fetchIssueForWorktreeCachedOnly(
            wt,
            state.issueCache,
            now
          )
  ...
  def worktreeDetail(
    ...
    state.worktrees.get(issueId) match
      ...
      case Some(worktree) =>
        ...
        val progress = DashboardService.fetchProgressForWorktree(
          worktree,
          state.progressCache
        )
  ...
  def artifactPage(issueId: String, path: String): cask.Response[String] =
    ...
    val readFile = (filePath: java.nio.file.Path) =>
      scala.util
        .Try {
    ...
    worktreeOpt match
      ...
      case Some(worktree) =>
        ...
        result.fetchedReviewState.foreach { cachedReviewState =>
          stateService.updateReviewStateCache(issueId)(_ =>
            Some(cachedReviewState)
          )
        }
  ...
  def worktreeDetailContent(
    ...
    state.worktrees.get(issueId) match
      ...
      case Some(worktree) =>
        ...
        val issueData = DashboardService.fetchIssueForWorktreeCachedOnly(
          worktree,
          updatedState.issueCache,
          now
        )
        val progress = DashboardService.fetchProgressForWorktree(
          worktree,
          updatedState.progressCache
        )
        val gitStatus = DashboardService.fetchGitStatusForWorktree(worktree)
        val prData = DashboardService.fetchPRForWorktreeCachedOnly(
          worktree,
          updatedState.prCache,
          now
        )
        val reviewStateResult = updatedState.reviewStateCache
          .get(issueId)
          .map(cached => Right(cached.state))

        val fragment = WorktreeDetailView
          .renderContent(
            worktree = worktree,
            issueData = issueData,
            progress = progress,
  ...
  def registerWorktree(
    ...
    val parsedBody: Either[cask.Response[ujson.Value], ujson.Value] = try
      ...
      case e: Exception =>
        System.err.println(
          s"Error reading request body: ${e.getClass.getName}: ${e.getMessage}"
        )
        Left(
          cask.Response(
            data = ujson.Obj(
  ...
  def searchIssues(
    ...
    val configPath =
      projectPath / Constants.Paths.IwDir / Constants.Paths.ConfigFileName
  ...
  def createWorktree(
    ...
    val projectPathResolved =
      projectPath.filter(_.nonEmpty).map(p => os.Path(p)).getOrElse(os.pwd)
    val configPath =
      projectPathResolved / Constants.Paths.IwDir / Constants.Paths.ConfigFileName
    val configOpt = ConfigFileRepository.read(configPath)
    ...
    configOpt match
      case None =>
        cask.Response(
          data =
            renderErrorView("Project not configured. Run './iw init' first."),
          statusCode = 500,
          headers = Seq("Content-Type" -> "text/html; charset=UTF-8")
        )
      ...
      case Some(config) =>
        ...
        val createWorktreeOp = (path: String, branchName: String) =>
          val actualPath =
            projectPathResolved / os.up / os.RelPath(path.stripPrefix("../"))
          GitWorktreeAdapter.createWorktree(
            actualPath,
            branchName,
            projectPathResolved
          )
        ...
        val createTmuxOp = (sessionName: String, workPath: String) =>
          val actualPath = projectPathResolved / os.up / os.RelPath(
            workPath.stripPrefix("../")
          )
          TmuxAdapter.createSession(sessionName, actualPath)

        // Register worktree function
        val registerWorktreeOp =
          (issueId: String, path: String, trackerType: String, team: String) =>
            val actualPath =
              projectPathResolved / os.up / os.RelPath(path.stripPrefix("../"))
        ...
        WorktreeCreationService.createWithLock(
          ...
          case Left(error) =>
            ...
            System.err.println(
              s"Worktree creation error for $issueId: ${error.getClass.getSimpleName}"
            )
            val statusCode = errorToStatusCode(error)
            val userFriendlyError =
              WorktreeCreationError.toUserFriendly(error, issueId)
  ...
  /** Assemble WorktreeStatus from live filesystem data and cached API data.
    *
    * Reads review state and progress directly from the worktree filesystem
    * (cheap). Uses cached issue and PR data from server state (avoids expensive
    * API calls). Gets live git status via command execution.
    *
    * @param issueId
    *   Issue identifier
    * @param worktree
    ...
    * @param state
    *   Current server state with caches
    * @return
    *   Assembled WorktreeStatus
    */
  private def assembleWorktreeStatus(
    ...
    val progress =
      DashboardService.fetchProgressForWorktree(worktree, state.progressCache)
  ...
  /** Fetch review state from worktree filesystem.
    ...
    * @return
    *   Optional ReviewState if file exists and is valid
    */
  ...
  /** Map domain error to appropriate HTTP status code.
    *
    * @param error
    *   Domain error from worktree creation
    * @return
    *   HTTP status code
    */
  private def errorToStatusCode(error: WorktreeCreationError): Int =
    error match
      case WorktreeCreationError.DirectoryExists(_) =>
  ...
  /** Build fetch function for WorktreeCardService based on tracker type.
    ...
    * @return
    *   Function that fetches issue by string ID
    */
  private def buildFetchFunction(
      trackerType: String,
      config: Option[ProjectConfiguration]
  ): String => Either[String, iw.core.model.Issue] =
    (issueId: String) =>
        ...
        case "youtrack" =>
          ...
          ApiToken.fromEnv(Constants.EnvVars.YouTrackApiToken) match
            case Some(token) =>
              IssueId
                .parse(issueId)
                .flatMap(YouTrackClient.fetchIssue(_, baseUrl, token))
            case None =>
  ...
  /** Build URL builder for WorktreeCardService based on tracker type.
    ...
    *   Optional project configuration
    * @return
    *   Function that builds issue URL from string ID
    */
  ...
  /** Build fetch function for IssueSearchService based on tracker type.
    ...
    *   Project configuration
    * @return
    *   Function that fetches issue by ID
    */
  private def buildFetchFunction(
      config: ProjectConfiguration
  ): IssueId => Either[String, iw.core.model.Issue] =
    (issueId: IssueId) =>
      config.trackerType.toString.toLowerCase match
        case "linear" =>
          ApiToken.fromEnv(Constants.EnvVars.LinearApiToken) match
  ...
  /** Build fetch recent function for IssueSearchService based on tracker type.
    ...
    *   Project configuration
    * @return
    *   Function that fetches recent issues with given limit
    */
  private def buildFetchRecentFunction(
      config: ProjectConfiguration
  ): Int => Either[String, List[iw.core.model.Issue]] =
    (limit: Int) =>
  ...
  /** Build search function for IssueSearchService based on tracker type.
    ...
    *   Project configuration
    * @return
    *   Function that searches issues by text query
    */
  private def buildSearchFunction(
      config: ProjectConfiguration
  ): String => Either[String, List[iw.core.model.Issue]] =
    (query: String) =>
      config.trackerType match
        case IssueTrackerType.GitHub =>
          config.repository match
  ...
  /** Extract GitHub issue number from issue ID.
    *
    * @param issueId
    *   Issue identifier (may have prefix)
    * @return
    *   Numeric issue number as string
    */
  private def extractGitHubIssueNumber(issueId: String): String =
    // Remove # prefix if present
    val withoutHash =
      if issueId.startsWith("#") then issueId.drop(1) else issueId
    // Extract number after hyphen if present (e.g., "IW-79" -> "79")
    val hyphenIndex = withoutHash.lastIndexOf('-')
    if hyphenIndex >= 0 then withoutHash.substring(hyphenIndex + 1)
    else withoutHash
