...
object WorktreeRegistrationService:
  ...
  /** Registers a new worktree or updates an existing one.
    *
    * For new worktrees: Sets both registeredAt and lastSeenAt to provided
    * timestamp. For existing worktrees: Updates path/trackerType/team and
    * lastSeenAt, preserves registeredAt.
    *
    * @param issueId
  ...
  /** Updates only the lastSeenAt timestamp for an existing worktree.
    ...
    *   The current server state
    * @return
    *   Either error message or updated server state
    */
  def updateLastSeen(
      issueId: String,
      timestamp: Instant,
