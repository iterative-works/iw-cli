...
class ServerStateService(repository: StateRepository):
  private val stateRef: AtomicReference[ServerState] = new AtomicReference(
    ServerState(
      Map.empty,
      Map.empty,
      Map.empty,
  ...
  /** Update a worktree entry using a function.
    ...
    *   Update function
    */
  def updateWorktree(issueId: String)(
      f: Option[WorktreeRegistration] => Option[WorktreeRegistration]
  ): Unit =
    lock.lock()
    try
  ...
  /** Update an issue cache entry using a function.
    ...
    *   Issue ID
    * @param f
    *   Update function (None to remove, Some to add/update)
    */
  def updateIssueCache(issueId: String)(
      f: Option[CachedIssue] => Option[CachedIssue]
  ): Unit =
    lock.lock()
    try
      val current = stateRef.get()
  ...
  /** Update a progress cache entry using a function.
    ...
    *   Update function (None to remove, Some to add/update)
    */
  def updateProgressCache(issueId: String)(
      f: Option[CachedProgress] => Option[CachedProgress]
  ): Unit =
    lock.lock()
    try
      val current = stateRef.get()
      ...
      val updated = f(currentCache) match
        case Some(cached) =>
          current.copy(progressCache =
            current.progressCache + (issueId -> cached)
          )
        case None =>
  ...
  /** Update a PR cache entry using a function.
    ...
    *   Update function (None to remove, Some to add/update)
    */
  def updatePRCache(issueId: String)(
      f: Option[CachedPR] => Option[CachedPR]
  ): Unit =
    lock.lock()
    try
      val current = stateRef.get()
  ...
  /** Update a review state cache entry using a function.
    ...
    *   Update function (None to remove, Some to add/update)
    */
  def updateReviewStateCache(issueId: String)(
      f: Option[CachedReviewState] => Option[CachedReviewState]
  ): Unit =
    lock.lock()
    try
      val current = stateRef.get()
      ...
      val updated = f(currentCache) match
        case Some(cached) =>
          current.copy(reviewStateCache =
            current.reviewStateCache + (issueId -> cached)
          )
        case None =>
  ...
  /** Update a project entry using a function.
    ...
    *   Update function
    */
  def updateProject(path: String)(
      f: Option[ProjectRegistration] => Option[ProjectRegistration]
  ): Unit =
    lock.lock()
    try
      val current = stateRef.get()
  ...
  /** Prune project registrations that don't pass validation.
    *
    * @param isValid
    *   Function to validate a project registration
    * @return
    *   Set of pruned paths
    */
  def pruneProjects(isValid: ProjectRegistration => Boolean): Set[String] =
    ...
    try
      val current = stateRef.get()
      val (valid, invalid) = current.projects.partition { case (_, p) =>
        isValid(p)
      }
  ...
  /** Prune worktrees that don't pass validation.
    ...
    *
    * @param isValid
    *   Function to validate a worktree
    * @return
    *   Set of pruned issue IDs
    */
  def pruneWorktrees(isValid: WorktreeRegistration => Boolean): Set[String] =
    lock.lock()
    try
      val current = stateRef.get()
      val (valid, invalid) = current.worktrees.partition { case (_, wt) =>
        isValid(wt)
      }
...
object ServerStateService:
  /** Legacy compatibility: load state from repository.
    *
    * @deprecated
    *   Use ServerStateService instance instead
    */
  ...
  /** Legacy compatibility: list worktrees by issue ID.
    *
    * @deprecated
    *   Use state.listByIssueId directly
    */
