...
/** Application service for workflow progress tracking.
  *
  * Uses file I/O injection pattern for testability. All logic is pure except
  * for file operations injected from caller.
  */
object WorkflowProgressService:

  /** Fetch workflow progress with cache support, returning CachedProgress.
    *
    * Uses explicit task list paths when provided, otherwise discovers phase
    * files. Checks cache validity using file modification times. Re-parses task
    * files if cache is invalid or missing. Returns CachedProgress so caller can
    * update server cache.
    *
    ...
    * @param cache
    *   Existing progress cache
    * @param taskListPaths
    *   Optional explicit list of task file paths (from review-state.json)
    * @param readFile
    *   Function to read file lines (injected I/O)
    * @param getMtime
    *   Function to get file modification time (injected I/O)
    * @return
    *   Either error message or CachedProgress
    */
  ...
  /** Fetch workflow progress with cache support.
    ...
    * Uses explicit task list paths when provided, otherwise discovers phase
    * files. Checks cache validity using file modification times. Re-parses task
    * files if cache is invalid or missing.
    *
    * File I/O is injected for testability (FCIS pattern).
    *
    * @param issueId
    ...
    *   Function to get file modification time (injected I/O)
    * @return
    *   Either error message or WorkflowProgress
    */
  def fetchProgress(
      issueId: String,
      worktreePath: String,
  ...
  private def fetchProgressInternal(
    ...
    val existingFiles = taskListPaths match
      case Some(paths) =>
        ...
        paths.zipWithIndex.flatMap { case (path, idx) =>
          val absolutePath =
            if path.startsWith("/") then path else s"$worktreePath/$path"
    ...
    if existingFiles.isEmpty then Left(s"No phase files found in $taskDir")
    else
      ...
      if mtimeErrors.nonEmpty then
        ...
        val currentMtimes = mtimesResult.collect { case Right(pair) =>
          pair
        }.toMap
  ...
  /** Parse phase files into PhaseInfo objects.
    *
    * @param phaseFiles
    *   List of discovered phase files
    * @param readFile
    *   Function to read file lines
    * @return
    *   Either error or list of PhaseInfo
    */
  private def parsePhaseFiles(
    ...
    val results = phaseFiles.map { file =>
      readFile(file.path).map { lines =>
        ...
        val phaseName = MarkdownTaskParser
          .extractPhaseName(lines)
    ...
    else Right(results.collect { case Right(info) => info })

  /** Compute workflow progress from phase info list.
    *
    * Pure function that aggregates phase information and determines current
    * phase.
    *
    * @param phases
    *   List of phase information
    * @param phaseIndex
    *   Phase completion status from tasks.md (source of truth for current
    *   phase)
    * @return
    *   WorkflowProgress with aggregated data
    */
  def computeProgress(
      phases: List[PhaseInfo],
      phaseIndex: List[PhaseIndexEntry] = List.empty
  ): WorkflowProgress =
    ...
    WorkflowProgress(
      ...
      totalPhases =
        if phaseIndex.nonEmpty then phaseIndex.size else phases.size,
      phases = phases,
  ...
  /** Determine current phase number.
    *
    * Uses Phase Index from tasks.md as source of truth when available. Falls
    * back to task-based detection if no Phase Index provided.
    *
    * With Phase Index:
    ...
    *   List of phase information
    * @param phaseIndex
    *   Phase completion status from tasks.md
    * @return
    *   Current phase number (1-based) or None
    */
  def determineCurrentPhase(
      phases: List[PhaseInfo],
      phaseIndex: List[PhaseIndexEntry] = List.empty
  ): Option[Int] =
    if phases.isEmpty then None
