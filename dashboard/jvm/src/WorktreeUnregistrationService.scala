...
object WorktreeUnregistrationService:
  /** Remove a worktree from state.
    ...
    * @param issueId
    *   Issue ID of the worktree to remove
    * @return
    *   Right with updated state if worktree exists, Left with error message if
    *   not found
    */
  def unregister(
      state: ServerState,
    ...
    else Left(s"Worktree not found: $issueId")
  ...
  /** Prune worktrees whose paths no longer exist.
    ...
    *   Current server state
    * @param pathExists
    *   Function to check if a path exists on filesystem
    * @return
    *   Updated state with non-existent worktrees and their caches removed
    */
