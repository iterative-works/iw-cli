...
object ProjectRegistrationService:
  ...
  /** Registers a new project or updates an existing one.
    *
    * For new projects: Sets registeredAt to provided timestamp. For existing
    * projects: Updates projectName/trackerType/team/trackerUrl, preserves
    * registeredAt.
    *
    * @param path
    ...
    * @param trackerType
    *   The tracker system (e.g., "Linear", "GitHub")
    * @param team
    *   The team identifier
    * @param trackerUrl
    *   Optional URL to the issue tracker
    * @param timestamp
    *   The timestamp for this registration
    * @param state
    *   The current server state
    * @return
    *   Either error message or tuple of (updated server state, wasCreated flag)
    */
  def register(
      path: String,
      projectName: String,
    ...
    ProjectRegistration
      .create(path, projectName, trackerType, team, trackerUrl, timestamp)
      .map { _ =>
        state.projects.get(path) match
          ...
          case None =>
            ...
            val created = ProjectRegistration(
              path,
              projectName,
              trackerType,
              team,
  ...
  /** Removes a project registration from state.
    ...
    *   The current server state
    * @return
    *   Updated server state with project removed
    */
