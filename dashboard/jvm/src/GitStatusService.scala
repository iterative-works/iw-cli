...
/** Git status detection service. Pure application logic - receives command
  * execution function from caller. Follows FCIS pattern: no side effects in
  * service, effects injected.
  */
object GitStatusService:

  /** Get git status for worktree.
    *
    * Executes git commands via injected execCommand function. Returns GitStatus
    * with branch name and clean/dirty state.
    *
    * @param worktreePath
    ...
    * @return
    *   Right(GitStatus) if success, Left(error message) if git unavailable
    */
  def getGitStatus(
      worktreePath: String,
    ...
    for
      branchOutput <- execCommand(
        "git",
        Array("-C", worktreePath, "rev-parse", "--abbrev-ref", "HEAD")
      )
      statusOutput <- execCommand(
        "git",
        Array("-C", worktreePath, "status", "--porcelain")
      )
    yield
  ...
  /** Parse branch name from git output.
    ...
    *
    * @param output
    *   Git rev-parse output (e.g., "main\n")
    * @return
    *   Some(branch name) if found, None if empty
    */
  ...
  /** Parse working tree status from git status --porcelain output.
    *
    * Returns true if clean (empty output), false if dirty. Dirty = uncommitted
    * changes OR staged changes.
    *
    * @param output
    *   Git status --porcelain output
    * @return
    *   true if working tree is clean, false if dirty
    */
