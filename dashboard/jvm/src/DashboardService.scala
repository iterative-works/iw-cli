...
object DashboardService:
  /** Render projects overview page with worktree summary counts.
    ...
    *   Explicitly registered projects (may have no worktrees)
  ...
  /** Fetch issue data for a single worktree from cache only (non-blocking).
    ...
    * @return
    *   Optional tuple of (IssueData, fromCache flag, isStale flag)
    */
  ...
  /** Fetch issue data for a single worktree using cache or API.
    ...
    *   Current timestamp
    * @return
    *   Optional tuple of (IssueData, fromCache flag)
    */
  private def fetchIssueForWorktree(
    ...
    val configPath =
      os.Path(wt.path) / Constants.Paths.IwDir / Constants.Paths.ConfigFileName
  ...
  /** Build fetch function based on tracker type.
    *
    * @param trackerType
    *   Tracker type string ("Linear" or "YouTrack")
    * @param config
    ...
    (issueId: String) =>
      trackerType.toLowerCase match
        case "linear" =>
          ...
          val tokenOpt =
            ApiToken.fromEnv(iw.core.model.Constants.EnvVars.LinearApiToken)
  ...
  /** Build URL builder based on tracker type.
    ...
    *   Optional project configuration
    * @return
    *   Function that builds issue URL from ID
    */
  ...
  /** Fetch workflow progress for a single worktree.
    *
    * Reads task files from the filesystem and parses progress. Returns None on
    * any error (missing files, read errors, etc.)
    *
    * @param wt
    *   Worktree registration
    * @param cache
    ...
    * @return
    *   Optional WorkflowProgress, None if unavailable
    */
  ...
  /** Fetch git status for a single worktree.
    ...
    *   Worktree registration
    * @return
    *   Optional GitStatus, None if unavailable
    */
  ...
  /** Fetch PR data for a single worktree from cache only (non-blocking).
    *
    * Returns cached PR data without calling CLI, regardless of cache age. PR
    * cache doesn't need isStale flag in phase 1 (not displayed separately).
    *
    * @param wt
    *   Worktree registration
    * @param cache
    *   Current PR cache
    * @param now
    *   Current timestamp (not used but kept for consistency)
    * @return
    *   Optional PullRequestData, None if not cached
    */
  ...
  /** Fetch PR data for a single worktree.
    *
    * Uses PR cache with TTL, re-fetches from gh/glab if expired. Returns None
    * on any error (no PR tool, no PR found, etc.)
    *
    * @param wt
    *   Worktree registration
    * @param cache
    ...
    * @return
    *   Optional PullRequestData, None if unavailable
    */
