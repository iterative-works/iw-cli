...
/** Result from rendering a card, including any freshly fetched data.
  ...
  * @param fetchedIssue
  *   Freshly fetched issue data (if any) that should be cached
  * @param fetchedProgress
  *   Freshly fetched progress data (if any) that should be cached
  * @param fetchedPR
  *   Freshly fetched PR data (if any) that should be cached
  * @param fetchedReviewState
  *   Freshly fetched review state (if any) that should be cached
  */
...
object WorktreeCardService:
  /** Render a single worktree card HTML fragment.
    ...
    *
    * @param issueId
    *   Issue ID for the worktree
    * @param worktrees
    *   Map of all registered worktrees
    * @param issueCache
    *   Current issue cache
    * @param progressCache
    ...
    * @param prCache
    *   Current PR cache
    * @param reviewStateCache
    *   Current review state cache
    * @param throttle
    *   Refresh throttle instance
    * @param now
    *   Current timestamp
    * @param sshHost
    *   SSH hostname for Zed editor links
    * @param fetchIssue
    *   Function to fetch issue from API
    * @param buildUrl
    *   Function to build issue URL
    * @return
    *   CardRenderResult with HTML string and optional fetched issue data
    */
  def renderCard(
      issueId: String,
    ...
    worktrees.get(issueId) match
      ...
      case Some(worktree) =>
        ...
        val (issueDataOpt, fetchedCachedIssue) =
          if shouldFetch then
            ...
            fetchIssue(issueId) match
              case Right(issue) =>
                ...
                (
                  Some((freshData, false, false)),
                  Some(cachedIssue)
                ) // fresh, not from cache, not stale
              case Left(_) =>
        ...
        val freshReviewState =
          fetchReviewStateForWorktree(worktree, reviewStateCache)
        ...
        val (reviewStateResult, reviewStateCacheUpdate) =
          freshReviewState match {
            ...
            case None =>
              val cached =
                reviewStateCache.get(issueId).map(cached => Right(cached.state))
              (cached, None)
          }
        ...
        CardRenderResult(
          ...
          progressCacheUpdate,
          prCacheUpdate,
          reviewStateCacheUpdate
        )

  /** Fetch review state for a single worktree.
    *
    * Reads review-state.json from the worktree and parses review state. Uses
    * cache with mtime validation.
    *
    * Returns:
    *   - None: No review state file (normal case, not an error)
    *   - Some(Left(error)): File exists but is invalid (parse error, malformed
    *     JSON)
  ...
  private def fetchReviewStateForWorktree(
    ...
    val reviewStatePath =
      s"${wt.path}/project-management/issues/${wt.issueId}/review-state.json"
    ...
    ReviewStateService.fetchReviewState(
      ...
      case Left(err)
          if err == reviewStatePath || err.contains(
            "NoSuchFileException"
      ...
      case Left(err) =>
        ...
        System.err.println(
          s"[WARN] Failed to load review state for ${wt.issueId}: $err"
        )
        Some(Left(err))
      case Right(cached) =>
        // Valid state
  ...
  /** Fetch workflow progress for a single worktree.
    *
    * Reads phase task files from the worktree and computes progress. Uses cache
    * with mtime validation.
    *
    * Returns:
  ...
  private def fetchProgressForWorktree(
    ...
    WorkflowProgressService.fetchProgressCached(
      ...
      case Left(error) =>
        ...
        System.err.println(
          s"[WARN] Failed to load progress for ${wt.issueId}: $error"
        )
        None
