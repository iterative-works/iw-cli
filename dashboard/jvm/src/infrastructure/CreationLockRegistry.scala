...
object CreationLockRegistry:
  ...
  /** Remove locks older than the specified maximum age.
    *
    * This cleanup prevents stale locks from blocking creation forever in case
    * of crashes or unexpected termination.
    *
    * @param maxAge
    *   Maximum age for a lock (e.g., Duration.ofSeconds(30))
    */
  def cleanupExpired(maxAge: Duration): Unit =
    ...
    locks.asScala.foreach { case (issueId, lock) =>
      if lock.startedAt.isBefore(cutoff) then locks.remove(issueId)
    }
