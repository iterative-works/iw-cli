...
object ReviewStateService:
  ...
  /** JSON codecs for review state domain models using snake_case disk format.
    */
  private object Codecs:
    ...
    given ReadWriter[Badge] = readwriter[ujson.Value].bimap[Badge](
      badge =>
        ujson.Obj(
          "label" -> ujson.Str(badge.label),
          "type" -> ujson.Str(badge.badgeType)
        ),
      jsValue => {
        val obj = jsValue.obj
    ...
    given ReadWriter[ReviewState] = readwriter[ujson.Value].bimap[ReviewState](
      state => {
        ...
        state.needsAttention.foreach(na =>
          obj("needs_attention") = ujson.Bool(na)
        )
  ...
  /** Parse review state from JSON string.
    *
    * Expects JSON format (v2): { "display": { // optional "text":
    * "Implementing", "subtext": "Phase 2 of 3", "type": "progress" }, "badges":
    * [ // optional {"label": "Batch", "type": "info"} ], "task_lists": [ //
    * optional {"label": "Phase 2", "path":
    * "project-management/issues/IW-136/phase-02-tasks.md"} ],
    * "needs_attention": false, // optional "message": "Ready", // optional
    * "activity": "working", // optional ("working" | "waiting")
    * "workflow_type": "agile", // optional ("agile" | "waterfall" |
    * "diagnostic") "artifacts": [ // required {"label": "Analysis", "path":
    * "project-management/issues/46/analysis.md"} ] }
    *
    * @param json
    *   JSON string to parse
    * @return
    *   Either error message or ReviewState
    */
  def parseReviewStateJson(json: String): Either[String, ReviewState] =
    try
      ...
      case e: Exception =>
        Left(s"Failed to parse review state JSON: ${e.getMessage}")

  /** Serialize review state to JSON string using the disk format (snake_case
    * keys).
    *
    * @param state
    *   ReviewState to serialize
    * @return
  ...
  /** Fetch review state with cache support.
    *
    * Checks cache validity using file modification time. Re-parses JSON file if
    ...
    * @param cache
    *   Existing review state cache
    * @param readFile
    *   Function to read file content (injected I/O)
    * @param getMtime
    *   Function to get file modification time (injected I/O)
    * @return
    *   Either error message or CachedReviewState
    */
  def fetchReviewState(
    ...
    val reviewStatePath =
      s"$worktreePath/project-management/issues/$issueId/review-state.json"
    ...
    getMtime(reviewStatePath) match {
      ...
      case Right(currentMtime) =>
        ...
        cache.get(issueId) match {
          case Some(cached)
              if CachedReviewState.isValid(cached, currentMtimes) =>
