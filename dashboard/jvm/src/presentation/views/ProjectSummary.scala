...
/** View model combining a main project with summary statistics.
  ...
  *   The main project
  * @param worktreeCount
  *   Number of worktrees for this project
  * @param attentionCount
  *   Number of worktrees needing attention
  */
...
object ProjectSummary:
  /** Compute project summaries from worktrees and review state cache.
    ...
    *   Map of issue ID to cached review state
    * @return
  ...
  def computeSummaries(
    ...
    projects.map { project =>
      ...
      val projectWorktrees =
        worktreesByProject.getOrElse(Some(projectPath), List.empty)
      ...
      val attentionCount = projectWorktrees.count { wt =>
        reviewStateCache
          .get(wt.issueId)
          .exists(_.state.needsAttention == Some(true))
      }
