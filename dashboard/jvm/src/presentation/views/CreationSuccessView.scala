...
object CreationSuccessView:
  /** Render success state after worktree creation.
    ...
    *
    * @param result
    *   Creation result with paths and commands
    * @return
    *   HTML fragment for success state
    */
  ...
  /** Render tmux command section with copy button.
    ...
    *   Creation result
    * @return
    *   HTML fragment for command section
    */
    ...
    div(
      ...
      div(
        ...
        button(
          ...
          attr(
            "onclick"
          ) := s"navigator.clipboard.writeText('${result.tmuxAttachCommand}')",
          "Copy"
        )
      )
    )
