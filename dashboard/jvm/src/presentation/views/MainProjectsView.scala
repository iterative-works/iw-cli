...
object MainProjectsView:
  /** Render main projects section with create buttons.
    *
    * @param summaries
    *   List of project summaries with worktree counts
    * @return
    *   HTML fragment
    */
  ...
  private def renderProjectCard(summary: ProjectSummary): Frag =
    ...
    val encodedPath =
      URLEncoder.encode(project.path.toString, StandardCharsets.UTF_8.toString)

    // Determine worktree count text (singular vs plural)
    val worktreeText =
      if summary.worktreeCount == 1 then "1 worktree"
      else s"${summary.worktreeCount} worktrees"
  ...
  /** Format tracker type for display.
    *
    * @param trackerType
    *   Tracker type string (lowercase)
    * @return
    *   Formatted tracker type (capitalized)
    */
