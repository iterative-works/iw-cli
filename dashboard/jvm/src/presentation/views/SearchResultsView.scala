...
object SearchResultsView:
  ...
  /** Render search results list.
    ...
    * @param projectPath
    *   Optional project path to scope worktree creation
    * @return
    *   HTML fragment for results
    */
  def render(
      results: List[IssueSearchResult],
      projectPath: Option[String] = None
  ): Frag =
    if results.isEmpty then renderEmptyState()
    else renderResults(results.take(MaxResults), projectPath)

  /** Render empty state when no results found.
    *
    * @return
    *   HTML fragment for empty state
    */
  ...
  /** Render list of search results.
    *
    * @param results
    *   List of results (already limited to max)
    * @param projectPath
    *   Optional project path to scope worktree creation
    * @return
    *   HTML fragment for results list
    */
  ...
  /** Render a single search result item.
    ...
    *
    * When clicked, triggers worktree creation via HTMX POST to
    * /api/worktrees/create, unless the issue already has a worktree.
    *
    * @param result
    *   Search result
    * @param projectPath
    *   Optional project path to scope worktree creation
    * @return
    *   HTML fragment for result item
    */
  private def renderResultItem(
    ...
    div(
      ...
      if result.hasWorktree then renderWorktreeBadge()
      else frag()
    )
