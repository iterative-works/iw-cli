...
/** Configuration for HTMX attributes on cards.
  ...
  *
  * @param trigger
  *   hx-trigger attribute value
  * @param swap
  *   hx-swap attribute value
  * @param oobSwap
  *   Optional hx-swap-oob attribute for OOB responses
  */
...
object HtmxCardConfig:
  /** Config for initial dashboard render - includes refresh trigger and
    * transitions
    */
...
object WorktreeCardRenderer:
  ...
  def renderCard(
    ...
    div(
      ...
      attr(
        "hx-disinherit"
      ) := "hx-vals", // Attempt to prevent inheriting parent's hx-vals (HTMX bug #1119 may prevent this)
  ...
  /** Render a skeleton card when issue data is not yet available.
    ...
    *   Worktree registration
    * @param gitStatus
    *   Optional git status (may be available before issue data)
    * @param now
    *   Current timestamp
    * @param htmxConfig
    *   HTMX attribute configuration
  ...
  def renderSkeletonCard(
    ...
    div(
      ...
      h3(
        cls := "skeleton-title",
        a(href := s"/worktrees/${worktree.issueId}", "Loading...")
      ),
      p(
        cls := "issue-id",
    ...
    reviewStateResult match {
      ...
      case Some(Left(error)) =>
        ...
        div(
          ...
          p(
            cls := "review-error-detail",
            "The review state file exists but could not be loaded. Check for JSON syntax errors."
          )
        )
