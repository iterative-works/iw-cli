...
setup() {
    ...
    # --describe never invokes scala-cli, so it does not need the core/ tree
    # or a resolved IW_CORE_JAR — only the shared commands so iw-run can
    # resolve the canonical install layout.
