...
@test "phase-merge --max-retries 0 exits non-zero immediately without recovery hook" {
    mkdir -p "project-management/issues/TEST-100"
    cat > "project-management/issues/TEST-100/review-state.json" << 'EOF'
{
  "version": 2,
...
GHEOF
    ...
    # Without a recovery hook, warns about missing hook
    [[ "$output" == *"No recovery action hook"* ]]
    [[ "$output" == *"lint"* ]]
...
@test "phase-merge CI failure without recovery hook exits non-zero with warning" {
    mkdir -p "project-management/issues/TEST-100"
    cat > "project-management/issues/TEST-100/review-state.json" << 'EOF'
{
  "version": 2,
...
GHEOF
    ...
    # Without a recovery hook, warns and exits
    [[ "$output" == *"No recovery action hook"* ]]
    [[ "$output" == *"Fix the failures manually"* ]]
}

@test "phase-merge CI failure without hook sets review-state activity to waiting" {
    mkdir -p "project-management/issues/TEST-100"
    cat > "project-management/issues/TEST-100/review-state.json" << 'EOF'
{
  "version": 2,
