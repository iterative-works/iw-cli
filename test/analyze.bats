#!/usr/bin/env bats
# PURPOSE: E2E smoke for iw analyze (full round-trip through scala-cli + live adapters)
# PURPOSE: Detailed scenarios live in core/test/AnalyzeHarnessTest.scala

PROJECT_ROOT="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)"

TMUX_SOCKET="iw-test-$$"

setup() {
    export IW_SERVER_DISABLED=1
    export IW_TMUX_SOCKET="$TMUX_SOCKET"

    TEST_DIR="$(mktemp -d)"
    cd "$TEST_DIR"

    git init -q 2>/dev/null
    git config user.email "test@example.com"
    git config user.name "Test User"
    echo "initial" > README.md
    git add README.md
    git commit -q -m "Initial commit"

    mkdir -p .iw
    cat > .iw/config.conf << 'EOF'
project {
  name = testproject
}

tracker {
  type = linear
  team = TEST
}
EOF
}

teardown() {
    tmux -L "$TMUX_SOCKET" kill-server 2>/dev/null || true
    if [ -n "$TEST_DIR" ]; then
        local parent_dir="$(dirname "$TEST_DIR")"
        rm -rf "$parent_dir"/testproject-* 2>/dev/null || true
    fi
    cd /
    rm -rf "$TEST_DIR"
}

@test "analyze delegates to iw start and creates worktree + session" {
    local parent_dir="$(dirname "$(pwd)")"
    local expected_worktree="$parent_dir/testproject-IWLE-123"

    run "$PROJECT_ROOT/iw" analyze IWLE-123

    [ "$status" -eq 0 ]
    [ -d "$expected_worktree" ]
    tmux -L "$TMUX_SOCKET" has-session -t "testproject-IWLE-123" 2>/dev/null
}
