...
- [x] Read existing `start.scala` and `open.scala` to understand current arg parsing
- [x] Read `TmuxAdapter.sendKeys` signature and tests from Phase 2

## Tests (write first — TDD)

- [x] Create `start-prompt.bats` with E2E tests:
...
- [x] Create `open-prompt.bats` with E2E tests:
...
- [x] Run `./iw test e2e` — all existing + new tests pass
- [x] Run `./iw test unit` — all unit tests pass
- [x] Manual smoke test: `iw start --prompt "hello" TEST-1` in a real tmux session
