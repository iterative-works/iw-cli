...
- [ ] [setup] Confirm the self-hosted runner is reachable and currently servicing `ci.yml`'s `compile`, `format`, `lint`, `test`, and `dashboard-build` jobs (so the same runner pool will pick up the modified `release.yml`).
- [ ] [setup] Confirm `ghcr.io/iterative-works/iw-cli-ci:latest` exists in GHCR and the runner has registry credentials via `secrets.GITHUB_TOKEN`.
- [ ] [setup] Confirm `WEBAWESOME_NPM_TOKEN` is configured in the repo's secrets (already consumed by `ci.yml:dashboard-build`).
- [ ] [setup] Confirm Phase 1 commits are in the working tree (PR #371 merged; `scripts/package-release.sh`, `iw-run`, `iw-bootstrap` reflect Phase 1 layout).
- [ ] [setup] Have a freshly-built local tarball available for BATS verification (`./scripts/package-release.sh "$(cat VERSION)"` or `./scripts/package-release.sh 0.1.0-dev` depending on whether the version-parameterisation optional task is taken).

## Implementation — `.github/workflows/release.yml`

- [ ] [impl] Change `runs-on: ubuntu-latest` (line 13) to `runs-on: self-hosted`.
- [ ] [impl] Add a `container:` block immediately after `runs-on:` with image `ghcr.io/iterative-works/iw-cli-ci:latest` and credentials (`username: ${{ github.actor }}`, `password: ${{ secrets.GITHUB_TOKEN }}`), mirroring `ci.yml:dashboard-build` lines 68-85.
- [ ] [impl] Drop the `coursier/setup-action@v1` step (lines 16-19). The container provides Java + scala-cli.
- [ ] [impl] Drop the `coursier/cache-action@v6` step (line 20). The container has its own coursier cache.
- [ ] [impl] Drop the `Install E2E test dependencies` step (lines 22-26). `tmux`, `jq`, and `bats` are baked into the container.
- [ ] [impl] Keep `actions/checkout@v4` (line 15) verbatim.
- [ ] [impl] Add `WEBAWESOME_NPM_TOKEN: ${{ secrets.WEBAWESOME_NPM_TOKEN }}` to the `env:` of the `Run all tests` step (lines 28-30) — defensive parity with `ci.yml:test` (lines 60-61).
- [ ] [impl] Keep the `Extract version from tag` step (lines 32-42) verbatim.
- [ ] [impl] Add an `env:` block to the `Build release tarball` step (lines 44-45) containing `WEBAWESOME_NPM_TOKEN: ${{ secrets.WEBAWESOME_NPM_TOKEN }}`. This is **load-bearing** — `package-release.sh` invokes `./mill ... show dashboard.assembly`, which runs Vite, which fetches Web Awesome from the Pro npm registry.
- [ ] [impl] Keep the `Upload to versioned release` step (lines 47-58) verbatim.
- [ ] [impl] Keep the `Update vlatest release` step (lines 60-80) verbatim.

## Implementation — `test/bootstrap.bats`

- [ ] [impl] In the structure test (currently lines 73-93), drop the `core/Config.scala` assertion at line 87. Phase 1 (Decision 1) stops shipping that file.
- [ ] [impl] Keep the `core/project.scala` assertion at line 88 — that's the deps manifest the launcher reads.
- [ ] [impl] Add `[ -f "iw-cli-0.1.0-dev/build/iw-core.jar" ]` to the structure test.
- [ ] [impl] Add `[ -f "iw-cli-0.1.0-dev/build/iw-dashboard.jar" ]` to the structure test.
- [ ] [impl] Add non-empty assertions for both jars (e.g. `[ -s "iw-cli-0.1.0-dev/build/iw-core.jar" ]` and `[ -s "iw-cli-0.1.0-dev/build/iw-dashboard.jar" ]`) — the launcher does a literal path test, drift would silently break installed users.
- [ ] [impl] Update the `--bootstrap` test (currently lines 42-54): replace the `[[ "$output" == *"Bootstrap complete"* ]]` assertion (line 53) with `[[ "$output" == *"pre-built jars present"* ]]`. The stricter form doubles as a contract check that bootstrap from a tarball does NOT fall through to Mill.
- [ ] [impl] Keep the `[[ "$output" == *"Bootstrapping"* ]]` entry-message assertion (line 52) unchanged.
- [ ] [impl] Add a new test `@test "iw-run works without Mill on PATH (bundled jars only)"` exactly as drafted in phase-02-context.md lines 121-141: extracts the tarball, narrows `PATH` to keep `scala-cli`/`java`/`jq` reachable but Mill hidden, runs `./iw-run --list` (assert success and `Available commands` substring) and `./iw-run --bootstrap` (assert success and `pre-built jars present` substring), restores `PATH`. Adjust PATH munging to the CI container's layout if necessary; principle is "remove `./mill` reachability while keeping `scala-cli`, `java`, `jq`".
...
- [ ] [impl] Rewrite step 2's tarball description (line 26) from `release/iw-cli-0.1.0.tar.gz - Contains iw-run, commands, and core files` to reflect the Phase 1 reality: `release/iw-cli-0.1.0.tar.gz - Contains iw-run, iw-bootstrap, commands, build/iw-core.jar, build/iw-dashboard.jar, and core/project.scala (deps manifest)`.
- [ ] [impl] Replace the "Release tarball too large" troubleshooting checklist (lines 112-117) with the actual Phase 1 layout: `iw-run`, `iw-bootstrap`, `VERSION`, `commands/**/*.scala`, `core/project.scala` (deps manifest only — NOT all of `core/`), `build/iw-core.jar`, `build/iw-dashboard.jar`.
- [ ] [impl] Add a callout to the same troubleshooting entry that the tarball MUST NOT contain `./mill`, `.mill-version`, `build.mill`, `out/`, `.bsp/`, `dashboard/jvm/`, or `dashboard/frontend/` (Decision 5).
- [ ] [impl] Add a new "Tarball Contract" section (placement: after "Distribution Model" at line 108 / before "Troubleshooting" at line 110, OR as a subsection under "Distribution Model"). The section MUST cover:
...
- [ ] [optional] [impl] Add a brief paragraph after step 2 listing the tarball's `build/` contents for auditors (pulls from `analysis.md:129`).
- [ ] [optional] [impl] Add Mill / Node 20 / Yarn / `WEBAWESOME_NPM_TOKEN` to the prerequisites for running `package-release.sh` locally, near step 2 (pulls from `analysis.md:128`).

## Verification

- [ ] [verify] **Local BATS run** against a freshly-built tarball: `./iw ./test e2e` (or `bats test/bootstrap.bats` directly). The updated structure test, the updated `--bootstrap` test, and the new "iw-run works without Mill on PATH" test must all pass.
...
- [ ] [verify] Record outcomes in `implementation-log.md`: BATS results, RC tag dry-run results, and explicit disposition of the BATS hardcoded-version drift (fixed in this phase or punted to a follow-up).
