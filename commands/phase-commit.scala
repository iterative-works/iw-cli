...
@main def phaseCommit(args: String*): Unit =
  ...
  val taskFilePath =
    os.pwd / "project-management" / "issues" / issueId.value / s"phase-${phaseNumber.value}-tasks.md"
  if os.exists(taskFilePath) then
    ...
    if unchecked.nonEmpty then
      ...
      Output.error("")
      Output.error(
        s"If these tasks have been implemented, check them off in phase-${phaseNumber.value}-tasks.md and retry."
      )
      Output.error(
        "If they have NOT been implemented, complete them before committing."
      )
      sys.exit(1)

  // Require a clean worktree: all changes must be staged, nothing unstaged or untracked
  val stagingCheck =
    CommandHelpers.exitOnError(GitAdapter.getStagingCheck(os.pwd))
  stagingCheck.readyToCommit match
    case Left(err) =>
      ...
      Output.error("")
      Output.error(
        "Stage your implementation files with `git add`, then re-run phase-commit."
      )
      sys.exit(1)
    case Right(()) => ()
