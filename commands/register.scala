  ...
  IssueId.fromBranch(branch) match
    case Left(_) =>
      ...
      ServerClient.registerProject(
        ...
        case Right(_) =>
          Output.success(
            s"Registered project '${config.projectName}' at ${currentDir}"
          )
      ...
      ServerClient.registerWorktree(
        ...
        case Right(_) =>
          ...
          ProjectPath.deriveMainProjectPath(currentDir.toString) match
            case None =>
              Output.warning(
                "Could not derive parent project path from current directory"
              )
            case Some(parentPath) =>
              val parentConfigPath =
                os.Path(parentPath) / Constants.Paths.IwDir / "config.conf"
              ConfigFileRepository.read(parentConfigPath) match
                case None =>
                  Output.warning(
                    s"Could not read parent project config at $parentConfigPath, skipping project registration"
                  )
                case Some(parentConfig) =>
                  val trackerUrl =
                    TrackerUrlBuilder.buildTrackerUrl(parentConfig)
                  ServerClient.registerProject(
          ...
          Output.success(
            s"Registered worktree for ${issueId.value} at ${currentDir}"
          )
