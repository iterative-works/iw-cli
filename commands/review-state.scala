...
private def handleWriteStdin(argList: List[String]): Unit =
  ...
  val outputPath = ReviewStateCliParser.extractFlag(argList, "--output") match
  ...
  val issueId = ReviewStateCliParser.extractFlag(argList, "--issue-id") match
    case Some(id) => id
    case None     =>
...
private def handleWriteFlags(argList: List[String]): Unit =
  val resolvedIssueId =
    ReviewStateCliParser.extractFlag(argList, "--issue-id") match
      case Some(id) => id
      case None     =>
        GitAdapter.getCurrentBranch(os.pwd).flatMap(IssueId.fromBranch) match
          case Right(id) => id.value
  ...
  val defaults = ReviewStateCliParser.WriteDefaults(
    ...
    gitSha = GitAdapter.getHeadSha(os.pwd).toOption
  )

  val input = ReviewStateCliParser.parseWriteArgs(argList, defaults) match
    case Right(v)  => v
    case Left(err) =>
      Output.error(err)
      sys.exit(1)
  ...
  val outputPath = ReviewStateCliParser.extractFlag(argList, "--output") match
    case Some(p) => os.Path(p, os.pwd)
    case None    =>
      val dir = os.pwd / "project-management" / "issues" / resolvedIssueId
      dir / "review-state.json"

  os.makeDir.all(outputPath / os.up)
  os.write.over(outputPath, json)

  commitIfRequested(argList, resolvedIssueId, outputPath)
...
private def commitIfRequested(
  ...
  if argList.contains("--commit") then
    val status = ReviewStateCliParser.extractFlag(argList, "--status")
...
private def update(args: Seq[String]): Unit =
  ...
  val inputPath = ReviewStateCliParser.extractFlag(argList, "--input") match
    case Some(p) => os.Path(p, os.pwd)
    case None    =>
  ...
  val updateInput = ReviewStateCliParser.parseUpdateArgs(argList) match
    case Right(v)  => v
    case Left(err) =>
      Output.error(err)
      sys.exit(1)
