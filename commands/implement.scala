...
@main def implement(args: String*): Unit =
  ...
  val issueId = issueIdArg match
    ...
    case None =>
      IssueId.fromBranch(currentBranch) match
        ...
        case Left(_)   =>
          Output.error(
            "Cannot determine issue ID. Provide it as a positional argument (e.g. IWLE-123)"
          )
          Output.error(
            "Usage: iw implement [ISSUE_ID] [--batch] [--phase N] [--model MODEL]"
          )
          sys.exit(1)
  ...
  if batch then
    ...
    val batchArgs =
      buildBatchArgs(issueId, model, maxTurns, maxBudgetUsd, argList)
    val exitCode = ProcessAdapter.runInteractive(
      Seq(iwScript, "batch-implement") ++ batchArgs
    )
    ...
    if workflowType.isEmpty then
      ...
      Output.error(
        "Add a workflow_type field (agile, waterfall, or diagnostic) to review-state.json"
      )
      sys.exit(1)
...
private def buildBatchArgs(
  ...
  val knownFlags =
    Set("--batch", "--phase", "--model", "--max-turns", "--max-budget-usd")
  val passthroughFlags =
    argList.filter(a => a.startsWith("--") && !knownFlags.contains(a))
  Seq(issueId.value) ++
