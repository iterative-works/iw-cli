...
object ContributingHookDoctor:
  ...
  val fileExists: Check =
    Check("CONTRIBUTING.md", ContributingChecks.checkFileExists, "Quality")

  // Check if CONTRIBUTING.md has sections covering CI, hooks, and local checks
  val sectionsCovered: Check = Check(
    "CONTRIBUTING.md sections",
    ContributingChecks.checkSectionsCovered,
    "Quality"
  )
