...
@main def phaseStart(args: String*): Unit =
  ...
  val phaseNumber =
    CommandHelpers.exitOnError(PhaseNumber.parse(phaseNumberRaw))
  ...
  CommandHelpers.exitOnError(
    GitAdapter.push(featureBranch, os.pwd, setUpstream = true)
  )

  CommandHelpers.exitOnError(
    GitAdapter.createAndCheckoutBranch(branchName, os.pwd)
  )

  val baselineSha =
    CommandHelpers.exitOnError(GitAdapter.getFullHeadSha(os.pwd))
  ...
  if os.exists(reviewStatePath) then
    ReviewStateAdapter.update(
      ...
      case Right(_) =>
        // Commit the review-state update so the working tree stays clean
        GitAdapter
          .commitFileWithRetry(
            reviewStatePath,
