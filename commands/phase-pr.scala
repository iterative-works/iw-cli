...
@main def phasePr(args: String*): Unit =
  ...
  val prBody = bodyArg.getOrElse {
    val fileUrlBase =
      remoteOpt.flatMap(r => FileUrlBuilder.build(r, currentBranch).toOption)
  ...
  val prUrl = forgeType match
    case ForgeType.GitHub =>
      GitHubClient.createPullRequest(
        ...
        case Left(err)  =>
          Output.error(
            s"The branch '$currentBranch' was already pushed. You can create the PR manually."
          )
    ...
    case ForgeType.GitLab =>
      ...
      GitLabClient.createMergeRequest(
        ...
        title,
        prBody,
        execCommand = glabExec
      ) match
        case Right(url) => url
        case Left(err)  =>
          Output.error(
            s"The branch '$currentBranch' was already pushed. You can create the MR manually."
          )
    ...
    if batchMode then
      val mergeResult = forgeType match
        ...
        case ForgeType.GitLab =>
          ...
          ProcessAdapter.run(
            Seq(forgeType.cliTool, "mr", "merge", "--squash", prUrl),
            env = envVars
          )
      ...
      if mergeResult.exitCode != 0 then
        ...
        Output.error(
          s"PR was created at $prUrl. Run 'iw phase-advance' after merging manually."
        )
        sys.exit(1)
      ...
      CommandHelpers.exitOnError(
        GitAdapter.fetchAndReset(featureBranch, os.pwd)
      )

      if os.exists(reviewStatePath) then
        ReviewStateAdapter.update(
          reviewStatePath,
          ...
          case Right(_) =>
            commitReviewState(reviewStatePath, issueId, phaseNumber) match
              case Left(err) =>
                Output.error(s"Warning: Failed to persist review-state: $err")
              case Right(_) => ()
      ...
      if os.exists(reviewStatePath) then
        ReviewStateAdapter.update(
          ...
          case Right(_) =>
            val committed =
              commitReviewState(reviewStatePath, issueId, phaseNumber)
