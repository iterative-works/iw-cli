...
object ScalafmtHookDoctor:
  ...
  val configExists: Check =
    Check(".scalafmt.conf", ScalafmtChecks.checkConfigExists, "Quality")

  // Check if .scalafmt.conf has a version configured
  val versionExists: Check = Check(
    ".scalafmt.conf version",
    ScalafmtChecks.checkVersionExists,
    "Quality"
  )
