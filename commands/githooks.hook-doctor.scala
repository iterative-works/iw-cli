...
object GitHooksHookDoctor:
  ...
  val hooksDir: Check =
    Check("Git hooks dir", GitHooksChecks.checkHooksDirExists, "Quality")

  // Check if pre-commit and pre-push hook files exist
  val hookFiles: Check =
    Check("Git hook files", GitHooksChecks.checkHookFilesExist, "Quality")

  // Check if hooks are installed in git hooks directory
  val hooksInstalled: Check =
    Check("Git hooks installed", GitHooksChecks.checkHooksInstalled, "Quality")
