...
@main def phaseMerge(args: String*): Unit =
  ...
  val maxRetriesStr =
    PhaseArgs.namedArg(argList, "--max-retries").getOrElse("2")
  ...
  val maxRetries = maxRetriesStr.toIntOption match
    case None =>
      Output.error(
        s"Invalid --max-retries: '$maxRetriesStr' is not a valid integer"
      )
      sys.exit(1)
    case Some(n) if n < 0 =>
  ...
  val currentBranch =
    CommandHelpers.exitOnError(GitAdapter.getCurrentBranch(os.pwd))
  ...
  val reviewStatePath =
    os.pwd / "project-management" / "issues" / issueId.value / "review-state.json"
  ...
  def tryUpdateState(input: ReviewStateUpdater.UpdateInput): Unit =
    if os.exists(reviewStatePath) then
      ReviewStateAdapter.update(reviewStatePath, input) match
        case Left(err) =>
          Output.error(s"Warning: Failed to update review-state: $err")
        case Right(_) => ()

  val prUrl =
    CommandHelpers.exitOnError(ReviewStateAdapter.readPrUrl(reviewStatePath))
  ...
  def poll(): CIVerdict =
    ...
    if elapsed > mergeConfig.timeoutMs then
      ...
      Output.error(
        s"Timed out waiting for CI checks after ${PhaseMerge.formatDuration(mergeConfig.timeoutMs)}."
      )
    ...
    verdict match
      ...
      case CIVerdict.StillRunning =>
        val pendingNames = checks
          .filter(_.status == CICheckStatus.Pending)
          .map(_.name)
          .mkString(", ")
        Output.info(
          s"CI still running (pending: $pendingNames). Waiting ${PhaseMerge.formatDuration(mergeConfig.pollIntervalMs)}..."
        )
  ...
  def invokeRecoveryAgent(
    ...
    tryUpdateState(
      ReviewStateUpdater.UpdateInput(
        ...
        displayText = Some(
          s"Phase ${phaseNumber.value}: CI Fixing (attempt $attemptDisplay)"
        )
      )
    )
    ...
    poll() match
      case CIVerdict.SomeFailed(failedChecks) =>
        if PhaseMerge.shouldRetry(attempt, mergeConfig) then
          ...
          Output.error(
            s"CI checks still failing after ${mergeConfig.maxRetries} recovery attempt(s). Giving up."
          )
  ...
  val mergeCmd = forgeType match
    case ForgeType.GitHub =>
      Seq("gh") ++ GitHubClient.buildMergePrWithDeleteCommand(prUrl).toSeq
    case ForgeType.GitLab =>
      Seq("glab") ++ GitLabClient.buildMergeMrWithDeleteCommand(prUrl).toSeq
  val mergeEnv =
