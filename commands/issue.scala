...
@main def issue(args: String*): Unit =
  // Handle subcommands
  if args.nonEmpty && args(0) == "create" then handleCreateSubcommand(args.tail)
  else
...
private def createYouTrackIssue(
    title: String,
    description: String,
    config: ProjectConfiguration
): Unit =
...
def loadConfig(): Either[String, ProjectConfiguration] =
  val configPath = os.Path(
    System.getProperty(Constants.SystemProps.UserDir)
        ...
        case Some(repository) =>
          ...
          GitLabClient.fetchIssue(
            ...
            case Left(error) if GitLabClient.isNotFoundError(error) =>
              Left(
                GitLabClient.formatIssueNotFoundError(issueId.value, repository)
              )
