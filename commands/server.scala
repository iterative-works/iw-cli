...
def startServer(): Unit =
  ...
  val spawnResult =
    ProcessManager.spawnServerProcess(statePath, config.port, config.hosts)
  ...
  def waitForHealth(remaining: Int): Boolean =
    if remaining <= 0 then false
      ...
      val isHealthy = try
        val response =
          quickRequest.get(uri"http://localhost:${config.port}/health").send()
  ...
  if healthy then
    ...
    ServerLifecycleService.formatSecurityWarning(securityAnalysis).foreach {
      warning =>
        println(warning)
        println()
    }
  ...
  maybePid match
    ...
    case Some(pid) =>
      ...
      if !ProcessManager.isProcessAlive(pid) then
        ...
        try
          val response =
            quickRequest.get(uri"http://localhost:$port/api/status").send()
