  ...
  def fromHocon(hocon: String): Either[String, ProjectConfiguration] =
    try
      ...
      val trackerTypeEither = trackerTypeStr match
        case Constants.TrackerTypeValues.Linear =>
          Right(IssueTrackerType.Linear)
        case Constants.TrackerTypeValues.YouTrack =>
          Right(IssueTrackerType.YouTrack)
        case Constants.TrackerTypeValues.GitHub =>
      ...
      trackerTypeEither.flatMap { trackerType =>
        ...
        val team =
          if config.hasPath(Constants.ConfigKeys.TrackerTeam) then
        ...
        val teamPrefixEither: Either[String, Option[String]] =
          if config.hasPath(Constants.ConfigKeys.TrackerTeamPrefix) then
            val prefix =
              config.getString(Constants.ConfigKeys.TrackerTeamPrefix)
        ...
        val version =
          if config.hasPath(Constants.ConfigKeys.Version) then
