...
object FileUrlBuilder:
  ...
  /** Build a browseable file URL base from remote URL and branch name.
    *
    * GitHub pattern: https://github.com/owner/repo/blob/{branch}/ GitLab
    * pattern: https://gitlab.com/owner/project/-/blob/{branch}/
    *
    * @param remote
    ...
    *   Branch name
    * @return
    *   Right(url base) or Left(error) if remote format unrecognized
    */
  def build(remote: GitRemote, branch: String): Either[String, String] =
    remote.host.flatMap { host =>
      extractRepoPath(remote).map { repoPath =>
        ...
        else s"https://$host/$repoPath/-/blob/$branch/"
      }
    }

  /** Extract the repository path (owner/repo or owner/group/project) from the
    * remote URL.
    */
  private def extractRepoPath(remote: GitRemote): Either[String, String] =
    ...
    val rawPath =
      if url.startsWith("git@") then
        ...
        val afterProtocol =
          url.dropWhile(_ != '/').drop(2) // remove protocol://
        val afterUsername =
