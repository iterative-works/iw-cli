...
object ScalafmtChecks:
  ...
  /** Check if .scalafmt.conf exists in project root.
    ...
    * @return
    *   CheckResult indicating .scalafmt.conf presence
    */
  def checkConfigExistsWith(
    ...
    if fileExists(scalafmtConf) then CheckResult.Success("Found")
    else CheckResult.Error("Missing", "Create .scalafmt.conf in project root")
  ...
  /** Check if .scalafmt.conf has a version key configured.
    ...
    * @return
    *   CheckResult indicating version configuration status
    */
  def checkVersionExistsWith(
    ...
    readFile(scalafmtConf) match
      case None =>
        CheckResult.Error(
          "Cannot read file",
          "Ensure .scalafmt.conf exists and is readable"
        )
      case Some(content) =>
        ...
        if versionPattern.findFirstIn(content).isDefined then
          ...
          CheckResult.WarningWithHint(
            ...
            "Add 'version = \"3.x.x\"' to .scalafmt.conf"
          )

  /** Check if .scalafmt.conf has a version key configured (uses real file
    * system).
    *
    * @param config
    *   Project configuration
