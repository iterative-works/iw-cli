...
object PhaseNumber:
  /** Parse and validate a phase number string. Accepts: "01", "1", "12" --
    * always normalizes to zero-padded two digits. Rejects: "0", "100", "-1",
    * "abc", ""
    */
  def parse(raw: String): Either[String, PhaseNumber] =
    raw.toIntOption match
      case None =>
        Left(
          s"Invalid phase number: '$raw' (expected a number between 1 and 99)"
        )
      case Some(n) if n < 1 =>
        Left(s"Invalid phase number: $n (phases are 1-based, must be >= 1)")
      case Some(n) if n > 99 =>
        Left(
          s"Invalid phase number: $n (exceeds maximum two-digit phase number 99)"
        )
      case Some(n) =>
        Right(f"$n%02d")
