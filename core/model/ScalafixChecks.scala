...
object ScalafixChecks:
  ...
  /** Check if .scalafix.conf exists in project root.
    ...
    * @return
    *   CheckResult indicating .scalafix.conf presence
    */
  def checkConfigExistsWith(
    ...
    if fileExists(scalafixConf) then CheckResult.Success("Found")
    else CheckResult.Error("Missing", "Create .scalafix.conf in project root")
  ...
  /** Check if .scalafix.conf has DisableSyntax rule with required sub-rules
    ...
    * @return
    *   CheckResult indicating DisableSyntax rule configuration status
    */
  def checkDisableSyntaxRulesWith(
    ...
    readFile(scalafixConf) match
      case None =>
        CheckResult.Error(
          "Cannot read file",
          "Ensure .scalafix.conf exists and is readable"
        )
      case Some(content) =>
        // Check if DisableSyntax rule is present
        if !content.contains("DisableSyntax") then
          CheckResult.WarningWithHint(
            "DisableSyntax not configured",
            "Add DisableSyntax rule to .scalafix.conf"
          )
        else
          // Check for required sub-rules
          val requiredRules = List("noNulls", "noVars", "noThrows", "noReturns")
          val missingRules =
            requiredRules.filterNot(rule => content.contains(rule))

          if missingRules.isEmpty then CheckResult.Success("Configured")
          else
            CheckResult.WarningWithHint(
