...
/** API token wrapper that masks the value in toString to prevent accidental
  * logging. The actual value is private and only accessible via the `value`
  * method.
  */
final case class ApiToken private (private val rawValue: String):
  ...
  override def toString: String =
    ...
    if len <= 4 then "ApiToken(***)"
    else
      val prefix = rawValue.take(4)
      s"ApiToken($prefix***)"

object ApiToken:
  /** Create an ApiToken from a string, returning None if empty or
    * whitespace-only
    */
