...
object GitHubHookDoctor:
  ...
  /** Check if gh CLI is installed.
    ...
    *   Function to check command availability (injected for testability)
    * @return
    *   CheckResult indicating gh CLI installation status
    */
  def checkGhInstalledWith(
    ...
    else if isCommandAvailable("gh") then CheckResult.Success("Installed")
    else CheckResult.Error("Not found", "Install: https://cli.github.com/")
  ...
  /** Check if gh CLI is authenticated.
    ...
    *   Function to check command availability (injected for testability)
    * @param execCommand
    *   Function to execute commands (injected for testability)
    * @return
    *   CheckResult indicating gh CLI authentication status
    */
  def checkGhAuthenticatedWith(
    ...
    if config.trackerType != IssueTrackerType.GitHub then
      CheckResult.Skip("Not using GitHub")
    else if !isCommandAvailable("gh") then CheckResult.Skip("gh not installed")
    else
      // Use validateGhPrerequisites from Phase 4 to check auth
      val repository = config.repository.getOrElse("")
      GitHubClient.validateGhPrerequisites(
        repository,
        isCommandAvailable,
        execCommand
      ) match
