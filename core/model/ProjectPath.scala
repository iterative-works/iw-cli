...
object ProjectPath:
  /** Derive main project path from a worktree path by stripping the issue ID
    ...
    * issueId matches: [A-Z]+-[0-9]+ or just [0-9]+
    *
    * @param worktreePath
  ...
  def deriveMainProjectPath(worktreePath: String): Option[String] =
    ...
    issueIdPattern.findFirstIn(dirName) match
      case Some(suffix) =>
        ...
        val parentPath =
          worktreePath.stripSuffix("/" + dirName).stripSuffix(dirName)
        if parentPath.isEmpty then Some(mainProjectName)
        else Some(s"$parentPath/$mainProjectName")
      case None =>
