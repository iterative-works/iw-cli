...
/** Snapshot of worktree state for pre-commit validation.
  *
  * @param stagedFiles
  *   Files in the staging area (ready to commit)
  * @param unstagedFiles
...
case class StagingCheck(
  ...
  /** Validate that the worktree is ready for phase-commit.
    *
    * Requires: staged changes exist, no unstaged modifications, no untracked
    * files. Returns Right(()) if ready, Left with a descriptive error message
    * if not.
    */
...
object StagingCheck:
  ...
  /** Parse `git status --porcelain` output into a StagingCheck.
    *
    * Porcelain format: two-character status prefix, a space, then the file
    * path. First character = staged status, second character = unstaged status.
    * `??` = untracked, `!!` = ignored.
    */
  def fromPorcelain(output: String): StagingCheck =
    ...
    lines.foreach { line =>
      if line.length >= 3 then
        ...
        if x == '?' && y == '?' then untracked += file
        else if x == '!' && y == '!' then () // ignored, skip
        else
