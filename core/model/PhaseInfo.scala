...
/** Information about a single phase in the agile workflow.
  ...
  * @param taskFilePath
  *   Full path to the phase task file (phase-NN-tasks.md)
  * @param totalTasks
...
case class PhaseInfo(
  ...
  /** True if all tasks in this phase are done. Empty phases (0 tasks) are not
    * considered complete.
    */
  def isComplete: Boolean = totalTasks > 0 && completedTasks == totalTasks

  /** True if phase has some tasks done but not all. Requires at least one task
    * to be started.
    */
  def isInProgress: Boolean =
    totalTasks > 0 && completedTasks > 0 && completedTasks < totalTasks

  /** True if phase has tasks but none are done yet. Empty phases (0 tasks) are
    * not considered "not started".
    */
  def notStarted: Boolean = totalTasks > 0 && completedTasks == 0

  /** Calculate completion percentage (0-100). Returns 0 for empty phases.
