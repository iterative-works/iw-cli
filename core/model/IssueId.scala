...
object IssueId:
  ...
  def parse(
      raw: String,
      defaultTeam: Option[String] = None
    ...
    normalized match
      ...
      case _         =>
        ...
        defaultTeam
          ...
          .getOrElse {
            Left(
              s"Invalid issue ID format: $raw (expected: TEAM-123). For GitHub/GitLab projects, configure team prefix with 'iw init'."
            )
          }
  ...
  def fromBranch(branchName: String): Either[String, IssueId] =
    ...
    normalized match
      ...
      case _                      =>
        Left(
          s"Cannot extract issue ID from branch '$branchName' (expected: TEAM-123 or TEAM-123-description). Configure team prefix with 'iw init' for GitHub/GitLab projects."
        )
