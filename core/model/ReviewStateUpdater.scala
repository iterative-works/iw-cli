...
object ReviewStateUpdater:
  ...
  case class UpdateInput(
      ...
      artifacts: Option[List[(String, String, Option[String])]] =
        None, // (label, path, category)
      ...
      actions: Option[List[(String, String, String)]] =
        None, // (id, label, skill)
  ...
  def merge(existingJson: String, update: UpdateInput): String =
    ...
    if update.clearStatus && update.status.isEmpty then
      existing.obj.remove("status")

    update.message.foreach { v =>
      if update.clearMessage then existing.obj.remove("message")
      else existing("message") = ujson.Str(v)
    }
    if update.clearMessage && update.message.isEmpty then
      existing.obj.remove("message")
    ...
    if update.clearNeedsAttention && update.needsAttention.isEmpty then
      existing.obj.remove("needs_attention")

    update.prUrl.foreach { v =>
      if update.clearPrUrl then existing.obj.remove("pr_url")
      else existing("pr_url") = ujson.Str(v)
    }
    if update.clearPrUrl && update.prUrl.isEmpty then
      existing.obj.remove("pr_url")

    update.activity.foreach { v =>
      if update.clearActivity then existing.obj.remove("activity")
      else existing("activity") = ujson.Str(v)
    }
    if update.clearActivity && update.activity.isEmpty then
      existing.obj.remove("activity")
    ...
    if update.clearWorkflowType && update.workflowType.isEmpty then
      existing.obj.remove("workflow_type")

    update.gitSha.foreach(v => existing("git_sha") = ujson.Str(v))

    // Update display object (merge individual properties)
    if update.clearDisplay then existing.obj.remove("display")
    else if update.displayText.isDefined || update.displaySubtext.isDefined || update.displayType.isDefined || update.clearDisplaySubtext
    then
      ...
      if update.clearDisplaySubtext then displayObj.obj.remove("subtext")
      else
    ...
    update.artifacts.foreach { values =>
      mergeArray(existing, "artifacts", values, update.artifactsMode) {
        case (label, path, category) =>
          val obj =
    ...
    update.badges.foreach { values =>
      mergeArray(existing, "badges", values, update.badgesMode) {
        case (label, badgeType) =>
          ujson.Obj("label" -> ujson.Str(label), "type" -> ujson.Str(badgeType))
      }
    }

    update.taskLists.foreach { values =>
      mergeArray(existing, "task_lists", values, update.taskListsMode) {
        case (label, path) =>
          ujson.Obj("label" -> ujson.Str(label), "path" -> ujson.Str(path))
      }
    }
    ...
    update.actions.foreach { values =>
      mergeArray(existing, "available_actions", values, update.actionsMode) {
        case (id, label, skill) =>
          ujson.Obj(
            "id" -> ujson.Str(id),
            "label" -> ujson.Str(label),
            "skill" -> ujson.Str(skill)
          )
      }
    }
    ...
    update.phaseCheckpoints.foreach { values =>
      update.phaseCheckpointsMode match
        ...
        case ArrayMergeMode.Replace =>
          ...
          else existing.obj.remove("phase_checkpoints")
