...
/** Task count result from parsing markdown.
  *
  * @param total
  *   Total number of checkbox tasks found
  * @param completed
  *   Number of completed checkbox tasks (marked with [x])
  */
...
object MarkdownTaskParser:
  ...
  /** Parse markdown lines and count checkbox tasks.
    ...
    *
    * @param lines
    *   Lines from markdown file
    * @return
    *   TaskCount with total and completed task counts
    */
  def parseTasks(lines: Seq[String]): TaskCount =
    ...
    val (total, completed) = tasks.foldLeft((0, 0)) {
      case ((t, c), (taskTotal, taskCompleted)) =>
  ...
  /** Extract phase name from markdown header.
    ...
    *
    * If multiple matching headers exist, returns the first one. Returns None if
  ...
  def extractPhaseName(lines: Seq[String]): Option[String] =
    ...
    lines.collectFirst { case phaseHeaderPattern(name) =>
      name.trim
    }
  ...
  /** Parse Phase Index from tasks.md to determine phase completion status.
    ...
    *
    * @param lines
    *   Lines from tasks.md file
    * @return
    *   List of PhaseIndexEntry in order found
    */
  def parsePhaseIndex(lines: Seq[String]): List[PhaseIndexEntry] =
    ...
    val phaseIndexPattern =
      "^\\s*-\\s+\\[([ xX])\\]\\s+Phase\\s+(\\d+):\\s*([^(→]+)".r
