...
object PhaseTaskFile:
  ...
  /** Update the Phase Status line to "Complete".
    *
    * Finds the line matching `**Phase Status:**` (bold format) and replaces the
    * value with "Complete". If no such line exists, appends
    * `**Phase Status:** Complete` at the end.
    *
    * @param content
    *   Full markdown file content
    * @return
    *   Updated content with Phase Status set to Complete
    */
  def markComplete(content: String): String =
    val lines = content.split("\n", -1)
    if lines.exists(PhaseStatusPattern.matches) then
      lines
        .map { line =>
  ...
  /** Find all unchecked [impl] task lines.
    ...
    *   Full markdown file content
    * @return
    *   List of lines with unchecked [impl] tasks
    */
  ...
  /** Mark all checked tasks as [reviewed] where a [reviewed] marker exists.
    *
    * For lines matching `- [x] [tag] [ ] [reviewed]`, changes `[ ] [reviewed]`
    * to `[x] [reviewed]`.
    *
    * Only touches lines that already have a `[reviewed]` marker. Lines without
    * `[reviewed]` are left unchanged. Lines where the primary checkbox is
    * unchecked `- [ ]` are left unchanged.
    *
    * @param content
    *   Full markdown file content
    * @return
    *   Updated content with reviewed checkboxes marked
    */
