...
object ContributingChecks:
  ...
  /** Check if CONTRIBUTING.md exists in project root.
    ...
    * @return
    *   CheckResult indicating CONTRIBUTING.md presence
    */
  def checkFileExistsWith(
    ...
    if fileExists(contributingMd) then CheckResult.Success("Found")
    else CheckResult.Warning("Missing")
  ...
  /** Check if CONTRIBUTING.md covers key topics using case-insensitive keyword
    * matching.
    *
    * Topics checked:
    *   - CI: continuous integration, pipeline, workflow, github actions, gitlab
    *     ci
    ...
    *   Project configuration
    * @param readFile
    *   Function to read file contents (injected for testability)
    * @return
    *   CheckResult indicating section coverage status
    */
  def checkSectionsCoveredWith(
    ...
    readFile(contributingMd) match
      case None =>
        CheckResult.Error(
          "Cannot read file",
          "Ensure CONTRIBUTING.md exists and is readable"
        )
      case Some(content) =>
        ...
        val localKeywords =
          Seq("local", "locally", "running checks", "run checks")
        ...
        if missing.isEmpty then CheckResult.Success("Complete")
        else
          val missingStr = missing.mkString(", ")
          CheckResult.WarningWithHint(
            s"Missing: $missingStr",
            s"Add sections covering: $missingStr"
          )
