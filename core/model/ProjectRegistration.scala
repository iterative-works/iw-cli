...
object ProjectRegistration:
  def create(
    ...
    if path.trim.isEmpty then Left("Path cannot be empty")
    else if projectName.trim.isEmpty then Left("Project name cannot be empty")
    else if trackerType.trim.isEmpty then Left("Tracker type cannot be empty")
    else if team.trim.isEmpty then Left("Team cannot be empty")
    else
      Right(
        ProjectRegistration(
          path,
