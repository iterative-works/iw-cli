...
/** Cached review state with file modification timestamps.
  ...
  * @param filesMtime
  *   Map of file path to modification time (epoch millis)
  */
...
object CachedReviewState:
  /** Check if cached review state is still valid based on file modification
    * times.
    *
    * Cache is valid if:
    ...
    *
    * @param cached
    *   Cached review state with stored mtimes
    * @param currentMtimes
    *   Current file modification times
    * @return
    *   True if cache is still valid, false if needs re-parsing
    */
  def isValid(
      cached: CachedReviewState,
      currentMtimes: Map[String, Long]
  ): Boolean =
    // Check if sets of files are the same
    val cachedFiles = cached.filesMtime.keySet
    val currentFiles = currentMtimes.keySet

    if cachedFiles != currentFiles then false // File added or removed
    else
