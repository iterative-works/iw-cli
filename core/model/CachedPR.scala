...
/** Cached pull request data with TTL validation.
  *
  * @param pr
  *   Pull request data from GitHub/GitLab
  * @param fetchedAt
  *   Timestamp when PR data was fetched
  * @param ttlMinutes
  *   Time-to-live in minutes (default: 2)
  */
case class CachedPR(
    pr: PullRequestData,
    fetchedAt: Instant,
...
object CachedPR:
  /** TTL for PR cache in minutes. PRs change more frequently than issues, so
    * use shorter TTL (2 vs 5 minutes).
    * @deprecated
    *   Use DEFAULT_PR_CACHE_TTL_MINUTES or instance ttlMinutes instead
    */
  val TTL_MINUTES = DEFAULT_PR_CACHE_TTL_MINUTES

  /** Check if cached PR is still valid based on TTL.
    *
    * Cache is valid when: age < ttlMinutes Cache is invalid when: age >=
    * ttlMinutes
    *
    * @param cached
    *   Cached PR to validate
    * @param now
  ...
  /** Check if cached PR is stale (age >= TTL).
    *
    * Stale data can still be displayed with an indicator. This is different
    * from isValid() - stale data should show a visual indicator.
    *
    * @param cached
    *   Cached PR to check
    * @param now
    *   Current timestamp for age calculation
    * @return
    *   true if cache is stale (age >= TTL), false otherwise
    */
