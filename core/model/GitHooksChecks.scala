...
object GitHooksChecks:
  ...
  /** Check if .git-hooks/ directory exists in project root.
    ...
    * @param dirExists
    *   Function to check directory existence (injected for testability)
    * @return
    *   CheckResult indicating .git-hooks/ presence
    */
  def checkHooksDirExistsWith(
    ...
    if dirExists(hooksDir) then CheckResult.Success("Found")
    else
      CheckResult.Error(
        "Missing",
        "Create .git-hooks/ directory in project root"
      )
  ...
  /** Check if pre-commit and pre-push hook files exist in .git-hooks/.
    ...
    *   Project configuration
    * @param fileExists
    *   Function to check file existence (injected for testability)
    * @return
    *   CheckResult indicating hook files presence
    */
  def checkHookFilesExistWith(
    ...
    if missing.isEmpty then CheckResult.Success("Found")
    else
      CheckResult.Error(
        s"Missing: ${missing.mkString(", ")}",
        "Create missing hook files in .git-hooks/"
      )
  ...
  /** Check if hooks are installed (symlinked or via core.hooksPath).
    ...
    *   Project configuration
    * @param getGitDir
    *   Function to get git directory path (injected for testability)
    * @param getHooksPath
    *   Function to get core.hooksPath config (injected for testability)
    * @param readSymlink
    *   Function to read symlink target (injected for testability)
    * @return
    *   CheckResult indicating hook installation status
    */
      ...
      case _ =>
        ...
        getGitDir() match
          ...
          case Some(gitDir) =>
            ...
            val expectedTargets =
              hookNames.map(name => os.pwd / ".git-hooks" / name)
            val installedHooks = hookNames.zip(expectedTargets).map {
              case (name, expectedTarget) =>
  ...
  def checkHooksInstalled(config: ProjectConfiguration): CheckResult =
    val getGitDir = () =>
      try
        ...
        val gitDir =
          if gitDirStr.startsWith("/") then os.Path(gitDirStr)
          else os.pwd / os.RelPath(gitDirStr)
        Some(gitDir)
    ...
    val getHooksPath = () =>
      try
        val result =
          os.proc("git", "config", "core.hooksPath").call(cwd = os.pwd)
