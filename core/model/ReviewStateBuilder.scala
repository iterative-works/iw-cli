...
object ReviewStateBuilder:
  ...
  case class BuildInput(
      ...
      issueId: String,
      lastUpdated: String,
      artifacts: List[(String, String, Option[String])] =
        Nil, // (label, path, category)
  ...
  def build(input: BuildInput): String =
    val obj = ujson.Obj(
      ...
      "artifacts" -> ujson.Arr(input.artifacts.map {
    ...
    if input.actions.nonEmpty then
      obj("available_actions") =
        ujson.Arr(input.actions.map { case (id, label, skill) =>
          ujson.Obj(
            "id" -> ujson.Str(id),
