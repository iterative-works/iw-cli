...
object FeedbackParser:
  ...
  object IssueType:
    /** Parses a string into an IssueType enum value.
      *
      * Case-insensitive matching: "bug" or "BUG" → Bug, "feature" or "Feature"
      * → Feature.
      *
      * @param s
      *   Input string to parse
      * @return
      *   Right(IssueType) for valid input, Left(error message) for invalid
      *   input
      */
  ...
  /** Structured feedback request from parsed command arguments.
    *
    * @param title
    *   Issue title (non-empty, max 500 chars)
    * @param description
    *   Issue description (empty string if not provided, max 10000 chars)
    * @param issueType
    *   Type of issue (Bug or Feature)
    */
  ...
  /** Returns the Linear label ID for a given issue type.
    *
    * @param issueType
    *   The issue type (Bug or Feature)
    * @return
    *   Linear label UUID string
    */
  ...
  /** Parses feedback command arguments into a structured request.
    ...
    * @return
    *   Right(FeedbackRequest) on success, Left(error message) on validation
    *   failure
    */
  def parseFeedbackArgs(args: Seq[String]): Either[String, FeedbackRequest] =
    ...
    if titleParts.isEmpty then Left("Title is required")
    else
      ...
      if title.length > MaxTitleLength then
        Left(
          s"Title must be $MaxTitleLength characters or less (got ${title.length})"
        )
      else
        // Parse flags
        val flagArgs = args.dropWhile(!_.startsWith("--"))
        val description =
          extractFlagValue(flagArgs, "--description").getOrElse("")

        // Validate description length
        if description.length > MaxDescriptionLength then
          Left(
            s"Description must be $MaxDescriptionLength characters or less (got ${description.length})"
          )
        else
          val issueTypeStr =
            extractFlagValue(flagArgs, "--type").getOrElse("feature")

          // Validate issue type
          IssueType.fromString(issueTypeStr) match
            case Left(error)      => Left(error)
            case Right(issueType) =>
              Right(FeedbackRequest(title, description, issueType))

  private def extractFlagValue(
      args: Seq[String],
      flag: String
  ): Option[String] =
