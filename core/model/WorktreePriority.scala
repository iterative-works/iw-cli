...
object WorktreePriority:
  /** Calculate priority score for a worktree based on last activity timestamp.
    ...
    *
    * The score is calculated as the negative number of seconds since last
    * activity, so that sorting in descending order puts most recent first.
    *
    * @param registration
    *   Worktree registration with activity timestamp
    * @param now
    *   Current timestamp for comparison
    * @return
    *   Priority score (higher = more recent activity = higher priority)
    */
  def priorityScore(registration: WorktreeRegistration, now: Instant): Long =
    val secondsSinceActivity =
      Duration.between(registration.lastSeenAt, now).getSeconds
