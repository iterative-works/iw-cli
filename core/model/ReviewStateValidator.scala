...
/** Validates review-state.json content against the schema contract.
  *
  * All validation is pure (no I/O). The validator checks JSON syntax, required
  * fields, field types, nested object structure, and additional property
  * restrictions.
  */
object ReviewStateValidator:
  ...
  private val ValidWorkflowTypes: Set[String] =
    Set("agile", "waterfall", "diagnostic")
  ...
  private def validateFieldTypes(
      ...
      v.numOpt match
        case Some(n) =>
          if n != n.toInt || n.toInt < 1 then
            errors += ValidationError(
              "version",
              "Field 'version' must be an integer >= 1"
            )
        case None =>
          errors += ValidationError(
            "version",
    ...
    obj.get("issue_id").foreach { v =>
      if !v.strOpt.isDefined then
        errors += ValidationError(
          "issue_id",
          "Field 'issue_id' must be a string"
        )
    }
    ...
    obj.get("last_updated").foreach { v =>
      if !v.strOpt.isDefined then
        errors += ValidationError(
          "last_updated",
          "Field 'last_updated' must be a string"
        )
    }
  ...
  private def validateArtifact(
    ...
    if !item.objOpt.isDefined then
      errors += ValidationError(
        s"artifacts[$index]",
        "Each artifact must be a JSON object"
      )
    else
      ...
      if !artifactObj.contains("label") then
        errors += ValidationError(
          s"artifacts[$index].label",
          "Required field 'label' is missing in artifact"
        )
      else if !artifactObj("label").strOpt.isDefined then
        errors += ValidationError(
          s"artifacts[$index].label",
          "Field 'label' must be a string in artifact"
        )

      if !artifactObj.contains("path") then
        errors += ValidationError(
          s"artifacts[$index].path",
          "Required field 'path' is missing in artifact"
        )
      else if !artifactObj("path").strOpt.isDefined then
        errors += ValidationError(
          s"artifacts[$index].path",
          "Field 'path' must be a string in artifact"
        )
      ...
      artifactObj.get("category").foreach { v =>
        if !v.strOpt.isDefined then
          errors += ValidationError(
            s"artifacts[$index].category",
            "Field 'category' must be a string in artifact"
          )
      }
      ...
      artifactObj.keys.foreach { key =>
        if !allowedArtifactProps.contains(key) then
          errors += ValidationError(
            s"artifacts[$index]",
            s"Unknown property '$key' in artifact"
          )
      }
  ...
  private def validateOptionalFieldTypes(
    ...
    obj.get("display").foreach { v =>
      if !v.objOpt.isDefined then
        errors += ValidationError(
          "display",
          "Field 'display' must be an object"
        )
      else validateDisplay(v.obj, errors)
    }
    ...
    obj.get("task_lists").foreach { v =>
      if !v.arrOpt.isDefined then
        errors += ValidationError(
          "task_lists",
          "Field 'task_lists' must be an array"
        )
      else
        val arr = v.arr
        arr.zipWithIndex.foreach { case (item, idx) =>
    ...
    obj.get("needs_attention").foreach { v =>
      if !v.boolOpt.isDefined then
        errors += ValidationError(
          "needs_attention",
          "Field 'needs_attention' must be a boolean"
        )
    }
    ...
    obj.get("pr_url").foreach { v =>
      ...
      if !isStr && !isNull then
        errors += ValidationError(
          "pr_url",
          "Field 'pr_url' must be a string or null"
        )
    }
    ...
    obj.get("activity").foreach { v =>
      if !v.strOpt.isDefined then
        errors += ValidationError(
          "activity",
          "Field 'activity' must be a string"
        )
      else
        ...
        if !ValidActivityValues.contains(activityValue) then
          errors += ValidationError(
            "activity",
            s"Field 'activity' must be one of: ${ValidActivityValues.toSeq.sorted.mkString(", ")}"
          )
    }
    ...
    obj.get("workflow_type").foreach { v =>
      if !v.strOpt.isDefined then
        errors += ValidationError(
          "workflow_type",
          "Field 'workflow_type' must be a string"
        )
      else
        ...
        if !ValidWorkflowTypes.contains(workflowTypeValue) then
          errors += ValidationError(
            "workflow_type",
            s"Field 'workflow_type' must be one of: ${ValidWorkflowTypes.toSeq.sorted.mkString(", ")}"
          )
    }
    ...
    obj.get("phase_checkpoints").foreach { v =>
      if !v.objOpt.isDefined then
        errors += ValidationError(
          "phase_checkpoints",
          "Field 'phase_checkpoints' must be an object"
        )
      else
  ...
  private def validatePhaseCheckpoint(
    ...
    if !item.objOpt.isDefined then
      errors += ValidationError(
        s"phase_checkpoints.$key",
        "Each phase checkpoint must be a JSON object"
      )
    else
      val cpObj = item.obj
      if !cpObj.contains("context_sha") then
        errors += ValidationError(
          s"phase_checkpoints.$key.context_sha",
          "Required field 'context_sha' is missing in phase checkpoint"
        )
      else if !cpObj("context_sha").strOpt.isDefined then
        errors += ValidationError(
          s"phase_checkpoints.$key.context_sha",
          "Field 'context_sha' must be a string in phase checkpoint"
        )
      ...
      cpObj.keys.foreach { prop =>
        if !allowedCheckpointProps.contains(prop) then
          errors += ValidationError(
            s"phase_checkpoints.$key",
            s"Unknown property '$prop' in phase checkpoint"
          )
      }
    ...
    if !item.objOpt.isDefined then
      ...
      requiredActionFields.foreach { field =>
        if !actionObj.contains(field) then
          errors += ValidationError(
            s"available_actions[$index].$field",
            s"Required field '$field' is missing in action"
          )
        else if !actionObj(field).strOpt.isDefined then
          errors += ValidationError(
            s"available_actions[$index].$field",
            s"Field '$field' must be a string in action"
          )
      }
      ...
      actionObj.keys.foreach { key =>
        if !allowedActionProps.contains(key) then
          errors += ValidationError(
            s"available_actions[$index]",
            s"Unknown property '$key' in action"
          )
      }
  ...
  private def validateDisplay(
    ...
    if !obj.contains("text") then
      errors += ValidationError(
        "display.text",
        "Required field 'text' is missing in display"
      )
    else if !obj("text").strOpt.isDefined then
      errors += ValidationError(
        "display.text",
        "Field 'text' must be a string in display"
      )

    if !obj.contains("type") then
      errors += ValidationError(
        "display.type",
        "Required field 'type' is missing in display"
      )
    else if !obj("type").strOpt.isDefined then
      errors += ValidationError(
        "display.type",
        "Field 'type' must be a string in display"
      )
    else
      ...
      if !ValidDisplayTypes.contains(typeValue) then
        errors += ValidationError(
          "display.type",
          s"Field 'type' must be one of: ${ValidDisplayTypes.mkString(", ")}"
        )
    ...
    obj.get("subtext").foreach { v =>
      if !v.strOpt.isDefined then
        errors += ValidationError(
          "display.subtext",
          "Field 'subtext' must be a string in display"
        )
    }
    ...
    obj.keys.foreach { key =>
      if !allowedDisplayProps.contains(key) then
        errors += ValidationError(
          "display",
          s"Unknown property '$key' in display"
        )
    }
  ...
  private def validateBadge(
    ...
    if !item.objOpt.isDefined then
      errors += ValidationError(
        s"badges[$index]",
        "Each badge must be a JSON object"
      )
    else
      ...
      if !badgeObj.contains("label") then
        errors += ValidationError(
          s"badges[$index].label",
          "Required field 'label' is missing in badge"
        )
      else if !badgeObj("label").strOpt.isDefined then
        errors += ValidationError(
          s"badges[$index].label",
          "Field 'label' must be a string in badge"
        )

      if !badgeObj.contains("type") then
        errors += ValidationError(
          s"badges[$index].type",
          "Required field 'type' is missing in badge"
        )
      else if !badgeObj("type").strOpt.isDefined then
        errors += ValidationError(
          s"badges[$index].type",
          "Field 'type' must be a string in badge"
        )
      else
      ...
      badgeObj.keys.foreach { key =>
        if !allowedBadgeProps.contains(key) then
          errors += ValidationError(
            s"badges[$index]",
            s"Unknown property '$key' in badge"
          )
      }
  ...
  private def validateTaskList(
    ...
    if !item.objOpt.isDefined then
      errors += ValidationError(
        s"task_lists[$index]",
        "Each task list must be a JSON object"
      )
    else
      ...
      if !taskListObj.contains("label") then
        errors += ValidationError(
          s"task_lists[$index].label",
          "Required field 'label' is missing in task list"
        )
      else if !taskListObj("label").strOpt.isDefined then
        errors += ValidationError(
          s"task_lists[$index].label",
          "Field 'label' must be a string in task list"
        )

      if !taskListObj.contains("path") then
        errors += ValidationError(
          s"task_lists[$index].path",
          "Required field 'path' is missing in task list"
        )
      else if !taskListObj("path").strOpt.isDefined then
        errors += ValidationError(
          s"task_lists[$index].path",
          "Field 'path' must be a string in task list"
        )
      ...
      taskListObj.keys.foreach { key =>
        if !allowedTaskListProps.contains(key) then
          errors += ValidationError(
            s"task_lists[$index]",
            s"Unknown property '$key' in task list"
          )
      }
  ...
  private def validateAdditionalProperties(
    ...
    obj.keys.foreach { key =>
      if !AllowedRootProperties.contains(key) then
        errors += ValidationError(
          "root",
          s"Unknown property '$key' is not allowed"
        )
    }
