...
object WorktreeCreationService:
  /** Creates a worktree for the given issue ID.
    *
    * This is a pure function that orchestrates the creation workflow by
    * accepting all I/O operations as function parameters. This makes it easy to
    * test and keeps the business logic separated from side effects.
    *
    * @param issueId
    *   The issue identifier (e.g., "IW-79")
    * @param config
    *   Project configuration with tracker and project details
    * @param fetchIssue
    *   Function to fetch issue data from tracker API
    * @param createWorktree
    *   Function to create git worktree (path, branchName) => Either[error, ()]
    * @param createTmuxSession
    *   Function to create tmux session (sessionName, workDir) => Either[error,
    *   ()]
    * @param registerWorktree
    *   Function to register worktree in state (issueId, path, trackerType,
    *   team) => Either[error, ()]
    * @param checkDirectoryExists
    *   Function to check if directory already exists on disk
    * @param checkWorktreeExists
    *   Function to check if issue already has registered worktree
    * @return
    *   Either WorktreeCreationError or WorktreeCreationResult with paths and
    *   commands
    */
  def create(
      issueId: String,
      ...
      registerWorktree: (String, String, String, String) => Either[
        String,
        Unit
      ],
    ...
    for
      ...
      issueData <- fetchIssue(issueId).left.map(err =>
        WorktreeCreationError.IssueNotFound(issueId)
      )

      // Parse issue ID for branch naming
      parsedIssueId <- IssueId
        .parse(issueId, config.teamPrefix)
        .left
        .map(err => WorktreeCreationError.ApiError(err))
      ...
      _ <-
        if checkDirectoryExists(fullPath) then
      ...
      _ <- createWorktree(fullPath, branchName).left.map(err =>
        WorktreeCreationError.GitError(err)
      )

      // Create detached tmux session
      _ <- createTmuxSession(sessionName, fullPath).left.map(err =>
        WorktreeCreationError.TmuxError(err)
      )

      // Register worktree in state file
      trackerTypeStr = config.trackerType.toString
      team = parsedIssueId.team
      _ <- registerWorktree(issueId, fullPath, trackerTypeStr, team).left.map(
        err => WorktreeCreationError.ApiError(err)
      )
  ...
  /** Creates a worktree with lock protection to prevent concurrent creation.
    *
    * This wraps the create method with lock acquisition/release using
    * CreationLockRegistry. If the lock cannot be acquired, returns
    * CreationInProgress error immediately. The lock is always released after
    * creation completes (success or failure).
    *
    * @param issueId
    *   The issue identifier (e.g., "IW-79")
    * @param config
