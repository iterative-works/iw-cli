...
/** Service for fetching issues with caching support.
  *
  * All functions are pure - they receive current time and fetch function as
  * parameters (Functional Core / Imperative Shell pattern).
  */
object IssueCacheService:

  /** Get cached issue data without calling API.
    *
    * Returns cached data regardless of age (even if stale). Returns None if no
    * cache entry exists. Never calls fetch function - purely reads from cache.
    *
    * @param issueId
    *   Issue identifier to look up
    * @param cache
    *   Current issue cache map
    * @return
    *   Optional IssueData from cache (None if not cached)
    */
  def getCachedOnly(
      issueId: String,
  ...
  /** Fetch issue with cache support.
    ...
    *   5. If API fails and stale cache exists: return stale data with
    *      fromCache=true
    *   6. If API fails and no cache: return error
    *
    * @param issueId
    *   Issue identifier to fetch
    * @param cache
    *   Current issue cache map
    * @param now
    *   Current timestamp for TTL validation
    * @param fetchFn
    *   Function to fetch issue from API (injected for purity)
    * @param urlBuilder
    *   Function to build issue URL from ID (injected)
    * @return
    *   Either error message or (IssueData, fromCache) tuple
    */
  def fetchWithCache(
      issueId: String,
      cache: Map[String, CachedIssue],
  ...
  /** Build issue URL from tracker type and issue ID.
    *
    * @param issueId
    *   Issue identifier (e.g., "IWLE-123", "PROJ-456", "72")
    * @param trackerType
    *   Tracker type ("Linear", "YouTrack", or "GitHub")
    * @param configValue
    *   Optional config value (baseUrl for YouTrack, repository for GitHub)
    * @return
    *   Direct link to issue in tracker
    */
  def buildIssueUrl(
      issueId: String,
      trackerType: String,
    ...
    trackerType.toLowerCase match
      ...
      case "youtrack" =>
        ...
        configValue match
          ...
          case None      =>
            s"https://youtrack.example.com/issue/$issueId" // Fallback
      ...
      case "github" =>
        ...
        configValue match
          case Some(repository) =>
            s"https://github.com/$repository/issues/$issueNumber"
          case None =>
            s"https://github.com/unknown/repo/issues/$issueNumber" // Fallback

      case "gitlab" =>
        // GitLab URL format: https://gitlab.com/{group}/{project}/-/issues/{number}
        // configValue format: "repository" or "repository|baseUrl"
        val issueNumber = extractGitHubIssueNumber(
          issueId
        ) // Same number extraction logic
        configValue match
