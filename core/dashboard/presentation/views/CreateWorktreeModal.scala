...
object CreateWorktreeModal:
  /** Render create worktree modal with search input.
    ...
    *
    * @param projectPath
    *   Optional project path to scope the modal to a specific project
    * @return
    *   HTML fragment for modal
    */
        ...
        div(
          ...
          div(
            ...
            input(
              ...
              attr("hx-get") := projectPath.fold("/api/issues/search") {
                projPath =>
                  val encodedPath =
                    java.net.URLEncoder.encode(projPath, "UTF-8")
            ...
            div(
              id := "search-results",
              cls := "search-results",
              attr("hx-get") := projectPath.fold("/api/issues/recent") {
                projPath =>
                  val encodedPath =
                    java.net.URLEncoder.encode(projPath, "UTF-8")
