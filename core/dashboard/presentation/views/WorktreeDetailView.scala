...
object WorktreeDetailView:
  /** Render the full worktree detail page body content.
    ...
    * @param issueData
    *   Optional issue data with cache flag and stale flag; None = skeleton
    *   state
    * @param progress
    ...
    *   Optional review state (Left = error message, Right = valid state)
    * @param projectName
    *   Optional project name derived from worktree path
    * @param now
    *   Current timestamp for relative time formatting
    * @param sshHost
    *   SSH hostname for Zed editor links
    * @return
    *   Scalatags Frag for the page body content
    */
  def render(
  ...
  /** Render only the content section of the worktree detail page, without
    * breadcrumb or page shell.
    *
    * Used by the HTMX polling endpoint to return refreshed content that
    * replaces the inner HTML of the content wrapper div on the full page.
    *
    * @param worktree
    *   Worktree registration
    * @param issueData
    ...
    *   SSH hostname for Zed editor links
    * @return
    *   Scalatags Frag for the content section only
    */
  def renderContent(
      worktree: WorktreeRegistration,
      issueData: Option[(IssueData, Boolean, Boolean)],
  ...
  /** Render a not-found page for an unknown worktree issue ID.
    *
    * @param issueId
    *   The issue ID that was requested but not found
    * @return
    *   Scalatags Frag for the not-found page body content
    */
  ...
  private def renderFull(
    ...
    frag(
      ...
      div(
        ...
        if isStale then span(cls := "stale-indicator", " · stale")
        else ()
      ),
      // Git status section
      gitStatus.map { status =>
      ...
      progress.flatMap(p => p.currentPhaseInfo.map(info => (p, info))).map {
        case (prog, phaseInfo) =>
          div(
            cls := "phase-info",
            span(
      ...
      WorktreeCardRenderer.renderReviewArtifacts(
        worktree.issueId,
        reviewStateResult
      )
    )
