...
object CreationErrorView:
  /** Render error state after worktree creation failure.
    ...
    *
    * @param error
    *   User-friendly error with title, message, suggestion, retry flag, and
    *   issueId
    * @return
    *   HTML fragment for error state
    */
  ...
  /** Render action buttons (retry if applicable, and dismiss).
    ...
    * @param issueId
    *   Optional issue ID for retry request
    * @return
    *   HTML fragment for buttons
    */
  ...
  /** Render retry button that re-posts the creation request.
    *
    * The retry button includes the issue ID in hx-vals so the server can
    * process the retry request correctly.
    *
    * @param issueId
    *   Optional issue ID to include in retry request
    * @return
    *   HTML fragment for retry button
    */
  ...
  /** Render dismiss button that closes the modal.
    *
    * @return
    *   HTML fragment for dismiss button
    */
