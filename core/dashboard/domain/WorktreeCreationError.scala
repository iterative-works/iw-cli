...
object WorktreeCreationError:
  /** Map domain error to user-friendly error with actionable message.
    *
    * Error messages are kept generic to avoid information disclosure. Full
    * technical details are logged server-side.
    *
    * @param error
    *   Domain error from worktree creation
    * @param issueId
    *   The issue ID being processed (for retry functionality)
    * @return
    *   User-friendly error with title, message, suggestion, and retry flag
    */
  def toUserFriendly(
    ...
    error match
      case DirectoryExists(path) =>
        ...
        UserFriendlyError(
          ...
          suggestion = Some(
            s"Remove the existing directory or use './iw open $issueId' to register it"
          ),
          canRetry = false,
      ...
      case AlreadyHasWorktree(existingIssueId, existingPath) =>
        val sessionName =
          existingPath.split("/").lastOption.getOrElse(existingIssueId)
        UserFriendlyError(
          title = "Worktree Already Exists",
          message = s"Issue $existingIssueId already has a worktree.",
          suggestion = Some(
            s"Use 'tmux attach -t $sessionName' to open the existing worktree"
          ),
          canRetry = false,
          issueId = Some(issueId)
        )
      ...
      case GitError(_) =>
        ...
        UserFriendlyError(
          ...
          suggestion =
            Some("If the problem persists, check your git configuration"),
          canRetry = true,
          issueId = Some(issueId)
        )
      ...
      case TmuxError(_) =>
        ...
        UserFriendlyError(
          ...
          suggestion =
            Some("If the problem persists, check that tmux is running"),
          canRetry = true,
          issueId = Some(issueId)
        )
      ...
      case IssueNotFound(notFoundId) =>
        UserFriendlyError(
          ...
          suggestion =
            Some("Verify the issue ID is correct and exists in your tracker"),
          canRetry = false,
          issueId = Some(issueId)
        )
      ...
      case CreationInProgress(_) =>
        UserFriendlyError(
          ...
          suggestion =
            Some("Please wait for the current creation to complete."),
          canRetry = true,
          issueId = Some(issueId)
        )
