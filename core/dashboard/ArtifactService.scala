...
object ArtifactService:
  /** Load and render artifact content.
    ...
    * @param state
    *   Current server state (for worktree lookup)
    * @param readFile
    *   Function to read file content (injected I/O)
    * @param resolveSymlinks
    *   Function to resolve symlinks (injected I/O, for testing)
    * @return
    *   Either error message or (artifact label, rendered HTML, worktree path)
    */
  def loadArtifact(
      issueId: String,
      artifactPath: String,
  ...
  /** Extract human-readable label from file path.
    *
    * Examples: analysis.md → "analysis.md" phase-03-context.md →
    * "phase-03-context.md" project-management/issues/46/review.md → "review.md"
    */
