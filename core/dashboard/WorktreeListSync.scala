...
object WorktreeListSync:
  /** Result of comparing two worktree lists.
    *
    * @param additions
    *   Issue IDs that are new in the second list
    * @param deletions
    *   Issue IDs that were removed from the first list
    * @param reorders
    *   Issue IDs that changed position (moved)
    */
  ...
  /** Find the ID that should precede the given ID in sorted order.
    ...
    *   ID being inserted
    * @param existingIds
    *   Current list of IDs (must be sorted)
    * @return
    *   ID that should precede newId, or None if newId should be first
    */
  def findPredecessor(
      newId: String,
  ...
  /** Detect changes between two ordered lists of issue IDs.
    ...
    *
    * @param oldIds
    *   Previous list of issue IDs (ordered)
    * @param newIds
    *   Current list of issue IDs (ordered)
    * @return
    *   ListChanges describing what changed
    */
  ...
  /** Generate HTMX OOB swap HTML for adding a new worktree card.
    ...
    *   - If predecessorId is None, inserts at beginning:
    *     `afterbegin:#worktree-list`
    *   - If predecessorId is Some(id), inserts after that card:
    *     `afterend:#card-{id}`
    *
    * @param registration
    *   Worktree registration
    * @param issueData
    *   Optional cached issue data
    * @param progress
    ...
    *   Current timestamp
    * @param sshHost
    *   SSH hostname for Zed links
    * @param predecessorId
    *   ID of card to insert after, or None for first position
    * @return
    *   HTML string with OOB swap attribute
    */
  ...
  /** Generate HTMX OOB swap HTML for deleting a worktree card.
    ...
    *
    * @param issueId
    *   Issue ID for the card to delete
    * @return
    *   HTML string with OOB delete attribute
    */
  ...
  /** Generate HTMX OOB swap HTML for reordering a worktree card.
    ...
    *   Optional review state
    * @param position
    *   New position (0 = top, 1 = second, etc.)
    * @param now
    *   Current timestamp
    * @param sshHost
    *   SSH hostname for Zed links
    * @return
    *   HTML string with delete + add OOB swaps
    */
  def generateReorderOob(
    ...
    val oobTarget =
      if position == 0 then "afterbegin:#worktree-list"
      else "beforeend:#worktree-list"
    val addHtml = div(
      attr("hx-swap-oob") := oobTarget,
      cardHtml
    ).render
  ...
  /** Generate complete HTMX response with all OOB swaps for detected changes.
    ...
    *
    * @param changes
    *   Detected list changes
    * @param registrations
    *   All worktree registrations (by issue ID)
    * @param issueCache
    *   Issue data cache
    * @param progressCache
    *   Progress data cache
    ...
    *   SSH hostname for Zed links
    * @param currentIds
    *   Current list of IDs in sorted order (excluding additions)
    * @return
    *   HTML string with all OOB swaps, or empty string
    */
