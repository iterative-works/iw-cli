...
object WorktreeListView:
  /** Render worktree list with issue data, progress, git status, PR data, and
    * review state.
    *
    ...
    *
    * @param worktreesWithData
    *   List of tuples (worktree, optional issue data with cache flag and stale
    *   flag, optional progress, optional git status, optional PR data, optional
    *   review state result)
    * @param now
    *   Current timestamp for relative time formatting
    * @param sshHost
  ...
  def render(
    ...
    if worktreesWithData.isEmpty then
      ...
      div(
        ...
        attr(
          "hx-vals"
        ) := "js:{have: [...document.querySelectorAll('#worktree-list > [id^=\"card-\"]')].map(e => e.id.replace('card-', '')).join(',')}",
        attr("hx-trigger") := "every 30s",
        attr("hx-swap") := "none",
        worktreesWithData.zipWithIndex.map {
          case (
  ...
  /** Calculate HTMX polling delay based on card position.
    ...
    *
    * @param position
    *   Card position in list (1-based)
    * @return
    *   Delay string for HTMX trigger (e.g., "500ms", "2s", "5s")
    */
  ...
  /** Format cache age as human-readable string.
    *
    * @param fetchedAt
    *   When issue data was fetched
    * @param now
    *   Current timestamp
    * @return
    *   Formatted string (e.g., "3m ago", "2h ago")
    */
  ...
  def formatRelativeTime(instant: Instant, now: Instant): String =
    ...
    if days > 0 then s"${days}d ago"
    else if hours > 0 then s"${hours}h ago"
    else if minutes > 0 then s"${minutes}m ago"
    else "just now"

  /** Map display type to CSS class for badge styling.
    *
    * @param displayType
    *   Display type from review-state.json (info, success, warning, error,
    *   progress)
    * @return
    *   CSS class name (e.g., "display-type-progress")
    */
