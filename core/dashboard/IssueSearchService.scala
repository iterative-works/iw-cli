...
object IssueSearchService:
  /** Search for issues by ID or text.
    *
    * Phase 2 implementation: Supports exact ID match (priority) and text search
    * fallback.
    ...
    *
    * @param query
    *   Search query (issue ID like "IW-79" or text like "fix bug")
    * @param config
    *   Project configuration with tracker type and settings
    * @param fetchIssue
    *   Function to fetch an issue by ID from the tracker
    * @param searchIssues
    *   Function to search issues by text query
    * @param checkWorktreeExists
    *   Function to check if issue already has a registered worktree
    * @return
    *   Either error message or list of search results
    */
  def search(
      query: String,
    ...
    if trimmedQuery.isEmpty then Right(List.empty)
    else
  ...
  /** Search issues by text query.
    ...
    * @param checkWorktreeExists
    *   Function to check if issue has worktree
    * @return
    *   Either error message or list of search results (open first, closed at
    *   end)
    */
  ...
  /** Fetch recent open issues for quick access.
    *
    * @param config
    *   Project configuration with tracker type
    * @param fetchRecentIssues
    *   Function to fetch recent issues from tracker
    * @param checkWorktreeExists
    *   Function to check if issue has worktree
    * @return
    *   Either error message or list of recent issues as search results
    */
  def fetchRecent(
      config: ProjectConfiguration,
