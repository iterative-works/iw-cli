...
object PathValidator:
  ...
  /** Validate that an artifact path is safe to serve.
    ...
    *   Function to resolve symlinks (injected for testing/I/O isolation)
    * @return
    *   Right(resolvedPath) if valid, Left(errorMessage) if invalid
    */
  def validateArtifactPath(
    ...
    if artifactPath.trim.isEmpty then Left("Invalid artifact path")
    // Reject absolute paths
    else if isAbsolute(artifactPath) then Left("Artifact path must be relative")
    else
  ...
  /** Check if a path is absolute. Handles both Unix (/path) and Windows
    * (C:\path) paths.
    */
  ...
  /** Check if the resolved path is within the base path boundary.
    *
    * @param basePath
    *   The boundary (worktree root)
    * @param targetPath
    *   The path to check (must be normalized)
    * @return
    *   true if targetPath is within basePath
    */
  ...
  /** Default symlink resolver that uses toRealPath to resolve symlinks. Returns
    * Left if the path doesn't exist or symlink resolution fails.
    *
    * @param path
    *   The path to resolve
    * @return
    *   Right(realPath) if successful, Left(errorMessage) if not
    */
