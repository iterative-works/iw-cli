...
object MarkdownRenderer:
  /** Convert markdown content to HTML.
    ...
    * @return
    *   Rendered HTML (fragment, not full page)
    */
  ...
  /** Transform Mermaid code blocks from flexmark format to Mermaid.js format.
    *
    * Converts: <pre><code class="language-mermaid">content</code></pre> To:
    * <div class="mermaid">content</div>
    *
    * HTML entities in the content are decoded (e.g., &gt; becomes >).
    *
    * @param html
    *   HTML output from flexmark
    * @return
    *   HTML with Mermaid blocks transformed
    */
  private def transformMermaidBlocks(html: String): String =
    // Use DOTALL flag to match across newlines
    val mermaidPattern =
      """(?s)<pre><code class="language-mermaid">(.*?)</code></pre>""".r

    mermaidPattern.replaceAllIn(
      html,
      m => {
        ...
        scala.util.matching.Regex
          .quoteReplacement(s"""<div class="mermaid">$decodedContent</div>""")
      }
    )
