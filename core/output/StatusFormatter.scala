...
object StatusFormatter:
  ...
  private def formatGitSection(status: WorktreeStatus): Option[String] =
    if status.branchName.isEmpty && status.gitClean.isEmpty then None
    else
      val lines = List(
        Some("Git"),
  ...
  private def formatIssueSection(status: WorktreeStatus): Option[String] =
    if status.issueTitle.isEmpty && status.issueStatus.isEmpty && status.issueUrl.isEmpty
    then None
    else
      val lines = List(
        Some("Issue"),
  ...
  private def formatPRSection(status: WorktreeStatus): Option[String] =
    if status.prState.isEmpty && status.prUrl.isEmpty && status.prNumber.isEmpty
    then None
    else
      val lines = List(
        Some("Pull Request"),
  ...
  private def formatReviewSection(status: WorktreeStatus): Option[String] =
    if status.reviewDisplay.isEmpty && status.reviewBadges.isEmpty && !status.needsAttention
    then None
    else
      val lines = List(
        Some("Review"),
  ...
  private def formatProgressSection(status: WorktreeStatus): Option[String] =
    if status.currentPhase.isEmpty && status.overallProgress.isEmpty then None
    else
      val lines = List(
        Some("Progress"),
        (status.currentPhase, status.totalPhases) match
          case (Some(current), Some(total)) =>
            Some(s"  Phase:      $current/$total")
          case _ => None
        ,
