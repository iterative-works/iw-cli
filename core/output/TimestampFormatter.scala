...
object TimestampFormatter:
  ...
  def formatUpdateTimestamp(timestamp: Instant, now: Instant): String =
    ...
    if seconds < 30 then "Updated just now"
    else if seconds < 60 then s"Updated $seconds seconds ago"
    else
      val minutes = seconds / 60
      if minutes < 2 then "Updated 1 minute ago"
      else if minutes < 60 then s"Updated $minutes minutes ago"
      else
        val hours = minutes / 60
        if hours < 2 then "Updated 1 hour ago"
        else s"Updated $hours hours ago"
