...
/** Structured issue creation request from parsed command arguments.
  *
  * @param title
  *   Issue title (non-empty, required)
  * @param description
...
object IssueCreateParser:
  ...
  /** Parses issue create command arguments into a structured request.
    *
    * Expects --title flag with value (required). Optional --description flag
    * with value.
    *
    * @param args
    *   Raw command line arguments
    * @return
    *   Right(IssueCreateRequest) on success, Left(error message) on validation
    *   failure
    */
  def parse(args: Seq[String]): Either[String, IssueCreateRequest] =
    // Extract --title value
    val title = extractFlagValue(args, "--title")
    if title.isEmpty then Left("--title flag is required")
    else
  ...
  /** Extracts the value of a command line flag.
    *
    * Supports multi-word values (joined with spaces). Returns None if flag is
    * not present. Returns Some("") if flag is present but has no value.
    *
    * @param args
    ...
    *   Flag name (e.g., "--title")
    * @return
    *   Some(value) if flag is present, None otherwise
    */
