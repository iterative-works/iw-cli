...
class GitWorktreeAdapterTest extends FunSuite:
  ...
  val tempDirRef =
    new java.util.concurrent.atomic.AtomicReference[Option[os.Path]](None)
  val repoDirRef =
    new java.util.concurrent.atomic.AtomicReference[Option[os.Path]](None)

  def tempDir: os.Path =
    tempDirRef.get.getOrElse(sys.error("tempDir not initialized"))
  def repoDir: os.Path =
    repoDirRef.get.getOrElse(sys.error("repoDir not initialized"))
  ...
  override def afterEach(context: AfterEach): Unit =
    ...
    tempDirRef.get.filter(os.exists).foreach { td =>
      ...
      val worktreePaths = result.stdout
        .split("\n")
  ...
  test(
    "GitWorktreeAdapter.worktreeExists returns false for non-existent worktree"
  ):
  ...
  test("GitWorktreeAdapter.createWorktree creates worktree with new branch"):
    ...
    assert(
      GitWorktreeAdapter.worktreeExists(worktreePath, repoDir),
      "Worktree should be registered"
    )
    assert(
      GitWorktreeAdapter.branchExists(branchName, repoDir),
      "Branch should exist"
    )
  ...
  test("GitWorktreeAdapter.createWorktree fails for duplicate branch"):
    ...
    val result = GitWorktreeAdapter.createWorktree(
      worktreePath,
      "duplicate-branch",
      repoDir
    )
    ...
    assert(
      result.isLeft,
      "Should fail to create worktree with duplicate branch name"
    )

  test(
    "GitWorktreeAdapter.createWorktreeForBranch creates worktree for existing branch"
  ):
    // Create a branch first using ProcessAdapter
    ...
    val result = GitWorktreeAdapter.createWorktreeForBranch(
      worktreePath,
      branchName,
      repoDir
    )
  ...
  test(
    "GitWorktreeAdapter.createWorktreeForBranch fails for non-existent branch"
  ):
    ...
    assert(
      result.isLeft,
      "Should fail to create worktree for non-existent branch"
    )
  ...
  test("GitWorktreeAdapter.removeWorktree succeeds for clean worktree"):
    ...
    // Remove worktree
    val result =
      GitWorktreeAdapter.removeWorktree(worktreePath, repoDir, force = false)
  ...
  test(
    "GitWorktreeAdapter.removeWorktree with force succeeds even with uncommitted changes"
  ):
    ...
    val result =
      GitWorktreeAdapter.removeWorktree(worktreePath, repoDir, force = true)
