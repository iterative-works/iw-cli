...
import iw.core.dashboard.PullRequestCacheService
...
class PullRequestCacheServiceTest extends FunSuite:
  ...
  test("fetchPR re-fetches when cache expired (>2 min)"):
    val now = Instant.now()
    val cachedPR =
      CachedPR(mockPRData, now.minusSeconds(180)) // 3 minutes ago (expired)
    val cache = Map("IWLE-123" -> cachedPR)

    val execCommand = (cmd: String, args: Array[String]) =>
      Right(
        """{"url": "https://github.com/org/repo/pull/99", "state": "OPEN", "number": 99, "title": "New"}"""
      )
  ...
  test("parseGitHubPR handles MERGED state"):
    val json =
      """{"url": "https://github.com/org/repo/pull/42", "state": "MERGED", "number": 42, "title": "Fix bug"}"""
    val result = PullRequestCacheService.parseGitHubPR(json)
  ...
  test("parseGitHubPR handles CLOSED state"):
    val json =
      """{"url": "https://github.com/org/repo/pull/42", "state": "CLOSED", "number": 42, "title": "Old PR"}"""
    val result = PullRequestCacheService.parseGitHubPR(json)
  ...
  test("parseGitLabPR handles merged state"):
    val json =
      """{"url": "https://gitlab.com/org/repo/-/merge_requests/42", "state": "merged", "iid": 42, "title": "Fix"}"""
    val result = PullRequestCacheService.parseGitLabPR(json)
  ...
  test("parseGitLabPR handles closed state"):
    val json =
      """{"url": "https://gitlab.com/org/repo/-/merge_requests/42", "state": "closed", "iid": 42, "title": "Old"}"""
    val result = PullRequestCacheService.parseGitLabPR(json)
  ...
  test("getCachedOnly returns stale cache without calling CLI"):
    val now = Instant.now()
    val stalePR =
      CachedPR(mockPRData, now.minusSeconds(600)) // 10 minutes ago (very stale)
  ...
  test("fetchPR preserves stale cache when CLI command fails"):
    ...
    assertEquals(
      prData.get.number,
      42,
      "Should return original cached data, not try to fetch"
    )
