...
class PhaseMergeTest extends FunSuite:
  ...
  test("CICheckResult can be constructed with name, status, and URL"):
    val check =
      CICheckResult("build", Failed, Some("https://ci.example.com/run/42"))
    assertEquals(check.name, "build")
  ...
  test("evaluateChecks single Passed returns AllPassed"):
    assertEquals(
      PhaseMerge.evaluateChecks(List(CICheckResult("build", Passed))),
      AllPassed
    )
  ...
  test(
    "evaluateChecks some Failed with others Passed returns SomeFailed carrying failed checks"
  ):
  ...
  test(
    "evaluateChecks mix of Failed and Cancelled returns SomeFailed with both"
  ):
    val failedCheck = CICheckResult("test", Failed)
  ...
  test("evaluateChecks single Pending returns StillRunning"):
    assertEquals(
      PhaseMerge.evaluateChecks(List(CICheckResult("build", Pending))),
      StillRunning
    )
  ...
  test("buildRecoveryPrompt empty list returns header with no check lines"):
    assertEquals(
      PhaseMerge.buildRecoveryPrompt(Nil),
      "The following CI checks failed:\n"
    )
  ...
  test("extractPrNumber extracts number from real GitHub org PR URL"):
    assertEquals(
      PhaseMerge.extractPrNumber(
        "https://github.com/iterative-works/iw-cli/pull/290"
      ),
      Right(290)
    )
  ...
  test("extractPrNumber extracts number from self-hosted GitLab MR URL"):
    assertEquals(
      PhaseMerge.extractPrNumber(
        "https://git.company.com/team/project/-/merge_requests/7"
      ),
      Right(7)
    )
