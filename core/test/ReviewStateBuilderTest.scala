...
class ReviewStateBuilderTest extends munit.FunSuite:
  ...
  test("build with all fields includes optional fields"):
    val input = ReviewStateBuilder.BuildInput(
      ...
      artifacts =
        List(("Analysis", "project-management/issues/IW-42/analysis.md", None)),
      status = Some("awaiting_review"),
      display = Some(("Awaiting Review", Some("Phase 3 of 5"), "warning")),
      badges = List(("TDD", "info")),
      taskLists =
        List(("Phase 3", "project-management/issues/IW-42/phase-03-tasks.md")),
      ...
      actions = List(
        ("implement", "Start Implementation", "iterative-works:ag-implement")
      ),
    ...
    assertEquals(
      parsed("task_lists")(0)("path").str,
      "project-management/issues/IW-42/phase-03-tasks.md"
    )
    ...
    assertEquals(
      parsed("available_actions")(0)("label").str,
      "Start Implementation"
    )
    assertEquals(
      parsed("available_actions")(0)("skill").str,
      "iterative-works:ag-implement"
    )
  ...
  test("build with artifact categories includes category field"):
    ...
    assert(
      !arts(2).obj.contains("category"),
      "artifact without category should not have category field"
    )
  ...
  test("built JSON passes ReviewStateValidator.validate()"):
    ...
    assert(
      result.isValid,
      s"Built JSON should be valid but got errors: ${result.errors}"
    )
  ...
  test(
    "build with both activity and workflowType produces valid JSON with both keys"
  ):
    val input = ReviewStateBuilder.BuildInput(
      issueId = "IW-1",
      lastUpdated = "2026-01-28T12:00:00Z",
  ...
  test("optional fields are omitted when not provided"):
    ...
    assert(
      !parsed.obj.contains("needs_attention"),
      "needs_attention should be omitted"
    )
    ...
    assert(
      !parsed.obj.contains("phase_checkpoints"),
      "phase_checkpoints should be omitted"
    )
    assert(
      !parsed.obj.contains("available_actions"),
      "available_actions should be omitted"
    )
    assert(!parsed.obj.contains("activity"), "activity should be omitted")
    assert(
      !parsed.obj.contains("workflow_type"),
      "workflow_type should be omitted"
    )
