...
class CIChecksTest extends FunSuite:
  ...
  test(
    "checkWorkflowExistsWith succeeds for GitLab when .gitlab-ci.yml exists"
  ):
    val config = testConfig(IssueTrackerType.GitLab)
    val fileExists = (path: os.Path) => path.toString.endsWith(".gitlab-ci.yml")
    val result = CIChecks.checkWorkflowExistsWith(config, fileExists)
  ...
  test(
    "checkWorkflowExistsWith succeeds for Linear when GitHub Actions exists"
  ):
  ...
  test(
    "checkWorkflowExistsWith succeeds for YouTrack when GitHub Actions exists"
  ):
