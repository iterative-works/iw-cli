...
class GitHubHookDoctorTest extends FunSuite:
  ...
  test("checkGhInstalled succeeds when gh CLI available"):
    ...
    val result =
      GitHubHookDoctor.checkGhInstalledWith(config, isCommandAvailable)
  ...
  test("checkGhAuthenticated skips when gh not installed"):
    ...
    val result = GitHubHookDoctor.checkGhAuthenticatedWith(
      config,
      isCommandAvailable,
      execCommand
    )
  ...
  test("checkGhAuthenticated succeeds when authenticated"):
    ...
    val execCommand =
      (_: String, _: Array[String]) => Right("Logged in to github.com")
    val result = GitHubHookDoctor.checkGhAuthenticatedWith(
      config,
      isCommandAvailable,
      execCommand
