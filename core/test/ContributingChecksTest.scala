...
class ContributingChecksTest extends FunSuite:
  ...
  test("checkSectionsCoveredWith warns when CI topic missing"):
    ...
    assertEquals(
      result,
      CheckResult.WarningWithHint("Missing: CI", "Add sections covering: CI")
    )
  ...
  test("checkSectionsCoveredWith warns when local checks topic missing"):
    ...
    assertEquals(
      ...
      CheckResult.WarningWithHint(
        ...
        "Add sections covering: local checks"
      )
    )
  ...
  test("checkSectionsCoveredWith warns when multiple topics missing"):
    ...
    assertEquals(
      result,
      CheckResult.WarningWithHint(
        "Missing: CI, local checks",
        "Add sections covering: CI, local checks"
      )
    )
  ...
  test("checkSectionsCoveredWith recognizes alternative CI keywords"):
    ...
    """.stripMargin)
    assertEquals(
      ContributingChecks.checkSectionsCoveredWith(config, readFile1),
    ...
    """.stripMargin)
    assertEquals(
      ContributingChecks.checkSectionsCoveredWith(config, readFile2),
      CheckResult.Success("Complete")
    )
