...
class ConfigFileTest extends munit.FunSuite, Fixtures:
  ...
  tempDir.test("ProjectConfiguration serializes to HOCON"): tempDir =>
  ...
  tempDir.test("ProjectConfiguration deserializes from HOCON"): tempDir =>
    val hocon = """
        tracker {
          type = linear
          team = IWLE
  ...
  tempDir.test("ProjectConfiguration roundtrip preserves data"): tempDir =>
  ...
  tempDir.test("ConfigSerializer handles YouTrack tracker type"): tempDir =>
    val hocon = """
        tracker {
          type = youtrack
          team = TEST
  ...
  test(
    "ConfigSerializer returns Left with meaningful message for invalid tracker type"
  ):
    val hocon = """
      tracker {
        type = invalid
  ...
  test(
    "ConfigSerializer returns Left with meaningful message for missing tracker type"
  ):
    val hocon = """
      tracker {
        team = TEST
  ...
  test(
    "ConfigSerializer succeeds with empty team when tracker team is missing"
  ):
    val hocon = """
      tracker {
        type = linear
  ...
  test(
    "ConfigSerializer returns Left with meaningful message for missing project name"
  ):
    val hocon = """
      tracker {
        type = linear
