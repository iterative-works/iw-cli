...
class MarkdownTaskParserTest extends FunSuite:
  ...
  test("parsePhaseIndex extracts completed phases"):
    ...
    assertEquals(
      index(0),
      PhaseIndexEntry(1, isComplete = true, "View basic dashboard")
    )
    assertEquals(
      index(1),
      PhaseIndexEntry(2, isComplete = true, "Automatic registration")
    )
    assertEquals(
      index(2),
      PhaseIndexEntry(3, isComplete = false, "Server lifecycle")
    )
