...
class StateReaderTest extends munit.FunSuite:
  ...
  fixture.test(
    "read valid state.json with all cache types populated returns populated ServerState"
  ): tempDir =>
    ...
    assert(
      result.isRight,
      s"Expected Right but got Left: ${result.left.getOrElse("")}"
    )
  ...
  fixture.test("read malformed JSON returns Left with error message"):
    tempDir =>
      ...
      assert(
        ...
        s"Error message should mention parsing failure: $error"
      )
  ...
  fixture.test("read state.json with projects populates state.projects map"):
    tempDir =>
      ...
      assertEquals(
        project.trackerUrl,
        Some("https://github.com/my-org/my-app/issues")
      )
