...
class ServerStateCodecTest extends FunSuite:
  ...
  test("roundtrip serialization of full StateJson with all caches populated"):
    ...
    val display =
      Display(text = "2 approvals", subtext = None, displayType = "success")
    val badge = Badge(label = "Approved", badgeType = "success")
    val taskList = TaskList(label = "Tasks", path = "/path/to/tasks.md")
    val reviewArtifact =
      ReviewArtifact(label = "Artifact", path = "/path/to/artifact")
    ...
    val stateJson = StateJson(
      ...
      progressCache = Map(
        "IWLE-123" -> CachedProgress(
          workflowProgress,
          Map("/path/to/tasks.md" -> 123456L)
        )
      ),
      prCache = Map("IWLE-123" -> CachedPR(prData, now, 60)),
      reviewStateCache = Map(
        "IWLE-123" -> CachedReviewState(
          reviewState,
          Map("/path/to/artifact" -> 123456L)
        )
      )
    )
  ...
  test(
    "backward compatibility - JSON with only worktrees key parses with all optional fields as empty"
  ):
    val json = """{"worktrees":{}}"""
    val parsed = read[StateJson](json)
  ...
  test(
    "ReviewState with activity and workflowType roundtrips through macroRW codec"
  ):
    val reviewArtifact =
      ReviewArtifact(label = "Artifact", path = "/path/to/artifact")
    val reviewState = ReviewState(
  ...
  test(
    "ReviewState with activity and workflowType absent roundtrips correctly"
  ):
    val reviewArtifact =
      ReviewArtifact(label = "Artifact", path = "/path/to/artifact")
    val reviewState = ReviewState(
