...
trait Fixtures:
  ...
  /** Git repository fixture with user config and initial commit.
    *
    * Creates a temp directory, initializes git, sets user config, and makes an
    * initial commit. The remote is set to a sample GitHub URL for tests that
    * need remote detection.
    */
    ...
    setup = { _ =>
      ...
      Process(
        Seq(
          ...
          "add",
          "origin",
          "https://github.com/iterative-works/test-repo.git"
        ),
        dir.toIO
      ).!
...
/** Companion object for Fixtures trait - provides factory methods for
  * standalone use.
  */
object Fixtures:
  /** Create a temporary directory and return its path. Caller responsible for
    * cleanup.
    */
  def createTempDir(prefix: String = "iw-test"): os.Path =
    os.Path(java.nio.file.Files.createTempDirectory(prefix))

  /** Create a git repo in a temporary directory. Caller responsible for
    * cleanup.
    */
...
/** Sample test data for consistent test objects.
  *
  * Provides canonical examples of domain objects with sensible defaults. Use
  * these to avoid creating similar objects in every test file.
  */
object SampleData:
  ...
  val githubRemote: GitRemote = GitRemote(
    "https://github.com/iterative-works/kanon.git"
  )

  /** Sample Git remote for GitLab (YouTrack detection). */
  val gitlabRemote: GitRemote = GitRemote(
    "https://gitlab.e-bs.cz/iterative-works/project.git"
  )

  /** Sample worktree registrations across multiple tracker types. Includes 5
    * worktrees: 2 Linear (IWLE-123, IWLE-456), 1 GitHub (GH-100), 2 YouTrack
    * (YT-111, YT-222). Timestamps vary to simulate different registration times
    * and activity levels.
    */
  ...
  /** Sample issue data with various statuses and assignee states. Includes
    * issues from Linear, GitHub, and YouTrack trackers. One issue (GH-100) has
    * no assignee.
    */
  lazy val sampleIssues: List[IssueData] =
    ...
    List(
      IssueData(
        ...
        assignee = Some("Michal Příhoda"),
        description =
          Some("Add support for loading sample data in development mode"),
  ...
  /** Sample cached issues with varying TTL states. Includes fresh (valid),
    * aging (valid), and stale (expired) cache entries.
    */
  ...
  /** Sample pull request data with all PR states. Includes Open, Merged, and
    * Closed PRs to test UI state rendering.
    */
  ...
  /** Sample cached PRs with varying TTL states. Includes fresh (valid) and
    * stale (expired) cache entries. Note: PR cache TTL is 2 minutes (shorter
    * than issue cache).
    */
  ...
  /** Sample workflow progress with various completion levels. Covers 0%, 10%,
    * 40%, 60%, and 100% completion.
    */
      ...
      WorkflowProgress(
        ...
        phases = List(
          PhaseInfo(
            1,
            "Setup",
            "/tmp/sample-worktree-iwle-123/phase-01-tasks.md",
            10,
            10
          ),
          PhaseInfo(
            2,
            "Implementation",
            "/tmp/sample-worktree-iwle-123/phase-02-tasks.md",
            20,
            10
          ),
          PhaseInfo(
            3,
            "Testing",
            "/tmp/sample-worktree-iwle-123/phase-03-tasks.md",
            15,
            0
          ),
          PhaseInfo(
            4,
            "Documentation",
            "/tmp/sample-worktree-iwle-123/phase-04-tasks.md",
            10,
            0
          ),
          PhaseInfo(
            5,
            "Review",
            "/tmp/sample-worktree-iwle-123/phase-05-tasks.md",
            5,
            0
          )
        ),
      ...
      WorkflowProgress(
        ...
        phases = List(
          PhaseInfo(
            1,
            "Setup",
            "/tmp/sample-worktree-iwle-456/phase-01-tasks.md",
            8,
            8
          ),
          PhaseInfo(
            2,
            "Implementation",
            "/tmp/sample-worktree-iwle-456/phase-02-tasks.md",
            15,
            15
          ),
          PhaseInfo(
            3,
            "Testing",
            "/tmp/sample-worktree-iwle-456/phase-03-tasks.md",
            12,
            12
          ),
          PhaseInfo(
            4,
            "Documentation",
            "/tmp/sample-worktree-iwle-456/phase-04-tasks.md",
            10,
            10
          ),
          PhaseInfo(
            5,
            "Review",
            "/tmp/sample-worktree-iwle-456/phase-05-tasks.md",
            5,
            5
          )
        ),
      ...
      WorkflowProgress(
        ...
        phases = List(
          PhaseInfo(
            1,
            "Analysis",
            "/tmp/sample-worktree-gh-100/phase-01-tasks.md",
            20,
            2
          ),
          PhaseInfo(
            2,
            "Implementation",
            "/tmp/sample-worktree-gh-100/phase-02-tasks.md",
            30,
            0
          ),
          PhaseInfo(
            3,
            "Testing",
            "/tmp/sample-worktree-gh-100/phase-03-tasks.md",
            10,
            0
          )
        ),
      ...
      WorkflowProgress(
        ...
        phases = List(
          PhaseInfo(
            1,
            "Design",
            "/tmp/sample-worktree-yt-222/phase-01-tasks.md",
            10,
            10
          ),
          PhaseInfo(
            2,
            "Implementation",
            "/tmp/sample-worktree-yt-222/phase-02-tasks.md",
            25,
            25
          ),
          PhaseInfo(
            3,
            "Testing",
            "/tmp/sample-worktree-yt-222/phase-03-tasks.md",
            20,
            5
          ),
          PhaseInfo(
            4,
            "Review",
            "/tmp/sample-worktree-yt-222/phase-04-tasks.md",
            5,
            0
          )
        ),
  ...
  /** Sample review states with various display states and artifact collections.
    * Covers different display types (progress, success, warning). Some have
    * artifacts, some have none.
    */
  lazy val sampleReviewStates: List[ReviewState] =
    List(
      ...
      ReviewState(
        ...
        taskLists = Some(
          List(
            TaskList(
              "Phase 2",
              "project-management/issues/IWLE-123/phase-02-tasks.md"
            )
          )
        ),
        needsAttention = None,
        ...
        artifacts = List(
          ...
          ReviewArtifact(
            "Phase 2 Context",
            "project-management/issues/IWLE-123/phase-02-context.md"
          )
        )
      ),
      ...
      ReviewState(
        display =
          Some(Display("Ready to Merge", Some("Phase 5 of 5"), "success")),
        badges = None,
        taskLists = None,
        needsAttention = None,
        ...
        artifacts = List(
          ReviewArtifact(
            "Implementation Log",
            "project-management/issues/IWLE-456/implementation-log.md"
          )
        )
      ),
      ...
      ReviewState(
        display =
          Some(Display("Awaiting Review", Some("Phase 1 of 3"), "warning")),
        badges = None,
        taskLists = None,
      ...
      ReviewState(
        ...
        taskLists = Some(
          List(
            TaskList(
              "Phase 3",
              "project-management/issues/YT-222/phase-03-tasks.md"
            )
          )
        ),
        needsAttention = None,
        ...
        artifacts = List(
          ...
          ReviewArtifact(
            "Review Notes",
            "project-management/issues/YT-222/review.md"
          )
        )
      )
    )

  /** Sample project registrations across multiple tracker types. Includes
    * GitHub, Linear, and YouTrack projects for testing.
    */
  ...
  /** Sample cached review states with file modification timestamps. Note:
    * YT-111 has no review state (edge case in design).
    */
