...
class WorktreePriorityTest extends munit.FunSuite:
  ...
  test("recent activity gets higher priority score than older activity"):
    ...
    assert(
      ...
      s"Recent activity (score=$recentScore) should have higher score than older (score=$olderScore)"
    )
  ...
  test("older activity gets lower priority score"):
    ...
    val somewhatOldScore =
      WorktreePriority.priorityScore(somewhatOldWorktree, now)

    assert(
      somewhatOldScore > veryOldScore,
      s"Somewhat old activity (score=$somewhatOldScore) should have higher score than very old (score=$veryOldScore)"
    )
  ...
  test("priority score handles very recent activity (just now)"):
    ...
    val oneHourAgoScore =
      WorktreePriority.priorityScore(oneHourAgoWorktree, now)

    assert(
      justNowScore >= oneHourAgoScore,
      s"Just now activity (score=$justNowScore) should have at least as high score as 1 hour ago (score=$oneHourAgoScore)"
    )
