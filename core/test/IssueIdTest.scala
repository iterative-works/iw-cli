...
class IssueIdTest extends FunSuite:
  ...
  test("IssueId.toBranchName returns value unchanged"):
    val issueId = IssueId
      .parse("IWLE-123")
      .getOrElse(fail("Failed to parse valid issue ID"))
    assertEquals(issueId.toBranchName, "IWLE-123")

  test("IssueId.toBranchName preserves exact format"):
    val issueId =
      IssueId.parse("ABC-99").getOrElse(fail("Failed to parse valid issue ID"))
  ...
  test("IssueId.team returns team for TEAM-NNN format"):
    val issueId =
      IssueId.parse("IWLE-132").getOrElse(fail("Failed to parse TEAM-NNN ID"))
  ...
  test(
    "IssueId.parse with team prefix composes TEAM-NNN format from numeric input"
  ):
  ...
  test(
    "IssueId.parse explicit TEAM-NNN format wins over different defaultTeam"
  ):
  ...
  test(
    "IssueId.parse with invalid team prefix still composes (validation happens in forGitHub)"
  ):
  ...
  test("IssueId.team returns correct team for GitLab TEAM-NNN format"):
    val issueId =
      IssueId.parse("123", Some("PROJ")).getOrElse(fail("Failed to parse"))
