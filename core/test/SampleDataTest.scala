...
class SampleDataTest extends FunSuite:
  ...
  test("sampleWorktrees includes YouTrack tracker types"):
    ...
    assert(
      youtrackWorktrees.size >= 2,
      "Should have at least 2 YouTrack worktrees"
    )
  ...
  test("sampleWorktrees have timestamps in the past"):
    ...
    worktrees.foreach { wt =>
      assert(
        wt.registeredAt.isBefore(now) || wt.registeredAt.equals(now),
        s"registeredAt should not be in the future for ${wt.issueId}"
      )
      assert(
        wt.lastSeenAt.isBefore(now) || wt.lastSeenAt.equals(now),
  ...
  test("sampleIssues includes at least one issue without assignee"):
    ...
    assert(
      unassignedIssues.nonEmpty,
      "Should have at least one issue without assignee"
    )
  ...
  test("sampleIssues have valid URLs"):
    ...
    issues.foreach { issue =>
      ...
      assert(
        ...
        s"URL should start with http for ${issue.id}"
      )
    }
  ...
  test("sampleCachedIssues isStale returns true for stale samples"):
    ...
    val staleCachedIssues =
      cachedIssues.filter(ci => CachedIssue.isStale(ci, now))
    assert(
      staleCachedIssues.nonEmpty,
      "Should have at least one stale cached issue"
    )
  ...
  test("samplePRs returns PRs with all states"):
    ...
    assert(
      states.contains(PRState.Merged),
      "Should have at least one Merged PR"
    )
    assert(
      states.contains(PRState.Closed),
      "Should have at least one Closed PR"
    )
  ...
  test("samplePRs have valid URLs"):
    ...
    prs.foreach { pr =>
      ...
      assert(
        ...
        s"URL should start with http for PR #${pr.number}"
      )
    }
  ...
  test("sampleWorkflowProgress covers all completion levels"):
    ...
    assert(
      progressList.size >= 4,
      "Should have at least 4 workflow progress samples"
    )

    val percentages = progressList.map(_.overallPercentage).toSet
    // Check for diversity in completion levels (10%, 40%, 60%, 100%)
    assert(
      percentages.exists(_ < 20),
      "Should have low completion sample (~10%)"
    )
    assert(
      percentages.exists(p => p >= 30 && p < 50),
      "Should have mid completion sample (~40%)"
    )
    assert(
      percentages.exists(p => p >= 50 && p < 70),
      "Should have mid-high completion sample (~60%)"
    )
  ...
  test("sampleWorkflowProgress have valid phase structures"):
    ...
    progressList.foreach { progress =>
      ...
      assert(
        progress.overallTotal >= progress.overallCompleted,
  ...
  test("sampleCachedProgress include valid filesMtime"):
    ...
    assert(
      cachedProgressList.size >= 3,
      "Should have at least 3 cached progress samples"
    )
  ...
  test("sampleReviewStates include both with and without artifacts"):
    ...
    assert(
      withArtifacts.nonEmpty,
      "Should have at least one review state with artifacts"
    )
    assert(
      withoutArtifacts.nonEmpty,
      "Should have at least one review state without artifacts"
    )

  test("sampleCachedReviewStates include valid filesMtime"):
    val cachedReviewStates = SampleData.sampleCachedReviewStates
    assert(
      cachedReviewStates.size >= 2,
      "Should have at least 2 cached review states"
    )
