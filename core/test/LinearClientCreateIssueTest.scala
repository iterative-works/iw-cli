...
class LinearClientCreateIssueTest extends FunSuite:
  ...
  test("parseCreateIssueResponse handles valid response"):
    val json =
      """{"data":{"issueCreate":{"success":true,"issue":{"id":"abc123","url":"https://linear.app/issue/abc123"}}}}"""
  ...
  test("parseCreateIssueResponse handles API error"):
    ...
    assert(
      error.contains("Invalid team"),
      s"Expected error to contain 'Invalid team', got: $error"
    )

  test("parseCreateIssueResponse handles missing url field"):
    val json =
      """{"data":{"issueCreate":{"success":true,"issue":{"id":"abc123"}}}}"""
    val result = LinearClient.parseCreateIssueResponse(json)
    assert(result.isLeft, "Expected Left for missing url field")
    val error = result.left.getOrElse("")
    assert(
      error.contains("url") || error.contains("field"),
      s"Expected error about missing field, got: $error"
    )
  ...
  test("buildCreateIssueMutation creates valid GraphQL mutation"):
    ...
    assert(
      mutation.contains("mutation"),
      "Mutation should contain 'mutation' keyword"
    )
    assert(
      mutation.contains("issueCreate"),
      "Mutation should contain 'issueCreate'"
    )

    // Verify input fields are present
    assert(mutation.contains("title"), "Mutation should contain 'title' field")
    assert(
      mutation.contains("description"),
      "Mutation should contain 'description' field"
    )
    assert(
      mutation.contains("teamId"),
      "Mutation should contain 'teamId' field"
    )

    // Verify actual values are present
    assert(
      mutation.contains("Test Issue"),
      "Mutation should contain the title value"
    )
    assert(
      mutation.contains("Test Description"),
      "Mutation should contain the description value"
    )
    assert(
      mutation.contains("team-123"),
      "Mutation should contain the teamId value"
    )

    // Verify response fields are requested
    assert(
      mutation.contains("success"),
      "Mutation should request 'success' field"
    )
