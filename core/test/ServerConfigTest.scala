...
class ServerConfigTest extends munit.FunSuite:
  ...
  test(
    "analyzeHostsSecurity returns exposed hosts warning for mixed localhost and non-localhost"
  ):
    val analysis =
      ServerConfig.analyzeHostsSecurity(Seq("127.0.0.1", "100.64.1.5"))
    assert(analysis.hasWarning)
    assert(!analysis.bindsToAll)
    assertEquals(analysis.exposedHosts, Seq("100.64.1.5"))

  test("analyzeHostsSecurity lists all non-localhost IPs in exposedHosts"):
    val analysis = ServerConfig.analyzeHostsSecurity(
      Seq("localhost", "192.168.1.100", "10.0.0.1")
    )
    assert(analysis.hasWarning)
    assert(!analysis.bindsToAll)
