...
class ProcessTest extends FunSuite:
  ...
  test("commandExists prevents shell injection with metacharacters"):
    ...
    assertEquals(
      ProcessAdapter.commandExists("command | cat /etc/passwd"),
      false
    )
  ...
  test("run truncates stdout when exceeding maxOutputBytes"):
    ...
    val result =
      ProcessAdapter.run(Seq("echo", largeString), maxOutputBytes = 100)
    assertEquals(result.exitCode, 0)
  ...
  test("run handles output at exact boundary"):
    ...
    val result =
      ProcessAdapter.run(Seq("echo", exactString), maxOutputBytes = 100)
    assertEquals(result.exitCode, 0)
