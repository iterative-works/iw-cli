...
class SearchResultsViewTest extends FunSuite:
  ...
  test("render with single result shows result item"):
    ...
    assert(
      html.contains("Add modal for worktree creation"),
      "Should show title"
    )
  ...
  test("result item includes title with correct CSS class"):
    ...
    assert(
      html.contains("class=\"search-result-title\""),
  ...
  test("result item includes status with correct CSS class"):
    ...
    assert(
      html.contains("class=\"search-result-status\""),
  ...
  test("result item is wrapped in clickable div"):
    ...
    assert(
      html.contains("class=\"search-result-item\""),
      "Should have result item class"
    )
  ...
  test("empty state has appropriate styling class"):
    ...
    assert(
      html.contains("class=\"search-empty-state\""),
  ...
  test("result item has hx-post attribute for worktree creation"):
    ...
    assert(
      ...
      "Should POST to creation endpoint"
    )
  ...
  test("result item has hx-indicator for loading spinner"):
    ...
    assert(
      html.contains("#creation-spinner"),
      "Should reference creation spinner"
    )
  ...
  test("render without hasWorktree flag shows no badge"):
    ...
    assert(
      !html.contains("Already has worktree"),
      "Should not show worktree badge"
    )
  ...
  test("result with existing worktree does not have hx-post attribute"):
    ...
    assert(
      itemPattern.findFirstIn(html).isEmpty,
      "Should not have create action for existing worktree"
    )
  ...
  test("results container has hx-on::before-request to disable UI"):
    ...
    assert(
      html.contains("hx-on::before-request"),
      "Should have before-request handler"
    )
    assert(
      html.contains("classList.add('disabled')"),
      "Should add disabled class on request start"
    )
  ...
  test("results container has hx-on::after-request to re-enable UI"):
    ...
    assert(
      html.contains("hx-on::after-request"),
      "Should have after-request handler"
    )
    assert(
      html.contains("classList.remove('disabled')"),
      "Should remove disabled class on request end"
    )
