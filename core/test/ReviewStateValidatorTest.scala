...
class ReviewStateValidatorTest extends munit.FunSuite:
  ...
  test("valid JSON without optional status field"):
    ...
    assert(
      result.isValid,
      s"Status is optional, should be valid: ${result.errors}"
    )
  ...
  test("malformed JSON returns parse error"):
    ...
    assert(
      result.errors.head.field == "root",
      s"Expected field 'root' but got '${result.errors.head.field}'"
    )
    assert(
      result.errors.head.message.contains("parse"),
      s"Expected parse error but got: ${result.errors.head.message}"
    )
  ...
  test("missing required fields returns field-specific errors"):
    val json =
      """{"version": 1, "artifacts": [], "last_updated": "2026-01-28T12:00:00Z"}"""
    val result = ReviewStateValidator.validate(json)
    assert(!result.isValid)
    val errorFields = result.errors.map(_.field).toSet
    assert(
      errorFields.contains("issue_id"),
      s"Expected error for issue_id, got: $errorFields"
    )
  ...
  test("missing all required fields returns errors for each"):
    ...
    assert(
      errorFields.contains("last_updated"),
      s"Missing error for last_updated"
    )
  ...
  test("wrong type for version returns type error"):
    ...
    assert(
      ...
      s"Expected error for version, got: ${result.errors}"
    )
  ...
  test("version less than 1 returns error"):
    ...
    assert(
      versionErrors.nonEmpty,
      s"Expected error for version minimum, got: ${result.errors}"
    )
  ...
  test("multiple wrong types returns multiple errors"):
    ...
    assert(
      ...
      s"Expected at least 3 errors, got ${result.errors.size}: ${result.errors}"
    )
  ...
  test("any status value is accepted without warning"):
    ...
    assert(
      result.isValid,
      s"Any status value should be valid: ${result.errors}"
    )
  ...
  test("unknown top-level property returns error"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected error for unknown property, got: ${result.errors}"
    )
  ...
  test("removed v1 fields (phase, step, branch, batch_mode) return errors"):
    ...
    assert(
      errorMessages.contains("phase"),
      s"Expected error for phase property"
    )
    assert(errorMessages.contains("step"), s"Expected error for step property")
    assert(
      errorMessages.contains("branch"),
      s"Expected error for branch property"
    )
    assert(
      errorMessages.contains("batch_mode"),
      s"Expected error for batch_mode property"
    )
  ...
  test("pr_url as string is valid"):
    ...
    assert(
      result.isValid,
      s"pr_url as string should be valid: ${result.errors}"
    )
  ...
  test("validates nested artifact structure - missing label"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected error for missing artifact label: ${result.errors}"
    )
  ...
  test("validates nested artifact structure - missing path"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected error for missing artifact path: ${result.errors}"
    )
  ...
  test("validates nested artifact structure - valid with category"):
    ...
    assert(
      result.isValid,
      s"Artifact with category should be valid: ${result.errors}"
    )
  ...
  test("validates nested artifact structure - wrong type for category"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected error for category type: ${result.errors}"
    )
  ...
  test("validates nested available_actions structure - missing required field"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected error for missing action skill: ${result.errors}"
    )
  ...
  test("validates nested phase_checkpoints structure - missing context_sha"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected error for missing context_sha: ${result.errors}"
    )

  test(
    "validates nested phase_checkpoints structure - wrong type for context_sha"
  ):
    val json = """{
      "version": 1,
      "issue_id": "IW-1",
  ...
  test(
    "validates nested phase_checkpoints structure - checkpoint not an object"
  ):
    val json = """{
      "version": 1,
      "issue_id": "IW-1",
  ...
  test(
    "validates nested phase_checkpoints structure - unknown property in checkpoint"
  ):
    val json = """{
      "version": 1,
      "issue_id": "IW-1",
  ...
  test("display with subtext is valid"):
    ...
    assert(
      result.isValid,
      s"Display with subtext should pass: ${result.errors}"
    )
  ...
  test("all display type values are valid"):
    ...
    validTypes.foreach { displayType =>
      ...
      assert(
        result.isValid,
        s"Display type '$displayType' should be valid: ${result.errors}"
      )
    }
  ...
  test("needs_attention as boolean is valid"):
    ...
    assert(
      result.isValid,
      s"needs_attention boolean should be valid: ${result.errors}"
    )
  ...
  test("activity 'working' passes validation"):
    ...
    assert(
      result.isValid,
      s"activity 'working' should be valid: ${result.errors}"
    )
  ...
  test("activity 'waiting' passes validation"):
    ...
    assert(
      result.isValid,
      s"activity 'waiting' should be valid: ${result.errors}"
    )
  ...
  test("activity 'idle' returns enum error"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected enum error for activity 'idle': ${result.errors}"
    )
  ...
  test("activity as integer returns type error"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected type error for activity integer: ${result.errors}"
    )
  ...
  test("workflow_type 'agile' passes validation"):
    ...
    assert(
      result.isValid,
      s"workflow_type 'agile' should be valid: ${result.errors}"
    )
  ...
  test("workflow_type 'waterfall' passes validation"):
    ...
    assert(
      result.isValid,
      s"workflow_type 'waterfall' should be valid: ${result.errors}"
    )
  ...
  test("workflow_type 'diagnostic' passes validation"):
    ...
    assert(
      result.isValid,
      s"workflow_type 'diagnostic' should be valid: ${result.errors}"
    )
  ...
  test("workflow_type 'kanban' returns enum error"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected enum error for workflow_type 'kanban': ${result.errors}"
    )
  ...
  test("workflow_type as boolean returns type error"):
    ...
    assert(
      errors.nonEmpty,
      s"Expected type error for workflow_type boolean: ${result.errors}"
    )
