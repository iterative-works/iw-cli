...
class GitTest extends munit.FunSuite, Fixtures:
  ...
  gitRepo.test("GitAdapter reads git remote URL"): repo =>
    val remote = GitAdapter.getRemoteUrl(repo)
    assertEquals(
      remote,
      Some(GitRemote("https://github.com/iterative-works/test-repo.git"))
    )

  gitRepo.test("GitAdapter extracts host from remote URL"): repo =>
    val remote = GitAdapter.getRemoteUrl(repo)
    assert(remote.isDefined)
    assertEquals(remote.get.host, Right("github.com"))

  tempDir.test("GitAdapter returns None for non-git directory"): dir =>
    val remote = GitAdapter.getRemoteUrl(dir)
    assertEquals(remote, None)

  gitRepo.test("GitAdapter returns None for git repo without remote"): repo =>
    // Remove the remote
    Process(Seq("git", "remote", "remove", "origin"), repo.toIO).!
    val remote = GitAdapter.getRemoteUrl(repo)
    assertEquals(remote, None)

  tempDir.test("GitAdapter checks if directory is in git repo"): dir =>
    // Not a git repo
  ...
  gitRepo.test("GitAdapter gets current branch name"): repo =>
  ...
  gitRepo.test("GitAdapter gets current branch name on custom branch"): repo =>
  ...
  gitRepo.test("GitAdapter handles detached HEAD"): repo =>
  ...
  tempDir.test(
    "GitAdapter returns error for non-git directory when getting branch"
  ): dir =>
  ...
  gitRepo.test("hasUncommittedChanges returns true for modified files"): repo =>
  ...
  gitRepo.test("GitAdapter gets HEAD SHA"): repo =>
    ...
    assert(
      sha.forall(c => "0123456789abcdef".contains(c)),
      s"SHA should be hex: $sha"
    )
  ...
  tempDir.test("GitAdapter returns error for getHeadSha on non-git directory"):
    dir =>
      ...
      assert(
        result.left.exists(_.contains("Failed to get HEAD SHA")),
        s"Expected error message, got: $result"
      )
  ...
  gitRepo.test(
    "createAndCheckoutBranch on an already-existing branch name returns Left"
  ): repo =>
  ...
  gitRepo.test("checkoutBranch switches to an existing branch"): repo =>
  ...
  gitRepo.test("checkoutBranch on a non-existent branch returns Left"): repo =>
  ...
  gitRepo.test(
    "stageFiles stages only the specified file, leaving other changes unstaged"
  ): repo =>
    ...
    assert(result.isRight, s"Expected Right but got: $result")
    val staged = ProcessAdapter.run(
      Seq("git", "-C", repo.toString, "diff", "--cached", "--name-only")
    )
    assert(staged.stdout.contains("target.txt"), "target.txt should be staged")
    assert(
      !staged.stdout.contains("other.txt"),
      "other.txt should NOT be staged"
    )

  gitRepo.test("stageFiles stages a modified tracked file"): repo =>
    os.write.over(repo / "README.md", "modified content")
  ...
  gitRepo.test("stageFiles with a non-existent path returns Left"): repo =>
  ...
  gitRepo.test("stageAll on a repo with unstaged changes stages all files"):
    repo =>
      ...
      val statusResult = ProcessAdapter.run(
        Seq("git", "-C", repo.toString, "diff", "--cached", "--name-only")
      )
      assert(
        statusResult.stdout.contains("new-file.txt"),
        "new-file.txt should be staged"
      )

  gitRepo.test("stageAll on a clean worktree succeeds with no error"): repo =>
    val result = GitAdapter.stageAll(repo)
    assert(result.isRight, s"Expected Right but got: $result")
  ...
  gitRepo.test(
    "commit with a staged change returns Right containing a 40-character SHA string"
  ): repo =>
  ...
  gitRepo.test("commit with no staged changes returns Left"): repo =>
  ...
  gitRepo.test("push with setUpstream = true pushes to bare repo remote"):
    repo =>
      ...
      val bareDir =
        os.Path(java.nio.file.Files.createTempDirectory("iw-test-bare"))
      try
        Process(Seq("git", "init", "--bare"), bareDir.toIO).!
        // Update origin to point to bare repo
        Process(
          Seq("git", "remote", "set-url", "origin", bareDir.toString),
          repo.toIO
        ).!
  ...
  gitRepo.test("diffNameOnly lists files changed since a baseline commit SHA"):
    repo =>
      ...
      assert(result.isRight, s"Expected Right but got: $result")
      assert(
        result.toOption.get.contains("new.txt"),
        s"Expected new.txt in diff: $result"
      )

  gitRepo.test(
    "diffNameOnly with an invalid/non-existent baseline SHA returns Left"
  ): repo =>
    val result =
      GitAdapter.diffNameOnly("deadbeefdeadbeefdeadbeefdeadbeefdeadbeef", repo)
    assert(result.isLeft, s"Expected Left but got: $result")

  gitRepo.test(
    "diffNameOnly with no changes since baseline returns Right(Nil)"
  ): repo =>
  ...
  gitRepo.test("pull on a branch with nothing to pull succeeds"): repo =>
    // Set origin to a bare repo clone
    val bareDir =
      os.Path(java.nio.file.Files.createTempDirectory("iw-test-bare-pull"))
    try
      Process(Seq("git", "init", "--bare"), bareDir.toIO).!
      Process(
        Seq("git", "remote", "set-url", "origin", bareDir.toString),
  ...
  gitRepo.test("getFullHeadSha returns a 40-character hex string"): repo =>
  ...
  gitRepo.test(
    "getFullHeadSha result differs from getHeadSha abbreviated output"
  ): repo =>
    ...
    assert(
      fullSha.startsWith(shortSha),
      s"Full SHA should start with short SHA"
    )

  // ========== fetchAndReset Tests ==========

  gitRepo.test(
    "fetchAndReset updates local branch to match remote after new commits"
  ): repo =>
    // Create a bare repo as origin
    val bareDir =
      os.Path(java.nio.file.Files.createTempDirectory("iw-test-bare-fr"))
    try
      Process(Seq("git", "init", "--bare"), bareDir.toIO).!
      Process(
        Seq("git", "remote", "set-url", "origin", bareDir.toString),
      ...
      val cloneDir =
        os.Path(java.nio.file.Files.createTempDirectory("iw-test-clone-fr"))
      try
        Process(Seq("git", "clone", bareDir.toString, cloneDir.toString)).!
        Process(
          Seq("git", "config", "user.email", "test@example.com"),
          cloneDir.toIO
        ).!
        ...
        val remoteSha = Process(
          Seq("git", "rev-parse", s"origin/$branch"),
          cloneDir.toIO
        ).!!.trim
        ...
        assert(
          beforeSha != afterSha,
          "HEAD SHA should change after fetchAndReset"
        )
        assert(
          os.exists(repo / "remote-change.txt"),
          "Remote file should exist after reset"
        )
        assertEquals(
          afterSha,
          remoteSha,
          "Local HEAD should match remote SHA after fetchAndReset"
        )
      finally os.remove.all(cloneDir)
    finally os.remove.all(bareDir)

  // ========== isFileDirty Tests ==========

  gitRepo.test("isFileDirty returns false for a clean tracked file"): repo =>
    assert(!GitAdapter.isFileDirty(repo / "README.md", repo))

  gitRepo.test("isFileDirty returns true for a modified tracked file"): repo =>
    os.write.over(repo / "README.md", "modified")
    assert(GitAdapter.isFileDirty(repo / "README.md", repo))

  gitRepo.test("isFileDirty returns true for an untracked file"): repo =>
  ...
  gitRepo.test("commitFileWithRetry commits a modified file and returns SHA"):
    repo =>
      ...
      val result = GitAdapter.commitFileWithRetry(
        repo / "README.md",
        "Test commit with retry",
        repo
      )
      assert(result.isRight, s"Expected Right but got: $result")
      val sha = result.toOption.get
  ...
  gitRepo.test(
    "commitFileWithRetry succeeds silently when file is already clean"
  ): repo =>
  ...
  gitRepo.test("fetchAndReset returns Left when no remote configured"): repo =>
    Process(Seq("git", "remote", "remove", "origin"), repo.toIO).!
