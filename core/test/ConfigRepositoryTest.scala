...
class ConfigRepositoryTest extends munit.FunSuite, Fixtures:
  ...
  tempDir.test("ConfigFileRepository writes config to file"): dir =>
  ...
  tempDir.test("ConfigFileRepository reads config from file"): dir =>
    val configPath = dir / ".iw" / "config.conf"
  ...
  tempDir.test("ConfigFileRepository roundtrip preserves data"): dir =>
  ...
  tempDir.test("ConfigFileRepository writes config with version field"): dir =>
    val config = ProjectConfiguration.create(
      trackerType = IssueTrackerType.Linear,
      team = "IWLE",
      projectName = "kanon",
  ...
  tempDir.test("ConfigFileRepository reads config with version field"): dir =>
    val configPath = dir / ".iw" / "config.conf"
    os.makeDir.all(configPath / os.up)
    os.write(
      configPath,
  ...
  tempDir.test(
    "ConfigFileRepository defaults to 'latest' when version is missing"
  ): dir =>
    val configPath = dir / ".iw" / "config.conf"
    os.makeDir.all(configPath / os.up)
    os.write(
