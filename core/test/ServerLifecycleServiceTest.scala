...
class ServerLifecycleServiceTest extends munit.FunSuite:
  ...
  test("Create status for running server"):
    ...
    val status =
      ServerLifecycleService.createStatus(state, startedAt, pid, port)
  ...
  test("Format status display with three hosts"):
    ...
    assertEquals(
      message,
      "Server running on localhost:9876, 127.0.0.1:9876, 192.168.1.5:9876"
    )
  ...
  test("Format security warning for bind-all (0.0.0.0)"):
    ...
    assert(
      warning.get.contains(
        "WARNING: Server is accessible from all network interfaces"
      )
    )
  ...
  test("Format security warning for specific exposed hosts"):
    val analysis =
      iw.core.model.SecurityAnalysis(Seq("100.64.1.5"), false, true)
    val warning = ServerLifecycleService.formatSecurityWarning(analysis)
    assert(warning.isDefined)
    assert(
      warning.get.contains(
        "WARNING: Server is accessible from non-localhost interfaces"
      )
    )
  ...
  test("Format security warning lists multiple exposed hosts"):
    val analysis = iw.core.model.SecurityAnalysis(
      Seq("192.168.1.100", "10.0.0.1"),
      false,
      true
    )
    val warning = ServerLifecycleService.formatSecurityWarning(analysis)
