...
class DoctorChecksTest extends FunSuite:
  ...
  test("CheckResult hint extraction helper works correctly"):
    ...
    assertEquals(
      CheckResult.WarningWithHint("Warn", "Fix it").hint,
      Some("Fix it")
    )
  ...
  test("DoctorChecks.runAll passes config to check functions"):
    val checks = List(
      Check(
        "Config Check",
        config =>
          if config.team == "EXPECTED" then CheckResult.Success("Correct team")
          else
    ...
    val correctConfig =
      ProjectConfiguration.create(IssueTrackerType.Linear, "EXPECTED", "test")
    val wrongConfig =
      ProjectConfiguration.create(IssueTrackerType.Linear, "WRONG", "test")
  ...
  test("DoctorChecks.runAll with empty list returns empty results"):
    val config =
      ProjectConfiguration.create(IssueTrackerType.Linear, "TEST", "test")
  ...
  test(
    "filterByCategory returns only Environment checks when filter is Some(Environment)"
  ):
    val checks = List(
      Check("Env Check 1", _ => CheckResult.Success("OK"), "Environment"),
      Check("Quality Check 1", _ => CheckResult.Success("OK"), "Quality"),
  ...
  test(
    "filterByCategory returns only Quality checks when filter is Some(Quality)"
  ):
    val checks = List(
      Check("Env Check 1", _ => CheckResult.Success("OK"), "Environment"),
      Check("Quality Check 1", _ => CheckResult.Success("OK"), "Quality"),
