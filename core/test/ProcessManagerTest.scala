...
import iw.core.adapters.ProcessManager
...
class ProcessManagerTest extends munit.FunSuite:
  ...
  fixture.test("Write PID file and read back returns same PID"): tempDir =>
  ...
  fixture.test("Check if current process is alive"): tempDir =>
  ...
  fixture.test("Detect process is not alive for invalid PID"): tempDir =>
  ...
  fixture.test("Read non-existent PID file returns None"): tempDir =>
  ...
  fixture.test("Remove PID file after write"): tempDir =>
    val pidPath = tempDir / "server.pid"
  ...
  fixture.test("Remove non-existent PID file succeeds"): tempDir =>
    val pidPath = tempDir / "nonexistent.pid"

    val removeResult = ProcessManager.removePidFile(pidPath.toString)
    assert(removeResult.isRight)

  fixture.test("Handle malformed PID file content"): tempDir =>
  ...
  fixture.test("Handle empty PID file"): tempDir =>
    ...
    assert(
      result.left.exists(err =>
        err.contains("Invalid PID") || err.contains("empty")
      )
    )

  fixture.test("PID file parent directory created if missing"): tempDir =>
