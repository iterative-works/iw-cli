...
class ServerStateServiceTest extends munit.FunSuite:
  ...
  tempDir.test(
    "ServerStateService.initialize creates empty state if file doesn't exist"
  ): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("ServerStateService.getState returns current state"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test(
    "ServerStateService.updateWorktree removes worktree when function returns None"
  ): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("ServerStateService.updateWorktree persists changes"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("ServerStateService concurrent updates don't lose data"):
    tempDir =>
      ...
      assertEquals(
        ...
        threadCount,
        "All concurrent updates should be preserved"
      )
      ...
      diskState.foreach { state =>
        assertEquals(
          state.worktrees.size,
          threadCount,
          "All concurrent updates should be persisted"
        )
      }
  ...
  tempDir.test("ServerStateService.pruneWorktrees removes invalid worktrees"):
    tempDir =>
      ...
      // Add three worktrees
      service.updateWorktree("IWLE-1")(_ =>
        Some(
          WorktreeRegistration(
            "IWLE-1",
            "/path/valid1",
      ...
      service.updateWorktree("IWLE-2")(_ =>
        Some(
          WorktreeRegistration(
            "IWLE-2",
            "/path/invalid",
      ...
      service.updateWorktree("IWLE-3")(_ =>
        Some(
          WorktreeRegistration(
            "IWLE-3",
            "/path/valid2",
  ...
  tempDir.test("ServerStateService.updatePRCache adds PR data"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("ServerStateService.updateReviewStateCache adds review state"):
    tempDir =>
      ...
      val cached =
        CachedReviewState(reviewState, Map("/path/review-state.json" -> 2000L))
  ...
  tempDir.test("ServerStateService.updateProject adds new project"): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
  ...
  tempDir.test("ServerStateService.updateProject updates existing project"):
    tempDir =>
      ...
      service.updateProject("/home/user/projects/my-project")(_ =>
        Some(project)
      )
      ...
      assertEquals(state.projects.size, 1)
      assertEquals(
        state.projects("/home/user/projects/my-project").projectName,
        "my-project-renamed"
      )
  ...
  tempDir.test(
    "ServerStateService.pruneProjects removes entries that fail validation"
  ): tempDir =>
    val statePath = tempDir.resolve("state.json")
    val repo = StateRepository(statePath.toString)
    ...
    service.updateProject("/path/valid1")(_ =>
      Some(
        ProjectRegistration(
          "/path/valid1",
          "valid1",
    ...
    service.updateProject("/path/invalid")(_ =>
      Some(
        ProjectRegistration(
          "/path/invalid",
          "invalid",
    ...
    service.updateProject("/path/valid2")(_ =>
      Some(
        ProjectRegistration(
          "/path/valid2",
          "valid2",
