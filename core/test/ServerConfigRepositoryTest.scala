...
class ServerConfigRepositoryTest extends munit.FunSuite:
  ...
  fixture.test("Write config file and read back returns same port"): tempDir =>
  ...
  fixture.test("Create default config if file missing"): tempDir =>
    val configPath = tempDir / "config.json"
  ...
  fixture.test("Handle invalid JSON in config file"): tempDir =>
    val configPath = tempDir / "config.json"
  ...
  fixture.test("Handle missing port field in JSON"): tempDir =>
    val configPath = tempDir / "config.json"
    os.write(configPath, "{}")
    ...
    assert(result.isLeft)
    assert(
      result.left.exists(err =>
        err.contains("port") || err.contains("Failed to parse")
      )
    )

  fixture.test("Handle invalid port value in JSON"): tempDir =>
    val configPath = tempDir / "config.json"
    ...
    assert(result.isLeft)
    assert(
      result.left.exists(_.contains("Port must be between 1024 and 65535"))
    )

  fixture.test("Handle missing parent directory - creates it"): tempDir =>
  ...
  fixture.test("Read non-existent file returns error"): tempDir =>
  ...
  fixture.test("getOrCreateDefault on existing file reads it"): tempDir =>
    val configPath = tempDir / "config.json"
    val config = ServerConfig(8080)
  ...
  fixture.test(
    "Deserialize config without hosts field defaults to Seq(localhost)"
  ): tempDir =>
    val configPath = tempDir / "config.json"
  ...
  fixture.test("Deserialize config with hosts field preserves explicit values"):
    tempDir =>
      ...
      os.write(
        configPath,
        """{"port": 9876, "hosts": ["127.0.0.1", "192.168.1.5"]}"""
      )
