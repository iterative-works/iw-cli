...
class IssueIdFromBranchTest extends FunSuite:
  ...
  test(
    "IssueId.fromBranch extracts from branch with suffix IWLE-123-description"
  ):
  ...
  test(
    "IssueId.fromBranch extracts from branch with multiple suffixes IWLE-123-fix-bug-in-parser"
  ):
  ...
  test(
    "IssueId.fromBranch normalizes lowercase with suffix iwle-123-description"
  ):
  ...
  test(
    "IssueId.fromBranch extracts from different project with suffix ABC-456-test"
  ):
  ...
  test("IssueId.fromBranch rejects branch without issue ID (main)"):
    ...
    assert(
      result.left.exists(
        _.contains("Cannot extract issue ID from branch 'main'")
      )
    )
  ...
  test("IssueId.fromBranch rejects branch without issue ID (master)"):
    ...
    assert(
      result.left.exists(
        _.contains("Cannot extract issue ID from branch 'master'")
      )
    )
  ...
  test("IssueId.fromBranch rejects branch without issue ID (develop)"):
    ...
    assert(
      result.left.exists(
        _.contains("Cannot extract issue ID from branch 'develop'")
      )
    )

  test(
    "IssueId.fromBranch rejects branch with invalid format (feature-branch)"
  ):
    val result = IssueId.fromBranch("feature-branch")
    assert(result.isLeft)
    assert(
      result.left.exists(
        _.contains("Cannot extract issue ID from branch 'feature-branch'")
      )
    )
  ...
  test("IssueId.fromBranch rejects empty string"):
    ...
    assert(result.isLeft)
    assert(
      result.left.exists(_.contains("Cannot extract issue ID from branch ''"))
    )
  ...
  test(
    "IssueId.fromBranch handles long project names with suffix PROJECT-789-description"
  ):
  ...
  test(
    "IssueId.fromBranch extracts from TEAM-NNN branch with description for GitLab"
  ):
  ...
  test(
    "IssueId.fromBranch extracts single-digit from TEAM-N branch for GitLab"
  ):
