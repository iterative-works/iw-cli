...
class CachedPRTest extends FunSuite:
  ...
  test(
    "isValid returns true at just under TTL boundary (119 seconds ago, TTL 2)"
  ):
    val now = Instant.now()
  ...
  test(
    "isValid returns false when exactly at TTL boundary (120 seconds = 2 minutes)"
  ):
    val now = Instant.now()
  ...
  test("isStale respects custom TTL of 15 minutes"):
    ...
    assert(
      !result,
      "Cache should not be stale when 10 minutes old with 15 minute TTL"
    )
  ...
  test("isStale returns true with custom TTL of 15 minutes when old"):
    ...
    assert(
      result,
      "Cache should be stale when 16 minutes old with 15 minute TTL"
    )
