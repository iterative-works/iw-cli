...
class GitLabClientTest extends munit.FunSuite:
  ...
  test(
    "buildFetchIssueCommand generates correct glab CLI arguments for simple repository"
  ):
  ...
  test(
    "validateGlabPrerequisites returns GlabNotInstalled when glab not found"
  ):
    val mockIsCommandAvailable = (cmd: String) => false
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) => Right("shouldn't get here")
    ...
    assert(result.isLeft)
    assertEquals(
      result.left.getOrElse(fail("Expected Left")),
      GitLabClient.GlabPrerequisiteError.GlabNotInstalled
    )

  test(
    "validateGlabPrerequisites returns GlabNotAuthenticated when auth status fails"
  ):
    ...
    assertEquals(
      result.left.getOrElse(fail("Expected Left")),
      GitLabClient.GlabPrerequisiteError.GlabNotAuthenticated
    )

  test(
    "validateGlabPrerequisites returns Right(()) when glab is authenticated"
  ):
    val mockIsCommandAvailable = (cmd: String) => true
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
  ...
  test("fetchIssue validates prerequisites first - glab not installed"):
    val mockIsCommandAvailable = (cmd: String) => false
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) => fail("Should not execute command when glab not installed")
    ...
    assert(
      error.contains("glab CLI is not installed"),
      s"Error should mention glab not installed: $error"
    )
  ...
  test("fetchIssue validates prerequisites first - glab not authenticated"):
    ...
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        if args.contains("auth") && args.contains("status") then
          Left("not logged in")
        else fail("Should not execute issue command when not authenticated")
    ...
    val error = result.left.getOrElse("")
    assert(
      error.contains("glab is not authenticated"),
      s"Error should mention glab not authenticated: $error"
    )
  ...
  test("fetchIssue executes command with correct arguments"):
    ...
    val capturedArgs =
      new java.util.concurrent.atomic.AtomicReference[Array[String]](
        Array.empty[String]
      )
    val mockIsCommandAvailable = (cmd: String) => true
    val mockExec: (String, Array[String]) => Either[String, String] =
  ...
  test("fetchIssue returns Left when command execution fails"):
    ...
    assert(result.isLeft)
    assert(
      result.left.getOrElse("").contains("Failed to fetch issue"),
      "Error should mention failed to fetch"
    )
  ...
  test("fetchIssue returns Left when JSON parsing fails"):
    ...
    assert(result.isLeft)
    assert(
      result.left.getOrElse("").contains("Failed to parse"),
  ...
  test(
    "buildCreateIssueCommand generates correct glab CLI arguments for Bug type"
  ):
  ...
  test(
    "buildCreateIssueCommand generates correct glab CLI arguments for Feature type"
  ):
    val args = GitLabClient.buildCreateIssueCommand(
  ...
  test(
    "buildCreateIssueCommandWithoutLabel generates command without label flag"
  ):
  ...
  test("parseCreateIssueResponse handles URL with trailing newline"):
    ...
    assertEquals(
      issue.url.trim,
      "https://gitlab.e-bs.cz/owner/project/-/issues/789"
    )
  ...
  test("parseCreateIssueResponse parses work_items URL"):
    ...
    assertEquals(
      issue.url,
      "https://gitlab.com/fifty-forms/platform/-/work_items/210"
    )
  ...
  test("createIssue success path with label"):
    ...
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        ...
        else Left(s"Unexpected command: $cmd ${args.mkString(" ")}")
  ...
  test("createIssue retries without label on label error"):
    val commandCalls = new java.util.concurrent.atomic.AtomicInteger(0)
    val lastArgs =
      new java.util.concurrent.atomic.AtomicReference[Array[String]](Array())
    val mockIsCommandAvailable = (cmd: String) => true
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        if args.contains("auth") && args.contains("status") then
  ...
  test(
    "buildCreateMrCommand uses --source-branch and --target-branch for branch specification"
  ):
    val args = GitLabClient.buildCreateMrCommand(
      repository = "owner/project",
  ...
  test(
    "buildMergeMrCommand produces correct glab mr merge argument array with MR URL"
  ):
    val args = GitLabClient.buildMergeMrCommand(
      "https://gitlab.com/owner/project/-/merge_requests/42"
    )

    assert(args.contains("mr"), "Should contain 'mr'")
    assert(args.contains("merge"), "Should contain 'merge'")
    assert(
      args.contains("https://gitlab.com/owner/project/-/merge_requests/42"),
      "Should contain MR URL"
    )
  ...
  test(
    "createMergeRequest invokes execCommand with buildCreateMrCommand result and returns MR URL"
  ):
    val capturedArgs =
      new java.util.concurrent.atomic.AtomicReference[Array[String]](
        Array.empty
  ...
  test("mergeMergeRequest returns Left when glab is not authenticated"):
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        if args.contains("auth") && args.contains("status") then
          Left("not logged in")
        else Right("should not get here")
  ...
  test(
    "mergeMergeRequest invokes execCommand with buildMergeMrCommand and returns Right(()) on success"
  ):
    val capturedArgs =
      new java.util.concurrent.atomic.AtomicReference[Array[String]](
        Array.empty
  ...
  test("parseGlabJobsJson -- some jobs failed: failed status maps to Failed"):
    ...
    val checks = result.getOrElse(fail("Expected Right"))
    val lint =
      checks.find(_.name == "lint").getOrElse(fail("Expected lint check"))
  ...
  test("parseGlabJobsJson -- canceled (American) maps to Cancelled"):
    val json =
      """[{"id": 1, "name": "build", "status": "canceled", "web_url": "https://gitlab.com/-/jobs/1"}]"""
  ...
  test("parseGlabJobsJson -- cancelled (British) also maps to Cancelled"):
    val json =
      """[{"id": 1, "name": "build", "status": "cancelled", "web_url": "https://gitlab.com/-/jobs/1"}]"""
  ...
  test(
    "parseGlabJobsJson -- additional pending states: created, waiting_for_resource, preparing, manual"
  ):
    val statuses =
      List("created", "waiting_for_resource", "preparing", "manual")
    statuses.foreach { s =>
      val json =
        s"""[{"id": 1, "name": "job", "status": "$s", "web_url": "https://gitlab.com/-/jobs/1"}]"""
      val result = GitLabClient.parseGlabJobsJson(json)
      assert(result.isRight, s"Expected Right for status $s")
      assertEquals(
        result.getOrElse(Nil).head.status,
        CICheckStatus.Pending,
        s"Expected Pending for status $s"
      )
    }

  test("parseGlabJobsJson -- skipped maps to Passed (should not block)"):
    val json =
      """[{"id": 1, "name": "build", "status": "skipped", "web_url": "https://gitlab.com/-/jobs/1"}]"""
  ...
  test("parseGlabJobsJson -- unknown status string maps to Unknown"):
    val json =
      """[{"id": 1, "name": "build", "status": "something_weird", "web_url": "https://gitlab.com/-/jobs/1"}]"""
  ...
  test("parseGlabJobsJson -- entry missing name field returns Left"):
    val json =
      """[{"id": 1, "status": "success", "web_url": "https://gitlab.com/-/jobs/1"}]"""
  ...
  test("parseGlabJobsJson -- job URL present returns Some(url)"):
    ...
    assert(result.isRight)
    assertEquals(
      result.getOrElse(Nil).head.url,
      Some("https://gitlab.com/-/jobs/1")
    )

  test("parseGlabJobsJson -- empty web_url returns None"):
    val json =
      """[{"id": 1, "name": "test", "status": "success", "web_url": ""}]"""
  ...
  test(
    "buildCheckStatusesCommand produces correct glab CLI args for pipelines API"
  ):
    val args =
      GitLabClient.buildCheckStatusesCommand(42, "test-group/test-repo")
  ...
  test("buildPipelineJobsCommand produces correct glab CLI args for jobs API"):
    val args =
      GitLabClient.buildPipelineJobsCommand(100, "test-group/test-repo")
  ...
  test(
    "buildMergeMrWithDeleteCommand includes --remove-source-branch and MR URL"
  ):
    val args = GitLabClient.buildMergeMrWithDeleteCommand(
      "https://gitlab.com/owner/project/-/merge_requests/42"
    )
  ...
  test("fetchCheckStatuses -- returns parsed checks on success"):
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        if args.contains("auth") && args.contains("status") then
          Right("Logged in to gitlab.com")
        else if args.exists(_.contains("pipelines")) && !args.exists(
            _.contains("jobs")
          )
        then
          Right(
            """[{"id": 100, "status": "success", "ref": "feature", "sha": "abc"}]"""
          )
        else if args.exists(_.contains("jobs")) then
          Right(
            """[{"id": 1, "name": "test", "status": "success", "web_url": "https://gitlab.com/-/jobs/1"}]"""
          )
        else Left(s"Unexpected args: ${args.mkString(" ")}")
  ...
  test("fetchCheckStatuses -- returns Left when pipelines command fails"):
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        ...
        then Left("API error")
        else Left("Should not reach jobs call")
  ...
  test("fetchCheckStatuses -- returns Right(Nil) when no pipelines found"):
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        ...
        then Right("[]")
        else Left("Should not reach jobs call when no pipelines")
