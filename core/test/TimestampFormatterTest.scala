...
class TimestampFormatterTest extends munit.FunSuite:
  ...
  test(
    "formatUpdateTimestamp returns 'Updated X seconds ago' for exactly 30s"
  ) {
    val now = Instant.now()
  ...
  test(
    "formatUpdateTimestamp returns 'Updated X seconds ago' for exactly 59s"
  ) {
    val now = Instant.now()
  ...
  test(
    "formatUpdateTimestamp returns 'Updated X minutes ago' for 2-59 minutes"
  ) {
    val now = Instant.now()
  ...
  test(
    "formatUpdateTimestamp returns 'Updated 1 hour ago' for 60-119 minutes"
  ) {
    val now = Instant.now()
