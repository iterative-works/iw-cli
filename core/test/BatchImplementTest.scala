...
class BatchImplementTest extends FunSuite:
  ...
  test("resolveWorkflowCode waterfall returns Right(wf)"):
    assertEquals(
      BatchImplement.resolveWorkflowCode(Some("waterfall")),
      Right("wf")
    )

  test("resolveWorkflowCode diagnostic returns Right(dx)"):
    assertEquals(
      BatchImplement.resolveWorkflowCode(Some("diagnostic")),
      Right("dx")
    )
  ...
  test(
    "markPhaseComplete unchecked phase returns content with that phase checked"
  ):
    val result = BatchImplement.markPhaseComplete(sampleTasks, 1)
    result match
      case Right(content) =>
        assert(
          content.contains("- [x] Phase 1:"),
          s"Expected checked Phase 1 in: $content"
        )
  ...
  test("markPhaseComplete with multiple phases only checks the target phase"):
    ...
    result match
      case Right(content) =>
        assert(
          content.contains("- [x] Phase 2:"),
          s"Expected Phase 2 checked in: $content"
        )
        assert(
          content.contains("- [ ] Phase 1:"),
          s"Expected Phase 1 still unchecked in: $content"
        )
      case Left(err) => fail(s"Expected Right but got Left($err)")
    ...
    result match
      case Right(content) =>
        assert(
          ...
          "Phase 2 should remain unchecked"
        )
        assert(
          content.contains("- [x] Phase 3:"),
          "Phase 3 should remain checked"
        )
  ...
  test("markPhaseComplete handles zero-padded phase numbers"):
    ...
    result match
      case Right(content) =>
        assert(
          content.contains("- [x] Phase 01:"),
          s"Expected checked Phase 01 in: $content"
        )
      case Left(err) => fail(s"Expected Right but got Left($err)")

  test(
    "markPhaseComplete with zero-padded numbers only checks the target phase"
  ):
    val result = BatchImplement.markPhaseComplete(zeroPaddedTasks, 2)
    result match
      case Right(content) =>
        assert(
          content.contains("- [x] Phase 02:"),
          s"Expected Phase 02 checked in: $content"
        )
        assert(
          content.contains("- [ ] Phase 01:"),
          s"Expected Phase 01 still unchecked in: $content"
        )
      case Left(err) => fail(s"Expected Right but got Left($err)")
