...
class WorktreeListViewTest extends munit.FunSuite:
  ...
  test(
    "WorktreeListView renders review section when reviewState provided with artifacts"
  ):
    val reviewState = Some(
      Right(
        ReviewState(
          display = None,
          badges = None,
          ...
          artifacts = List(
            ReviewArtifact(
              "Analysis",
              "project-management/issues/IWLE-123/analysis.md"
            ),
            ReviewArtifact(
              "Context",
              "project-management/issues/IWLE-123/phase-01-context.md"
            )
          )
        )
      )
    ...
    val worktreesWithData = List(
      (
        sampleWorktree,
        Some((sampleIssueData, false, false)),
        None,
        None,
        None,
        reviewState
    ...
    assert(
      htmlStr.contains("Review Artifacts"),
      s"Should contain 'Review Artifacts' heading"
    )
    assert(
      htmlStr.contains("review-artifacts"),
      s"Should contain 'review-artifacts' class"
    )
    assert(
      htmlStr.contains("artifact-list"),
      s"Should contain 'artifact-list' class"
    )
    assert(
      htmlStr.contains("Analysis"),
      s"Should contain artifact label 'Analysis'"
    )
    assert(
      htmlStr.contains("Context"),
      s"Should contain artifact label 'Context'"
    )
  ...
  test("WorktreeListView omits review section when reviewState is None"):
    ...
    assert(
      !htmlStr.contains("Review Artifacts"),
      s"Should not contain 'Review Artifacts' when reviewState is None"
    )
    assert(
      !htmlStr.contains("review-artifacts"),
      s"Should not contain 'review-artifacts' class"
    )
    assert(
      !htmlStr.contains("artifact-list"),
      s"Should not contain 'artifact-list' class"
    )
  ...
  test("WorktreeListView omits review section when artifacts list is empty"):
    ...
    assert(
      !htmlStr.contains("Review Artifacts"),
      s"Should not contain 'Review Artifacts' when artifacts list is empty"
    )
    assert(
      !htmlStr.contains("review-artifacts"),
      s"Should not contain 'review-artifacts' class"
  ...
  test("WorktreeListView displays all artifact labels correctly"):
    ...
    assert(
      htmlStr.contains("Analysis Doc"),
      s"Should contain artifact label 'Analysis Doc'"
    )
    assert(
      htmlStr.contains("Phase Context"),
      s"Should contain artifact label 'Phase Context'"
    )
    assert(
      htmlStr.contains("Review Packet"),
      s"Should contain artifact label 'Review Packet'"
    )
    // Verify artifacts are clickable links
    assert(htmlStr.contains("<a"), s"Artifacts should be links")
    assert(
      htmlStr.contains("/worktrees/"),
      s"Artifact links should point to worktree artifacts route"
    )
    assert(
      htmlStr.contains("?path="),
      s"Artifact links should include path query parameter"
    )
  ...
  test("WorktreeListView renders empty state when no worktrees"):
    ...
    assert(
      htmlStr.contains("empty-state"),
      s"Should contain 'empty-state' class"
    )
    assert(
      htmlStr.contains("No worktrees registered yet"),
      s"Should show empty state message"
    )

  test(
    "WorktreeListView renders multiple worktrees with different review states"
  ):
    val wt1 = sampleWorktree.copy(issueId = "IWLE-1")
    val wt2 = sampleWorktree.copy(issueId = "IWLE-2")

    val reviewState1 = Some(
      Right(
        ReviewState(
          display = None,
          badges = None,
    ...
    assert(
      htmlStr.contains("Doc1"),
      s"Should contain artifact for first worktree"
    )
    ...
    assertEquals(
      reviewArtifactsCount,
      1,
      s"Should have exactly one 'Review Artifacts' section"
    )
  ...
  test("renderWorktreeCard includes display badge when display is defined"):
    val reviewState = Some(
      Right(
        ReviewState(
          display =
            Some(Display("Implementing", Some("Phase 2 of 3"), "progress")),
          badges = None,
          taskLists = None,
          needsAttention = None,
          message = None,
    ...
    assert(
      htmlStr.contains("review-status"),
      "Should contain review-status class"
    )
    assert(
      htmlStr.contains("display-type-progress"),
      "Should contain display type CSS class"
    )
  ...
  test(
    "renderWorktreeCard includes display badge with correct class for success type"
  ):
    val reviewState = Some(
      Right(
        ReviewState(
    ...
    assert(
      htmlStr.contains("display-type-success"),
      "Should contain success display type class"
    )
    assert(htmlStr.contains("Complete"), "Should contain Complete label")

  test(
    "renderWorktreeCard includes display badge with correct class for warning type"
  ):
    val reviewState = Some(
      Right(
        ReviewState(
          display =
            Some(Display("Awaiting Review", Some("Phase 3 of 4"), "warning")),
          badges = None,
          taskLists = None,
          needsAttention = None,
          message = None,
    ...
    assert(
      htmlStr.contains("display-type-warning"),
      "Should contain warning display type class"
    )
    assert(
      htmlStr.contains("Awaiting Review"),
      "Should contain Awaiting Review label"
    )
  ...
  test("renderWorktreeCard omits display badge when display is None"):
    ...
    assert(
      !htmlStr.contains("display-type-"),
      "Should not contain display type classes when display is None"
    )
    assert(
      !htmlStr.contains("review-status-label"),
      "Should not contain status label when display is None"
    )
  ...
  test("renderWorktreeCard renders badges array when badges are defined"):
    ...
    assert(
      htmlStr.contains("review-badges"),
      "Should contain review-badges class"
    )
    assert(htmlStr.contains("TDD"), "Should contain TDD badge label")
    assert(htmlStr.contains("Batch"), "Should contain Batch badge label")
    assert(
      htmlStr.contains("display-type-success"),
      "Should contain success type class for TDD"
    )
    assert(
      htmlStr.contains("display-type-info"),
      "Should contain info type class for Batch"
    )
  ...
  test("renderWorktreeCard omits badges section when badges is None"):
    ...
    assert(
      !htmlStr.contains("review-badges"),
      "Should not contain review-badges class when badges is None"
    )
  ...
  test("renderWorktreeCard includes message when message is defined"):
    ...
    assert(
      htmlStr.contains("review-message"),
      "Should contain review-message class"
    )
    assert(
      htmlStr.contains("Phase 3 complete - Ready for review"),
  ...
  test("renderWorktreeCard omits message when message is None"):
    ...
    assert(
      !htmlStr.contains("review-message"),
      "Should not contain review-message class when message is None"
    )
  ...
  test("renderWorktreeCard displays display, badges, and message together"):
    val reviewState = Some(
      Right(
        ReviewState(
          display =
            Some(Display("In Review", Some("Phase 3 of 4"), "progress")),
  ...
  test(
    "renderWorktreeCard handles missing display, badges, and message gracefully"
  ):
    val reviewState = Some(
      Right(
        ReviewState(
    ...
    assert(
      htmlStr.contains("Review Artifacts"),
      "Should contain Review Artifacts heading"
    )
    ...
    assert(
      !htmlStr.contains("display-type-"),
      "Should not render display badge"
    )
  ...
  test("renderWorktreeCard displays partial fields correctly (only display)"):
    ...
    assert(
      !htmlStr.contains("review-badges"),
      "Should not render badges when None"
    )
    assert(
      !htmlStr.contains("review-message"),
      "Should not render message when None"
    )
  ...
  test("displayTypeClass maps display types to CSS classes"):
    assertEquals(
      WorktreeListView.displayTypeClass("progress"),
      "display-type-progress"
    )
    assertEquals(
      WorktreeListView.displayTypeClass("success"),
      "display-type-success"
    )
    assertEquals(
      WorktreeListView.displayTypeClass("warning"),
      "display-type-warning"
    )
    assertEquals(
      WorktreeListView.displayTypeClass("error"),
      "display-type-error"
    )
