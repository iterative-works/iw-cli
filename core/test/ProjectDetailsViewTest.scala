...
class ProjectDetailsViewTest extends FunSuite:
  ...
  test("render includes breadcrumb with link to root"):
    ...
    assert(
      html.contains("breadcrumb") || html.contains("nav"),
      "Should have breadcrumb or nav element"
    )
  ...
  test("render includes tracker URL link when available"):
    ...
    assert(
      html.contains(
        "href=\"https://github.com/iterative-works/iw-cli/issues\""
      ),
      "Should include tracker URL as link"
    )
  ...
  test("render shows empty state when no worktrees"):
    ...
    assert(
      html.contains("No worktrees") || html.contains("empty"),
  ...
  test("render cards have HTMX polling attributes"):
    ...
    assert(
      html.contains("hx-get=\"/worktrees/IW-79/card\""),
      "Should have hx-get attribute for polling"
    )
    assert(
      html.contains("hx-trigger") && html.contains("30s"),
      "Should have hx-trigger with polling interval"
    )
  ...
  test("worktree-list div has hx-get for project-scoped changes endpoint"):
    ...
    assert(
      html.contains("hx-get=\"/api/projects/iw-cli/worktrees/changes\""),
      "Should have hx-get pointing to project-scoped changes endpoint"
    )
  ...
  test("worktree-list div has hx-trigger with polling interval"):
    ...
    assert(
      html.contains("hx-trigger=\"every 30s\""),
      "Should have hx-trigger with 30s polling interval"
    )
  ...
  test("worktree-list div has hx-swap none for OOB swaps"):
    ...
    assert(
      ...
      "Should have hx-swap=none for OOB swaps"
    )
  ...
  test("worktree-list div has hx-vals with JS expression for card IDs"):
    ...
    assert(
      html.contains("hx-vals=\"js:{have:"),
      "Should have hx-vals with JS expression for card IDs"
    )
  ...
  test("renderNotFound includes project name"):
    ...
    assert(
      html.contains("nonexistent-project"),
  ...
  test("renderNotFound includes breadcrumb"):
    ...
    assert(
      html.contains("breadcrumb") || html.contains("Projects"),
      "Should include breadcrumb or Projects link"
    )
  ...
  test("renderNotFound mentions registration"):
    ...
    assert(
      html.contains("not registered") || html.contains("register"),
  ...
  test("render with empty worktrees includes Create Worktree button"):
    ...
    assert(
      html.contains("Create Worktree"),
      "Empty state should include Create Worktree button"
    )
