...
import iw.core.adapters.{GitHubClient, CreatedIssue}
...
class GitHubClientTest extends munit.FunSuite:
  ...
  test("parseCreateIssueResponse parses valid URL"):
    ...
    assertEquals(
      result,
      Right(CreatedIssue("132", "https://github.com/owner/repo/issues/132"))
    )
  ...
  test("parseCreateIssueResponse handles URL with trailing newline"):
    ...
    assertEquals(
      result,
      Right(CreatedIssue("456", "https://github.com/owner/repo/issues/456"))
    )
  ...
  test("createIssue retries without label on label error"):
    ...
    assertEquals(
      result,
      Right(CreatedIssue("42", "https://github.com/owner/repo/issues/42"))
    )
  ...
  test("validateGhPrerequisites returns GhNotInstalled when gh not found"):
    ...
    assertEquals(
      ...
      GitHubClient.GhPrerequisiteError.GhNotInstalled
    )

  test(
    "validateGhPrerequisites returns GhNotAuthenticated when auth status fails with exit code 4"
  ):
    val mockIsCommandAvailable = (cmd: String) => true
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
    ...
    assertEquals(
      result.left.getOrElse(fail("Expected Left")),
      GitHubClient.GhPrerequisiteError.GhNotAuthenticated
    )
  ...
  test("parseFetchIssueResponse parses complete valid JSON"):
    val json =
      """{"number": 132, "title": "Add feature", "state": "OPEN", "assignees": [{"login": "user1"}], "body": "Description here"}"""
  ...
  test("parseFetchIssueResponse handles empty assignees array"):
    val json =
      """{"number": 132, "title": "Add feature", "state": "OPEN", "assignees": [], "body": "Description"}"""
  ...
  test("parseFetchIssueResponse handles null body"):
    val json =
      """{"number": 132, "title": "Add feature", "state": "OPEN", "assignees": [], "body": null}"""
    val result = GitHubClient.parseFetchIssueResponse(json, "132")
  ...
  test("parseFetchIssueResponse uses first assignee when multiple exist"):
    val json =
      """{"number": 132, "title": "Add feature", "state": "OPEN", "assignees": [{"login": "user1"}, {"login": "user2"}], "body": "Description"}"""
    val result = GitHubClient.parseFetchIssueResponse(json, "132")
  ...
  test("parseFetchIssueResponse maps CLOSED state to lowercase"):
    val json =
      """{"number": 132, "title": "Add feature", "state": "CLOSED", "assignees": [], "body": null}"""
    val result = GitHubClient.parseFetchIssueResponse(json, "132")
  ...
  test("parseFetchIssueResponse uses bare issue number as ID"):
    val json =
      """{"number": 999, "title": "Test", "state": "OPEN", "assignees": [], "body": null}"""
  ...
  test("parseFetchIssueResponse returns error for missing title field"):
    val json =
      """{"number": 132, "state": "OPEN", "assignees": [], "body": null}"""
    val result = GitHubClient.parseFetchIssueResponse(json, "132")
  ...
  test("parseFetchIssueResponse returns error for missing state field"):
    val json =
      """{"number": 132, "title": "Test", "assignees": [], "body": null}"""
    val result = GitHubClient.parseFetchIssueResponse(json, "132")
  ...
  test("fetchIssue validates prerequisites first - gh not installed"):
    val mockIsCommandAvailable = (cmd: String) => false
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) => fail("Should not execute command when gh not installed")
    ...
    val error = result.left.getOrElse("")
    assert(
      error.contains("gh CLI is not installed"),
      s"Error should mention gh not installed: $error"
    )
  ...
  test("fetchIssue validates prerequisites first - gh not authenticated"):
    ...
    val error = result.left.getOrElse("")
    assert(
      error.contains("gh is not authenticated"),
      s"Error should mention gh not authenticated: $error"
    )
    ...
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        if args.contains("auth") && args.contains("status") then
          ...
          Right(
            """{"number": 132, "title": "Test", "state": "OPEN", "assignees": [], "body": null}"""
          )
    ...
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        if args.contains("auth") && args.contains("status") then
          ...
          Right(
            """{"number": 132, "title": "Add feature", "state": "OPEN", "assignees": [{"login": "user1"}], "body": "Description"}"""
          )
  ...
  test(
    "parseListRecentIssuesResponse parses valid JSON array with multiple issues"
  ):
    val json = """[
  ...
  test("searchIssues success case with mocked command"):
    ...
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        if args.contains("auth") && args.contains("status") then
          Right("Logged in")
        else if args.contains("issue") && args.contains("list") && args
            .contains("--search")
        then
          Right("""[
  ...
  test(
    "buildCreatePrCommand includes --repo --head --base --title --body flags with correct values"
  ):
    val args = GitHubClient.buildCreatePrCommand(
      repository = "iterative-works/iw-cli",
  ...
  test(
    "buildMergePrCommand produces correct gh pr merge argument array with --merge flag"
  ):
    val args =
      GitHubClient.buildMergePrCommand("https://github.com/owner/repo/pull/42")

    assert(args.contains("pr"), "Should contain 'pr'")
    assert(args.contains("merge"), "Should contain 'merge'")
    assert(args.contains("--merge"), "Should contain '--merge' flag")
    assert(
      args.contains("https://github.com/owner/repo/pull/42"),
  ...
  test(
    "createPullRequest invokes execCommand with buildCreatePrCommand result and returns PR URL"
  ):
    val capturedArgs =
      new java.util.concurrent.atomic.AtomicReference[Array[String]](
        Array.empty
  ...
  test(
    "mergePullRequest invokes execCommand with buildMergePrCommand and returns Right(()) on success"
  ):
    val capturedArgs =
      new java.util.concurrent.atomic.AtomicReference[Array[String]](
        Array.empty
  ...
  test(
    "buildMergePrWithDeleteCommand builds command with --merge and --delete-branch"
  ):
  ...
  private def fetchWithJson(
      json: String
  ): Either[String, List[iw.core.model.CICheckResult]] =
    val mockExec: (String, Array[String]) => Either[String, String] =
      (cmd, args) =>
        if args.contains("auth") && args.contains("status") then
          Right("Logged in")
  ...
  test("fetchCheckStatuses maps SUCCESS, FAILURE, PENDING to correct statuses"):
    ...
    val checks = result.toOption.get
    assertEquals(
      checks.find(_.name == "test").map(_.status),
      Some(iw.core.model.CICheckStatus.Passed)
    )
    assertEquals(
      checks.find(_.name == "lint").map(_.status),
      Some(iw.core.model.CICheckStatus.Failed)
    )
    assertEquals(
      checks.find(_.name == "build").map(_.status),
      Some(iw.core.model.CICheckStatus.Pending)
    )

  test("fetchCheckStatuses maps CANCELLED state to CICheckStatus.Cancelled"):
    val json =
      """[{"bucket":"fail","link":"","name":"deploy","state":"CANCELLED"}]"""
    val result = fetchWithJson(json)
    assert(result.isRight)
    assertEquals(
      result.toOption.get.head.status,
      iw.core.model.CICheckStatus.Cancelled
    )

  test("fetchCheckStatuses maps unknown state string to CICheckStatus.Unknown"):
    val json =
      """[{"bucket":"pending","link":"","name":"whatever","state":"SKIPPED"}]"""
    val result = fetchWithJson(json)
    assert(result.isRight)
    assertEquals(
      result.toOption.get.head.status,
      iw.core.model.CICheckStatus.Unknown
    )
  ...
  test("fetchCheckStatuses returns Left for array entry missing name field"):
    val json =
      """[{"bucket":"pass","link":"https://ci.example.com/1","state":"SUCCESS"}]"""
    val result = fetchWithJson(json)
    assert(result.isLeft)
  ...
  test(
    "fetchCheckStatuses check with non-empty link returns Some(link) for url"
  ):
    val link = "https://ci.example.com/run/42"
    val json =
      s"""[{"bucket":"pass","link":"$link","name":"test","state":"SUCCESS"}]"""
    val result = fetchWithJson(json)
    assert(result.isRight)
