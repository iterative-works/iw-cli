...
class ScalafmtChecksTest extends FunSuite:
  ...
  test("checkVersionExistsWith detects version with various formats"):
    ...
    assertEquals(
      ScalafmtChecks.checkVersionExistsWith(config, readFile1),
    ...
    assertEquals(
      ScalafmtChecks.checkVersionExistsWith(config, readFile2),
      CheckResult.Success("Configured")
    )
    ...
    assertEquals(
      ScalafmtChecks.checkVersionExistsWith(config, readFile3),
      CheckResult.Success("Configured")
    )
