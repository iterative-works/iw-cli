...
class FixPromptTest extends munit.FunSuite:
  test("prompt includes all failed check names"):
    ...
    val prompt =
      FixPrompt.generate(failures, BuildSystem.Mill, "GitHub Actions")
  ...
  test("empty failures returns empty prompt"):
    ...
    assertEquals(
      prompt,
      "",
      "Prompt should be empty when there are no failures"
    )
  ...
  test("Mill build system uses mill commands"):
    ...
    assert(
      prompt.contains("./mill __.checkFormat"),
      "Should use mill checkFormat command"
    )
    assert(
      prompt.contains("./mill __.reformat"),
      "Should use mill reformat command"
    )
  ...
  test("SBT build system uses sbt commands"):
    ...
    assert(
      prompt.contains("sbt scalafmtCheckAll"),
      "Should use sbt scalafmtCheckAll"
    )
  ...
  test("scala-cli build system uses scala-cli commands"):
    ...
    assert(
      prompt.contains("scala-cli fmt --check"),
      "Should use scala-cli fmt --check"
    )
  ...
  test("Unknown build system uses placeholder commands"):
    ...
    assert(
      prompt.contains("<check-format-command>"),
      "Should use placeholder commands"
    )
  ...
  test("scalafmt guidance includes Scala 3 config"):
    ...
    assert(
      prompt.contains("runner.dialect = scala3"),
      "Should include Scala 3 dialect"
    )
    assert(prompt.contains("maxColumn = 100"), "Should include maxColumn")
    assert(
      prompt.contains("rewrite.scala3.convertToNewSyntax"),
      "Should include Scala 3 rewrites"
    )

  test("scalafmt version check triggers same guidance as config check"):
    val configPrompt = FixPrompt.generate(
      List(".scalafmt.conf"),
      BuildSystem.Mill,
      "GitHub Actions"
    )
    val versionPrompt = FixPrompt.generate(
      List(".scalafmt.conf version"),
      BuildSystem.Mill,
      "GitHub Actions"
    )
  ...
  test("scalafix guidance includes DisableSyntax rules"):
    ...
    assert(
      prompt.contains("DisableSyntax"),
      "Should include DisableSyntax rule"
    )
  ...
  test("scalafix guidance mentions Mill plugin for Mill builds"):
    ...
    assert(
      prompt.contains("ScalafixModule"),
      "Should mention ScalafixModule trait for Mill"
    )
    assert(
      prompt.contains("mill-scalafix"),
      "Should mention mill-scalafix plugin"
    )
  ...
  test("scalafix guidance mentions sbt plugin for SBT builds"):
    ...
    assert(
      prompt.contains("sbt-scalafix"),
      "Should mention sbt-scalafix plugin for SBT"
    )
  ...
  test("git hooks pre-commit does format check"):
    ...
    assert(
      prompt.contains("./mill __.checkFormat"),
      "Pre-commit should run format check"
    )
    assert(
      prompt.contains("./mill __.reformat"),
      "Should mention reformat as fix"
    )

  test("git hooks pre-push does compile warnings and tests"):
    val prompt = FixPrompt.generate(
      List("Git hooks installed"),
      BuildSystem.Mill,
      "GitHub Actions"
    )
    ...
    assert(
      ...
      "Pre-push should check for compile warnings"
    )
  ...
  test("git hooks guidance includes installation command"):
    ...
    assert(
      prompt.contains("git config core.hooksPath .git-hooks"),
      "Should include hook install command"
    )
  ...
  test("GitHub Actions CI guidance includes workflow structure"):
    ...
    assert(
      prompt.contains("needs: compile"),
      "Test job should depend on compile"
    )
  ...
  test("GitLab CI guidance uses stages"):
    val prompt =
      FixPrompt.generate(List("CI workflow"), BuildSystem.Mill, "GitLab CI")
  ...
  test("unknown CI platform provides generic guidance"):
    val prompt =
      FixPrompt.generate(List("CI workflow"), BuildSystem.Mill, "Unknown")
  ...
  test("contributing guidance covers required sections"):
    ...
    assert(
      prompt.contains("Local Development"),
      "Should mention Local Development section"
    )
    assert(
      prompt.contains("Troubleshooting"),
      "Should mention Troubleshooting section"
    )
  ...
  test("contributing guidance includes local commands"):
    ...
    assert(
      prompt.contains("./mill __.checkFormat"),
      "Should list format check command"
    )
