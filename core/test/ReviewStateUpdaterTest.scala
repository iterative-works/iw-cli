...
class ReviewStateUpdaterTest extends munit.FunSuite:
  ...
  test("replace array (artifacts) replaces entire array"):
    ...
    val input = ReviewStateUpdater.UpdateInput(
      artifacts =
        Some(List(("New1", "new1.md", None), ("New2", "new2.md", None))),
  ...
  test("merge clear wins over activity value when both provided"):
    ...
    val parsed = ujson.read(result)
    assert(
      !parsed.obj.contains("activity"),
      "activity should be removed when clear wins"
    )
  ...
  test("merge removes workflow_type when clearWorkflowType is true"):
    ...
    assert(
      ...
      "workflow_type should be removed"
    )
  ...
  test("merge clear wins over workflowType value when both provided"):
    ...
    val input = ReviewStateUpdater.UpdateInput(
      ...
      clearWorkflowType = true
    )
    val result = ReviewStateUpdater.merge(existing, input)
    val parsed = ujson.read(result)
    assert(
      !parsed.obj.contains("workflow_type"),
      "workflow_type should be removed when clear wins"
    )

  // --- Preservation ---

  test(
    "existing activity and workflow_type preserved when not mentioned in update"
  ):
    val existing = """{
      "version": 2,
      "issue_id": "IW-1",
  ...
  test("merged result passes ReviewStateValidator"):
    ...
    assert(
      validation.isValid,
      s"Merged result should be valid, but got errors: ${validation.errors}"
    )
