...
object ServerClient:
  ...
  /** Check if server communication is disabled via environment variable. Useful
    * for testing to prevent tests from registering worktrees in production
    * dashboard.
    */
  private def isServerDisabled: Boolean =
    Option(System.getenv("IW_SERVER_DISABLED")).exists(v =>
  ...
  /** Checks if the server is healthy.
    *
    * @param port
    *   The port to check (uses config file if not specified)
    * @return
    *   true if server responds with 200 OK to /health
    */
  ...
  /** Ensures the server is running, starting it if necessary. Uses
    * ProcessManager to spawn a persistent background process.
    *
    * @param statePath
    *   Path to state.json file for server
    * @return
    *   Right(()) if server is running, Left(error) if start fails
    */
  private def ensureServerRunning(statePath: String): Either[String, Unit] =
    val port = getServerPort()
    if isHealthy() then Right(())
    else
      ...
      ProcessManager.readPidFile(pidPath) match
        case Right(Some(pid)) if ProcessManager.isProcessAlive(pid) =>
          ...
          if waitForServer(port, timeoutSeconds = 5) then Right(())
          else Left("Server process exists but is not responding")
        case _ =>
          // No running process, spawn a new one
          val config = getServerConfig()
          ProcessManager.spawnServerProcess(
            statePath,
            config.port,
            config.hosts
          ) match
            ...
            case Right(pid)  =>
              ...
              if waitForServer(port, timeoutSeconds = 5) then Right(())
              else Left("Server failed to start within timeout")
  ...
  /** Registers or updates a worktree with the server.
    *
    * @param issueId
    *   The issue identifier (e.g., "IWLE-123")
    * @param path
    *   The filesystem path to the worktree
    * @param trackerType
    *   The tracker system (e.g., "Linear", "YouTrack")
    * @param team
    *   The team identifier
    * @param statePath
    *   Path to state.json for lazy start (default:
    *   ~/.local/share/iw/server/state.json)
    * @return
  ...
  def registerWorktree(
      ...
      team: String,
      statePath: String =
        s"${System.getProperty("user.home")}/.local/share/iw/server/state.json"
  ): Either[String, Unit] =
        ...
        case Right(_)    =>
          ...
          try
            ...
            response.code match
              ...
              case _                                  =>
                val errorMsg = Try(ujson.read(response.body)("message").str)
                  .getOrElse(response.body)
                Left(s"Server returned ${response.code.code}: $errorMsg")

          catch
            case e: Exception =>
              Left(s"Failed to register worktree: ${e.getMessage}")

  /** Updates the lastSeenAt timestamp for an existing worktree.
    *
    * This reuses registerWorktree since the PUT endpoint handles updates. The
    * endpoint will update lastSeenAt and preserve registeredAt.
    *
    * @param issueId
    ...
    *   The team identifier
    * @param statePath
    *   Path to state.json for lazy start
    * @return
    *   Right(()) on success, Left(error message) on failure
    */
  def updateLastSeen(
  ...
  /** Registers or updates a project with the server.
    ...
    * @param trackerType
    *   The tracker system (e.g., "GitHub", "Linear")
    * @param team
    *   The team identifier
    * @param trackerUrl
    *   Optional URL to the issue tracker
    * @param statePath
    *   Path to state.json for lazy start (default:
    *   ~/.local/share/iw/server/state.json)
    * @return
      ...
      ensureServerRunning(statePath) match
        ...
        case Right(_)    =>
          ...
          try
            ...
            case e: Exception =>
              Left(s"Failed to register project: ${e.getMessage}")
      ...
      try
        ...
        response.code match
          ...
          case StatusCode.NotFound =>
            Right(()) // Already removed - treat as success
          case _ =>
            val errorMsg = Try(ujson.read(response.body)("message").str)
              .getOrElse(response.body)
            Left(s"Server returned ${response.code.code}: $errorMsg")
        ...
        case e: Exception =>
          Left(s"Failed to unregister worktree: ${e.getMessage}")

  /** Fetches detailed worktree status from the server.
    *
    * Queries the server for live git state, filesystem-based review/progress
    * data, and cached issue/PR data. Does not auto-start the server — if the
    * server is not running, returns a helpful error message.
    *
    * @param issueId
    *   The issue identifier (e.g., "IWLE-123")
    * @return
    *   Right(WorktreeStatus) on success, Left(error message) on failure
    */
    ...
    if isServerDisabled then Left("Server communication is disabled")
      ...
      try
        ...
        response.code match
          case StatusCode.Ok =>
            Try(upickle.default.read[WorktreeStatus](response.body)) match
              ...
              case Failure(e)      =>
                Left(s"Failed to parse status response: ${e.getMessage}")
          case StatusCode.NotFound =>
            val msg = Try(ujson.read(response.body)("message").str)
              .getOrElse("Worktree not found")
            Left(msg)
          case _ =>
            val errorMsg = Try(ujson.read(response.body)("message").str)
              .getOrElse(response.body)
        ...
        case e: java.net.ConnectException =>
          Left(
            "Dashboard server is not running. Start it with: iw server start"
          )
        case e: Exception =>
