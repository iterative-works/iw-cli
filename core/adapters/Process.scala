...
object ProcessAdapter:
  ...
  def commandExists(command: String): Boolean =
    ...
    if !SafeCommandPattern.matches(command) then false
    else
      try
        val result = os
          .proc("which", command)
  ...
  def run(
    ...
    val (stdout, stdoutTruncated) =
      truncateOutput(result.out.text().trim, maxOutputBytes)
    val (stderr, stderrTruncated) =
      truncateOutput(result.err.text().trim, maxOutputBytes)
  ...
  def runStreaming(
    ...
    try
      val stdinSource =
        if closeStdin then
          os.ProcessInput.SourceInput(os.Source.WritableSource(""))
        else os.Inherit
      val result = os
        .proc(command)
        .call(
  ...
  private def truncateOutput(output: String, maxBytes: Int): (String, Boolean) =
    ...
    if bytes.length <= maxBytes then (output, false)
    else
