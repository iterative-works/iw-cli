...
object GitLabClient:
  ...
  /** Create an execCommand function that sets GITLAB_HOST for glab CLI.
    *
    * glab CLI needs GITLAB_HOST to identify the correct GitLab instance. This
    * creates a command executor with the host set in the environment.
    *
    * @param host
    *   Optional GitLab hostname (e.g., "gitlab.com")
    * @return
    *   execCommand function suitable for passing to GitLabClient methods
    */
  def execCommandWithHost(
      host: Option[String]
  ): (String, Array[String]) => Either[String, String] =
    host match
      case Some(h) =>
        (cmd, args) =>
          CommandRunner.execute(cmd, args, env = Map("GITLAB_HOST" -> h))
  ...
  /** Validate glab CLI prerequisites before fetching issues.
    ...
    *
    * @param repository
    *   GitLab repository in owner/project format
    * @param isCommandAvailable
    *   Function to check if command exists (injected for testability)
    * @param execCommand
    *   Function to execute shell command (injected for testability)
    * @return
    *   Right(()) if all prerequisites met, Left(error) otherwise
    */
...
Follow the prompts to sign in with your GitLab account.""".stripMargin
  ...
  /** Format error message for network errors.
    *
    * @param details
    *   Error details from glab CLI
    * @return
    *   User-friendly error message with remediation
    */
...
Check your network connection and try again.""".stripMargin
  ...
  /** Check if error message indicates resource not found.
    ...
    *   Error message from glab CLI
    * @return
    *   true if error indicates not found
    */
  ...
  /** Build glab CLI command arguments for fetching an issue.
    *
    * @param issueNumber
    *   GitLab issue number (e.g., "123")
    * @param repository
    *   GitLab repository in owner/project format (can include nested groups)
    * @return
    *   Array of command arguments for glab CLI
    */
  ...
  /** Parse JSON response from glab issue view command.
    *
    * Expected format: {"iid": 1, "title": "...", "state": "opened",
    * "assignees": [...], "description": "..."}
    *
    * @param jsonOutput
    *   JSON string from glab CLI
    * @param issueIdValue
    *   Full issue ID (e.g., "PROJ-123") used in the Issue object
    * @return
  ...
  /** Fetch a GitLab issue via glab CLI.
    *
    * @param issueIdValue
    *   Full issue ID (e.g., "PROJ-123") - number is extracted for API call
    * @param repository
    *   GitLab repository in owner/project format
    * @param isCommandAvailable
    *   Function to check if command exists (injected for testability)
  ...
  /** Build glab CLI command arguments for creating an issue.
    ...
    *   Issue title
    * @param description
    *   Issue description (empty string if not provided)
    * @param issueType
    *   Issue type (Bug or Feature) - mapped to GitLab labels
    * @return
    *   Array of command arguments for glab CLI
    */
  ...
  /** Build glab CLI command arguments without labels (for fallback).
    ...
    *   Issue description (empty string if not provided)
    * @return
    *   Array of command arguments for glab CLI without label
    */
  ...
  /** Parse URL response from glab issue create command.
    *
    * Expected format: https://gitlab.com/owner/project/-/issues/123 or
    * self-hosted: https://gitlab.company.com/owner/project/-/issues/123
    *
    * @param output
    *   URL string from glab CLI (may have trailing newline)
    * @return
  ...
  def parseCreateIssueResponse(output: String): Either[String, CreatedIssue] =
    val url = output.trim
    if url.isEmpty then Left("Empty response from glab CLI")
    else
  ...
  /** Check if error is related to labels (for graceful fallback).
    ...
    *   Error message from glab CLI
    * @return
    *   true if error indicates label-related issue
    */
  ...
  /** Create a new GitLab issue via glab CLI.
    *
    * If issue creation fails with a label-related error, retries without
    * labels.
    *
    * @param repository
    *   GitLab repository in owner/project format
  ...
  def createIssue(
    ...
    validateGlabPrerequisites(repository, isCommandAvailable, execCommand) match
      ...
      case Right(_) =>
        val args =
          buildCreateIssueCommand(repository, title, description, issueType)

        // Execute glab command
        execCommand("glab", args) match
          case Left(error) if isLabelError(error) =>
            // Retry without labels if label-related error
            val argsWithoutLabel = buildCreateIssueCommandWithoutLabel(
              repository,
              title,
              description
            )
  ...
  def createIssue(
    ...
    validateGlabPrerequisites(repository, execCommand = execCommand) match
      ...
      case Right(_) =>
        val args =
          buildCreateIssueCommandWithoutLabel(repository, title, description)
  ...
  /** Parse MR URL from glab mr create output.
    ...
    *   URL string from glab CLI (may have trailing newline)
    * @return
    *   Right(MR URL) on success, Left(error message) on failure
    */
  ...
  def createMergeRequest(
    ...
    validateGlabPrerequisites(repository, isCommandAvailable, execCommand) match
      ...
      case Right(_) =>
        val args =
          buildCreateMrCommand(repository, headBranch, baseBranch, title, body)
        execCommand("glab", args) match
          case Left(error)   => Left(error)
  ...
  /** Build glab CLI command arguments for fetching MR pipelines list.
    ...
    *   GitLab repository in owner/project format
    * @return
    *   Array of command arguments for glab CLI (without leading "glab")
    */
  def buildCheckStatusesCommand(
      mrNumber: Int,
      repository: String
  ): Array[String] =
    ...
    Array(
      "api",
      s"projects/$repo/merge_requests/$mrNumber/pipelines",
      "--method",
      "GET"
    )
  ...
  def buildPipelineJobsCommand(
    ...
    Array(
      "api",
      s"projects/$repo/pipelines/$pipelineId/jobs",
      "--method",
      "GET"
    )

  /** Build glab CLI command arguments for merging an MR with source branch
    * deletion.
    *
    * @param mrUrl
    *   URL of the MR to merge
  ...
  /** Parse pipelines list JSON to extract the latest pipeline ID.
    *
    * Expected format: [{"id": 123, "status": "success", ...}, ...] The first
    * element is the most recent pipeline.
    *
    * @param json
    *   Raw JSON string from glab pipelines API
    * @return
    *   Right(pipelineId) on success, Left(error) on failure
    */
  def parseGlabPipelinesJson(json: String): Either[String, Int] =
    try
      ...
      parsed match
        case arr: ujson.Arr =>
          if arr.value.isEmpty then Left("No pipelines found for this MR")
          else Right(arr.value.head("id").num.toInt)
        case other =>
          Left(
            s"Expected a JSON array but got: ${other.getClass.getSimpleName}"
          )
    catch
      case e: Exception =>
        Left(s"Failed to parse pipelines JSON: ${e.getMessage}")

  /** Map a GitLab job status string to a CICheckStatus.
    *
    * @param status
    *   GitLab job status string
    * @return
    *   Corresponding CICheckStatus
    */
  private def mapGitLabJobStatus(status: String): CICheckStatus =
    status match
      ...
      case "pending" | "running" | "created" | "waiting_for_resource" |
          "preparing" | "manual" =>
        CICheckStatus.Pending
      case _ => CICheckStatus.Unknown

  /** Parse jobs list JSON from GitLab pipeline jobs API into CICheckResult
    * list.
    *
    * Expected format: [{"id": 1, "name": "test", "status": "success",
    * "web_url": "..."}, ...]
    *
    * @param json
    *   Raw JSON string from glab jobs API
    * @return
    *   Right(checks) on success, Left(error) on parse failure
    */
  ...
  /** Fetch CI check statuses for an MR via glab CLI.
    *
    * Makes two API calls: first to get the latest pipeline ID, then to get job
    * statuses. Returns an empty list if no pipelines exist yet.
    *
    * @param mrNumber
    *   MR number (iid) to check
    * @param repository
    *   GitLab repository in owner/project format
    * @param isCommandAvailable
    *   Function to check if command exists (injected for testability)
    ...
    *   Function to execute shell command (injected for testability)
    * @return
    *   Right(checks) on success, Left(error message) on failure
    */
  def fetchCheckStatuses(
    ...
    validateGlabPrerequisites(repository, isCommandAvailable, execCommand) match
      ...
      case Right(_) =>
        execCommand(
          "glab",
          buildCheckStatusesCommand(mrNumber, repository)
        ) match
          ...
          case Right(pipelinesJson) =>
            parseGlabPipelinesJson(pipelinesJson) match
              ...
              case Right(pipelineId) =>
                execCommand(
                  "glab",
                  buildPipelineJobsCommand(pipelineId, repository)
                ) match
                  case Left(error) =>
                    Left(s"Failed to fetch pipeline jobs: $error")
