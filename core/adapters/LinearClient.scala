...
object LinearClient:
  ...
  /** Escapes special characters for safe JSON string embedding in GraphQL
    * queries. Handles backslashes, quotes, newlines, carriage returns, and
    * tabs.
    */
  ...
  def parseLinearResponse(json: String): Either[String, Issue] =
    try
      ...
      if !parsed.obj.contains("data") then
      ...
      else if parsed("data")("issue").isNull then Left("Issue not found")
      else
        ...
        if !issueData.obj.contains("identifier") then
          ...
          if !state.obj.contains("name") then
            ...
            val assignee =
              if issueData.obj.contains("assignee") && !issueData(
                  "assignee"
                ).isNull
              then Some(issueData("assignee")("displayName").str)
              else None

            val description =
              if issueData.obj.contains("description") && !issueData(
                  "description"
                ).isNull
              then
      ...
      case e: Exception =>
        Left(s"Failed to parse Linear response: ${e.getMessage}")
  ...
  def parseCreateIssueResponse(json: String): Either[String, CreatedIssue] =
    try
      ...
      case e: Exception =>
        Left(s"Failed to parse create issue response: ${e.getMessage}")
  ...
  def parseListRecentIssuesResponse(json: String): Either[String, List[Issue]] =
    try
      ...
      if !parsed.obj.contains("data") then
        ...
        results.foldRight[Either[String, List[Issue]]](Right(Nil)) {
          (item, acc) =>
            for
              issues <- acc
              issue <- item
      ...
      case e: Exception =>
        Left(s"Failed to parse list recent issues response: ${e.getMessage}")
  ...
  def parseSearchIssuesResponse(json: String): Either[String, List[Issue]] =
    try
      ...
      case e: Exception =>
        Left(s"Failed to parse search issues response: ${e.getMessage}")
  ...
  private def parseIssueNode(node: ujson.Value): Either[String, Issue] =
    if !node.obj.contains("identifier") then
      ...
      if !state.obj.contains("name") then Left("Missing 'name' field in state")
      else
  ...
  /** Create a new Linear issue via GraphQL mutation.
    ...
    *   Linear team UUID
    * @param token
    *   Valid Linear API token
    * @param labelIds
    *   Optional list of Linear label UUIDs to apply
    * @param backend
  ...
  def createIssue(
    ...
    try
      val mutation =
        buildCreateIssueMutation(title, description, teamId, labelIds)
  ...
  /** Search Linear issues by text query via GraphQL.
    *
    * @param query
    *   Search text (searches across title and description)
    * @param limit
