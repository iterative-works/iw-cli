...
object TmuxAdapter:
  ...
  def createSession(name: String, workDir: os.Path): Either[String, Unit] =
    ...
    val cleanEnv = sys.env.filter { case (key, _) =>
      key == "IW_HOME" || key == Constants.EnvVars.TmuxSocket || !key
        .startsWith("IW_")
    }

    val result = os
      .proc(tmuxCmd("new-session", "-d", "-s", name, "-c", workDir.toString))
      .call(
        check = false,
        stdout = os.Pipe,
        stderr = os.Pipe,
  ...
  def attachSession(name: String): Either[String, Unit] =
    ...
    val exitCode =
      ProcessAdapter.runStreaming(tmuxCmd("attach-session", "-t", name))
  ...
  def switchSession(name: String): Either[String, Unit] =
    ...
    val exitCode =
      ProcessAdapter.runStreaming(tmuxCmd("switch-client", "-t", name))
    if exitCode == 0 then Right(())
  ...
  /** Send keystrokes to a tmux session pane and press Enter to execute.
    ...
    *
    * @param sessionName
    *   The name of the tmux session to send keys to
    * @param keys
    *   The keystrokes to send (will be followed by Enter)
    * @return
    *   Right(()) on success, Left(error message) on failure
    */
  def sendKeys(sessionName: String, keys: String): Either[String, Unit] =
    val result =
      ProcessAdapter.run(tmuxCmd("send-keys", "-t", sessionName, keys, "Enter"))
    if result.exitCode == 0 then Right(())
    else
      Left(s"Failed to send keys to session '$sessionName': ${result.stderr}")
