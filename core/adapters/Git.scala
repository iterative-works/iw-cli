...
object GitAdapter:
  ...
  def getCurrentBranch(dir: os.Path): Either[String, String] =
    val result = ProcessAdapter.run(
      Seq("git", "-C", dir.toString, "rev-parse", "--abbrev-ref", "HEAD")
    )
    if result.exitCode == 0 then Right(result.stdout.trim)
    else Left(s"Failed to get current branch: ${result.stderr}")

  def getHeadSha(dir: os.Path): Either[String, String] =
    val result = ProcessAdapter.run(
      Seq("git", "-C", dir.toString, "rev-parse", "--short", "HEAD")
    )
    if result.exitCode == 0 then Right(result.stdout.trim)
    else Left(s"Failed to get HEAD SHA: ${result.stderr}")

  def hasUncommittedChanges(path: os.Path): Either[String, Boolean] =
    val result = ProcessAdapter.run(
      Seq("git", "-C", path.toString, "status", "--porcelain")
    )
    if result.exitCode == 0 then Right(result.stdout.trim.nonEmpty)
    else Left(s"Failed to check for uncommitted changes: ${result.stderr}")

  /** Inspect worktree state: staged, unstaged, and untracked files. */
  def getStagingCheck(dir: os.Path): Either[String, StagingCheck] =
    val result = ProcessAdapter.run(
      Seq("git", "-C", dir.toString, "status", "--porcelain")
    )
    if result.exitCode == 0 then
      Right(StagingCheck.fromPorcelain(result.stdout))
    else Left(s"Failed to inspect worktree state: ${result.stderr}")
  ...
  def createAndCheckoutBranch(
    ...
    val result =
      ProcessAdapter.run(Seq("git", "-C", dir.toString, "checkout", "-b", name))
  ...
  def checkoutBranch(name: String, dir: os.Path): Either[String, Unit] =
    val result =
      ProcessAdapter.run(Seq("git", "-C", dir.toString, "checkout", name))
    if result.exitCode == 0 then Right(())
  ...
  def stageFiles(paths: Seq[os.Path], dir: os.Path): Either[String, Unit] =
    if paths.isEmpty then Right(())
      ...
      val args =
        Seq("git", "-C", dir.toString, "add", "--") ++ paths.map(_.toString)
  ...
  def commit(message: String, dir: os.Path): Either[String, String] =
    val commitResult = ProcessAdapter.run(
      Seq("git", "-C", dir.toString, "commit", "-m", message)
    )
    if commitResult.exitCode != 0 then
      Left(s"Failed to commit: ${commitResult.stderr}")
    else getFullHeadSha(dir)
  ...
  def push(
    ...
    val args =
      if setUpstream then
        Seq("git", "-C", dir.toString, "push", "-u", "origin", branch)
  ...
  def diffNameOnly(
    ...
    val result = ProcessAdapter.run(
      Seq("git", "-C", dir.toString, "diff", "--name-only", baseline)
    )
    if result.exitCode == 0 then
      val files = result.stdout.trim
      if files.isEmpty then Right(Nil)
      else Right(files.split("\n").toList)
    else Left(s"Failed to diff from baseline '$baseline': ${result.stderr}")
  ...
  def getFullHeadSha(dir: os.Path): Either[String, String] =
    val result =
      ProcessAdapter.run(Seq("git", "-C", dir.toString, "rev-parse", "HEAD"))
    if result.exitCode == 0 then Right(result.stdout.trim)
    else Left(s"Failed to get full HEAD SHA: ${result.stderr}")
  ...
  /** Stage and commit a file, retrying once if the first attempt fails but the
    * file is still dirty.
    *
    * Handles transient failures (e.g., index.lock) and the case where commit
    * succeeds but SHA readback fails.
    */
  def commitFileWithRetry(
    ...
    val firstAttempt =
      stageFiles(Seq(path), dir).flatMap(_ => commit(message, dir))
    firstAttempt match
      ...
      case Left(firstErr) =>
        if !isFileDirty(path, dir) then
          ...
          stageFiles(Seq(path), dir).flatMap(_ => commit(message, dir)) match
            case Right(sha)     => Right(sha)
            case Left(retryErr) =>
              Left(s"$firstErr (retry also failed: $retryErr)")

  /** Fetch from origin and reset the current branch to the remote tracking
    * branch.
    *
    * Used after a squash-merge to advance the feature branch without spurious
    * conflicts. `reset --hard` is correct here because origin/{branch} is a
    * superset of local content.
    */
  def fetchAndReset(branch: String, dir: os.Path): Either[String, Unit] =
    val fetchResult =
      ProcessAdapter.run(Seq("git", "-C", dir.toString, "fetch", "origin"))
    if fetchResult.exitCode != 0 then
      Left(s"Failed to fetch from origin: ${fetchResult.stderr}")
    else
      val resetResult = ProcessAdapter.run(
        Seq("git", "-C", dir.toString, "reset", "--hard", s"origin/$branch")
      )
